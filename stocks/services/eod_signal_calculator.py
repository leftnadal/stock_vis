...
def profile_stage(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if elapsed > 30:
            logger.warning(
                f"SLOW: {func.__name__} took {elapsed:.1f}s (>30s threshold)"
            )
        else:
...
class EODSignalCalculator:
    ...
    def calculate_batch(self, target_date: date) -> pd.DataFrame:
        ...
        logger.info(
            f"[EODSignalCalculator] VIX regime: {regime} (target_date={target_date})"
        )
    ...
    def _load_price_data(self, target_date: date) -> pd.DataFrame:
        ...
        market_cap_map = {
            row["symbol"]: int(row["market_capitalization"])
            if row["market_capitalization"]
            else None
        ...
        logger.info(
            f"[EODSignalCalculator] 로드 완료: {len(df)}행, {df['symbol'].nunique()}종목"
        )
        return df
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["consecutive_down"] = df.groupby("symbol")["down"].transform(
            _count_consecutive
        )
        ...
        df["sector_avg_change"] = df.groupby(["date", "sector"])[
            "change_pct"
        ].transform("mean")
    ...
    def _detect_signals(
        self, df: pd.DataFrame, regime: str, target_date: date
    ) -> pd.DataFrame:
        """
        ...
        # ── P1: 연속 상승/하락 (3일 이상) ───────────────────────────
        today["sig_P1"] = (today["consecutive_up"] >= 3) | (
            today["consecutive_down"] >= 3
        )
        ...
        today["sig_P2_direction"] = np.where(
            today["change_pct"] > 0, "bullish", "bearish"
        )
        ...
        today["sig_P3_value"] = (
            (today["open"] - today["prev_close"])
            / today["prev_close"].replace(0, np.nan)
            * 100
        )
        ...
        # ── P4: 장대양봉/음봉 ────────────────────────────────────────
        today["sig_P4"] = (today["body_pct"] > t["P4_body_pct"]) & (
            today["body_ratio"] > 0.6
        )
        ...
        today["sig_P5_value"] = (
            today["close"] / today["high_52w"].replace(0, np.nan) * 100
        )
        ...
        # ── P7: 저가 대비 반등률 ─────────────────────────────────────
        today["sig_P7"] = (today["bounce_pct"] > t["P7_bounce_pct"]) & (
        ...
        # ── PV1: 가격-거래량 효율성 ──────────────────────────────────
        today["sig_PV1"] = (today["change_pct"].abs() > 2.0) & (
            today["vol_ratio"] < 1.0
        )
        today["sig_PV1_value"] = today["change_pct"]
        today["sig_PV1_direction"] = np.where(
            today["change_pct"] > 0, "bullish", "bearish"
        )
        today.loc[today["vol_ratio"].isna(), "sig_PV1"] = False

        # ── PV2: 매집 의심 ───────────────────────────────────────────
        today["sig_PV2"] = (today["vol_ratio"] > 2.0) & (
            today["change_pct"].abs() < 1.0
        )
        ...
        # ── MA1: 골든/데드크로스 ─────────────────────────────────────
        golden = (today["sma_50"] > today["sma_200"]) & (
            today["prev_sma_50"] <= today["prev_sma_200"]
        )
        dead = (today["sma_50"] < today["sma_200"]) & (
            today["prev_sma_50"] >= today["prev_sma_200"]
        )
        today["sig_MA1"] = golden | dead
        today["sig_MA1_value"] = today["sma_50"] - today["sma_200"]
        today["sig_MA1_direction"] = np.where(
            golden, "bullish", np.where(dead, "bearish", "")
        )
        # SMA가 NaN이면 False
        ma_nan = (
            today["sma_50"].isna()
            | today["sma_200"].isna()
            | today["prev_sma_50"].isna()
            | today["prev_sma_200"].isna()
        )
