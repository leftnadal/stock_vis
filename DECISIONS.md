...
**디렉터 오류 정정 3건**(장부 오염 방지):
1. 09-17 종결 문서의 *"`showEducation={false}`로 순서 역전"* 은 성립하지 않는다 — 그 prop은 교육 블록과 헤더 ⓘ를 **제거**한다. 순서 역전은 허브가 일반론을 **새로 그려야** 가능하다.
2. 같은 문서의 *"위젯 교육 토글"* 은 4장이 아니라 **2장**에만 있다.
3. *"의미문이 설명 대상에서 ~700px"* 은 **재현되지 않았다**(심리 카드 목업 재현 354px). 09-14 스크린샷 관측치이며 **현재값으로 인용 금지**.

**How to apply**: `frontend/app/market-pulse-v2/cards/SenseSlab.tsx`(트레이) · `cards/EducationNote.tsx`(일반론 공용) · `macro/page.tsx`(배선 + 심리·금리에 `showEducation={false}`). 회귀 가드 = `__tests__/market-pulse-v2/macro_hub.test.tsx`의 **DOM 순서 단언**(각 `data-guide` 래퍼에서 `sense-note`가 `education-note`보다 앞) — 순서가 다시 뒤집히면 즉시 RED. 물가·고용 카드는 교육 키가 둘이라 라벨이 중복되므로 **그 카드만 상수 `title`을 라벨로 사용**(디렉터 지시서 결함을 CC가 자율 판단으로 정정, 디렉터 승인).
...
**[배포·확증 부기 — EODSIG-FRESH-GATE-OBSERVE-0901, 2026-09-01]**: 배포 완료(main `e95b4378`·워커 재시작 2026-08-31 01:24 ET) → 첫 EOD 사이클(2026-08-31 18:30 ET) 관측으로 확증. **D2 판정 기준 정정**: 갭 봉합의 관측 가능량은 **당일 DailyPrice의 게이트 공급**(`freshness_gate: fetched` 로그 + `DailyPrice.created_at`이 게이트 발화 시각과 초단위 일치)이다. **EODSignal 행 수는 갭 지표가 아니다** — 파이프라인이 `signal_count>0`인 심볼만 저장(`eod_pipeline.py:162`)하므로, 행 부재 = 그날 14개 신호 0발화(정당)이며 **신선도 갭이 아니다**. OBSERVE-0901 실측(IONQ 사례): 게이트 fetched 3/3(IONQ·IREN·TLN) + DailyPrice 3/3(created_at 22:30:43~46 UTC ≡ 게이트 07:30:43~47 KST) + EODSignal 행 2/3(IONQ 0발화로 무행·정당) + 파이프라인 116.9s success + advisor 6/6 failed 0 pv=v1.2. → 가격 수준 실질 갭봉합 3/3 확증(처분: 사용자 승인 R4+R3''). cf. INC-005·D-EODUNIV-S1S2-DEGEN·[[project_eodsig_fresh_gate]].
...
**결정 (처분 A = 기록·관찰, 즉시 수리 안 함)**: EODUNIV-P15-V01 백필 커맨드(`backfill_eod_signals_universe`)가 생성한 비SP500 3종 EODSignal의 relation 시그널 S1(섹터상대강도)·S2(섹터소외주) **degeneracy를 즉시 수리하지 않는다**. daily-forward(전체 유니버스) **20거래일** 축적 후 S1/S2 발화율을 재측정하여 재평가한다. **[정정 2026-08-31, EODSIG-FRESH-GATE]** 기산일을 "2026-08-27"에서 **"D-EODSIG-FRESH-GATE 수리 배포일"로 리셋**한다 — 사유: 08-27~수리 전 구간은 신선도 갭(OBS-BRIEFING-0827 O1, 타겟 08-27 EODSignal 0/3)으로 타겟 daily-forward 관측점이 **아예 생성되지 않아** 20거래일 카운트의 분모가 없다. **재평가 트리거일 = 수리 배포일 + 20거래일(배포일은 배포 보고 시 기입).** **[확정 — OBSERVE-0901, 2026-09-01]** 수리 배포일 = **2026-08-31**(배포 후 첫 정상 daily-forward 일자 = OBSERVE-0901 D2 확증일) → **기산일 = 2026-08-31**(관찰 1일차·포함) · **만료 예정일 = 2026-09-28(월)** = 미국 거래일 기준 +20(기산일 포함 20번째 거래일, Labor Day 2026-09-07 휴장 제외, 캘린더 확정). **집계 규칙**: EODSignal **무행인 거래일 = 해당 종목 S1/S2 발화 0인 유효 관측일로 산입**한다(무행 ≠ 관측 누락 — 게이트가 당일 가격을 공급했고 파이프라인이 `signal_count>0`만 저장[`eod_pipeline.py:162`]하므로 0발화일도 20거래일 분모에 포함). cf. INC-005·D-EODSIG-FRESH-GATE 배포 부기.
...
## [2026-08-10] D-SECB-V2-LEN — v2 인용 길이 정책 = 캡 제거 (C) [sec-beta]

**결정**: v1의 300자 캡(프롬프트 지시 `prompts.py:23,66`)을 **제거**. v2는 "원문 verbatim·완결 단위" 지시. 저장측 sanity 경고선 **2,000자**(초과 시 **로그만** — 절단·거부 금지).

**선택지·가중합**(기준: verbatim 충실 0.35·표시 영향 0.20·저장/매칭 리스크 0.25·단순성 0.20):

| 안 | 내용 | 가중합 |
|---|---|---|
| A | 300 캡 유지 | 2.80 (R2 verbatim 목표 실격) |
| B | 캡 상향(500/1000) | 3.60 (부분 완화·임의 경계 잔존) |
| **C (채택)** | 캡 제거 + 2,000 sanity 경고(로그만) | **4.40** |

**Why**: ⑵ 실측 = v1 길이는 **캡 아티팩트**(정확히 300 도달 **409행/23.1%** 우측검열, p50=208). 캡은 DDL/코드 절단 아닌 프롬프트 지시. **다운스트림이 이미 `[:100]`/`[:200]` 절단**(tasks.py 389/472·validator_track_b)이라 evidence_text 확장 = **표시 무영향**·grounding verbatim 매칭만 **쉬워지는 방향**. 그릇=`TextField()` 무제한·**마이그 0**(⑴). 2,000 경고선=런어웨이 방어(절단은 verbatim 위배라 금지). **병진 승인 = C.**

## [2026-08-10] D-SECB-V2-COEXIST — v1/v2 병존 취급 = v1 보존·소비 필터 (B) [sec-beta]

**결정**: v2 재추출 행은 v1과 **병존**(v1 무접촉 보존). 이중집계는 **소비측 v2 필터**로 차단. v1 삭제 안 함.

**선택지·가중합**(기준: 해자 시점보존 0.30·가역성 0.25·소비 복잡도 0.25·마이그 0.20):

| 안 | 내용 | 가중합 |
|---|---|---|
| A | v1 대체(delete→insert v2) | 3.40 (비가역·해자 손실) |
| **B (채택)** | v1 보존·소비측 v2 필터 | **4.30** |
| C | 이력 테이블 신설(versioned) | 3.00 (마이그+복잡) |

**Why**: ⑶ 실측 = 모델에 **이력 테이블 없음**·`bulk_create` 삽입·**tuple unique 없음**(v2는 v1 삭제 안 하면 병존, 중복 그룹 104/232행 이미 자연 다중인용). `prompt_version` 필드가 v1/v2 구분자 → **소비 필터로 충분**(마이그 0). v1 삭제(A)=비가역·해자(시점보존) 손실. 이력 테이블(C)=마이그+복잡 과잉. **절대 조건 = 소비 지점 전수 v2 필터**(§0에서 grep 확정). ⚠️ `rematch .delete()`가 v1·v2 무차별 삭제하는지 실측 후 필요 시 필터 추가. **병진 승인 = B.**
...
- **✅ 가드 첫 실전 engage·통과 (BATCH-28 판별, 2026-08-12)**: **MGMT-BATCH-A**(`7b6f8e12`, author 08-11 10:36 = 훅 착지 `4f9589cf` 08-10 이후)가 common-bugs #95·#96 채번(실변경). 세션 브랜치 **`monorepo/sess-mgmt-batch-a` = sess-mgmt 패턴 일치** → **가드 engage 후 통과가 정상**(우회 아님). 훅 hardening의 첫 실전 발화 = 정상 동작 실증. (부수 관찰: `7b6f8e12`는 단일 parent = no-ff 자가머지 아닌 ff 착지 — 가드 판정과 무관한 landing-style 편차.)
...
- **본판정 = 유기(organic) 표면만 소비**: 노출·반복·클릭·미노출 적체 판정은 KIND=organic(`dashboard_eod`·`news_chip`·`chain_sight`) 표면만 집계. `coverage_detail`(KIND=audit)은 본판정에서 **제외**.
- **coverage_detail = 병행 표시 층**: 노출 스테이지에 유기 세그먼트와 **분리된 점검 층**(빗금·저채도 = 본선과 시각 위계 분리) + 수치 병기. 적체 목록의 점검 관측 ref엔 **"점검됨" 배지**.
...
**보강 — 삭제 계열 명령서는 전 항목에 존재 가드 (2026-08-13, MGMT-BATCH-29)**: worktree/브랜치 삭제 명령서는 **각 대상에 존재 가드**(worktree=`[ -d <path> ]`·브랜치=`git show-ref --verify --quiet`)를 붙여 부재 항목이 SKIP되게 한다. **근거**: CLEANUP-4~7 명령서 block 1(worktree remove 루프)이 존재 가드 없이 `set -e`라, 발급 시점 이미 선처리(병진 script)된 부재 worktree에서 첫 항목부터 halt → 잔여(sv-hooks-scratch)에 도달 못 함(블록 0 HALT·스코프 축소 재발급). **명령서 대상 목록은 발급 즉시 스테일**(병렬 세션·자동화가 상태를 바꿈) → 존재 가드 = 부분 선처리 내성. block 2(브랜치)는 show-ref 가드가 있어 SKIP 정상 작동한 대조 실증.
...
## D-BOUNDARY-LLM-CB · D-TONEGUARD-RETRY (2026-08-11, BOUNDARY-LLM 재산정 C+B + 8일 재시도)
...
## [2026-09-03] D-AGENT-S2 — 야간 도그푸딩 2단계: 루브릭 채점 [infra][process]

> 에이전트 B+C 단계(2026-08-24~27 확정)의 2단계. 1단계(정량)는 05:20 가동 중.

- **루브릭 단일 출처 = `frontend/lib/guide/`의 confirmed `coreQuestion`**(D-GUIDE-TRACK). 별도 평가 기준을 두지 않는다 — 유저 가이드와 채점 기준이 갈라지면 그 순간 둘 다 신뢰를 잃는다. draft 화면은 문안이 확정되지 않아 채점 대상에서 뺀다(현재 대상 5건: `/` · `/market-pulse-v2` · `/chainsight` · `/monitor` · `/portfolio`).
- **★ 핵심 계약 — 화면 밖 지식 금지**: 채점 입력은 **렌더된 화면에서 추출한 텍스트뿐**이다. 모델이 서비스에 대해 아는 것으로 화면을 평가하면 "이 화면은 원래 이런 뜻이지" 하고 **관점 간극을 재생산**한다(A안 기각 사유). 강제 수단 = **인용 의무**: 근거에 화면 텍스트에서 그대로 복사한 8자 이상 구절이 없거나, 인용이 실제 추출 텍스트에 없으면 그 채점은 **무효**(점수 미집계). 무효는 전체를 중단시키지 않고 "채점 불가"로 기록된다.
- **프롬프트 계약**: 화면마다 ⑴ 1~5 점수("coreQuestion에 **오늘 데이터로** 답이 되는가") ⑵ 2문장 이내 근거 + 인용 ⑶ "있으면 좋겠다"는 **전 화면 통틀어 1건**(개선 요구가 매일 쏟아지면 읽히지 않는다) ⑷ 화면 밖 지식 사용 금지 명시. **claude -p 1회 호출로 전 화면 일괄** 채점(비용 통제, tier3 패턴).
- **행동 규율(절대)**: GET 네비게이션 + 로그인 1회만. 클릭·폼 제출·POST/PUT/DELETE 금지. 로그인도 폼 클릭이 아니라 API 직접 호출 후 토큰을 `localStorage`에 주입한다. 도그푸딩 계정은 일반 권한이라(읽기 전용 개념 없음) **권한이 아니라 행동으로 제한**한다.
- **격리**: 2·3단계 실패는 경고로 기록하고 **1단계 점검과 메일은 그대로 발송**한다. 루브릭 산출물이 없으면 메일에 "측정 불가(사유)" 한 줄만 남는다(행위보존).
- **실행 트리**: 렌더 수집은 `sv-web-runtime/frontend/node_modules`의 playwright를 쓴다 — dogfood가 도는 `sv-worker-runtime`에는 `frontend/node_modules`가 없다(실측). node를 web 트리 cwd로 실행하고, **ESM이 `NODE_PATH`를 무시하므로 playwright 진입점을 절대 경로로 주입**한다.
- **앵커 fallback**: 가이드 `anchor` 이름이 실제 DOM과 어긋난 화면이 있다(실측 7건). 앵커 텍스트가 200자 미만이면 본문(`main`)으로 보완한다 — 그러지 않으면 모델이 인용할 문장 자체가 없어 무효만 쌓인다.
- **3단계는 범위 밖**: 관찰 후보(종목 추천)·성적 원장은 구현하지 않는다. 착수 전 **RC v3.0 분포 재측정이 선행**돼야 한다(눈금이 [0,1]로 바뀐 뒤의 분포를 모르는 채로 추천을 만들면 기준이 없다).

### 계정 생성 위임 (원문 인용)

> 병진 2026-09-03: **"DOGFOOD 전용 계정 생성을 CC에 위임한다 (signup API 경유, 이 1건 한정). 자격증명은 .env에만 기록하고 값은 출력하지 말 것."**

- 집행: signup API **201**(`dogfood_agent`, user id=15) + 로그인 **200** 검증. `password2`가 필수라 첫 시도 400 → 재시도 성공.
- 자격증명은 `.env`의 `DOGFOOD_USER`/`DOGFOOD_PASSWORD` 2키로만 존재하며 **값은 어디에도 출력하지 않았다**. 위임은 이 1건 한정이며 다른 prod DB 쓰기는 여전히 금지다.
...
## [2026-09-03] D-HUB-V02 — 허브 v0.2 사이클(번역층·슬라이스 순서) [frontend][backend][market_pulse]

> HUB-V02-RECON(`f039decb`) 사실 입력 기반, 병진 v0.1 검수 6의문 처분. 본 등재 = 사이클 결정 + S1 착수 근거.

- **번역층(⑴ 투자 행동 함의) = (c) 계층 문구**: 가중합 (c)4.10 > (b)LLM확장 3.95 > (a)정적 단일 3.45. 마진 0.15 → **타이브레이커 = 병진 명시 요구**("감이 안 옴"의 해소는 결정론 계층 문구가 우선, LLM은 S4 요약에서 별도). 레짐·지표 밴드별 계층(사실→함의→행동힌트) 문구 테이블. Translation 일일 태스크(C 선례) 재사용 가능.
...
- **결정**: `compute_breadth`/`calculate`의 기준일 미지정 시 **`DailyPrice`가 실재하는 최신 거래일(≤오늘)** 로 해석(`resolve_as_of_date`). 종전 `localdate()`(오늘)는 EOD 하루 지연을 미보정해 항상 total=0이었다(RECON A1).
- **캘린더 산술 금지**: 주말·공휴일을 날짜 빼기로 추정하지 않고 **실데이터에서 최신일 조회**(#117류 함정 회피).
...
### 애든덤 ADDENDUM-MON-ALERT-DOWN-3A-0917-B — 배수 4(리드타임) + 상한 15% + ET 정합 [3-A 최종]

**왜 다시 열었나**: 0917-A의 E-6 실측에서 배수 2는 6종 중 4종이 바닥값에 걸려 **기각된 안 B(고정 5%)와 발화 집합이 09-15·09-16 양일 모두 동일**했다(차분 0). 그대로 배포하면 기각한 안을 배포하는 셈이다. 동시에 리드타임이 역전돼 있었다 — GOOGL 4.2일 vs IONQ 2.0일, **가장 사나운 종목이 가장 짧은 경고**를 받았다.

**결정 ①(배수 재정의)**: 배수 = **리드타임**. 밴드 ÷ 평소 일간 변동률 ≈ 경고 후 손절까지의 평소 거래일 수. `NEAR_STOP_MULTIPLIER = 4` — 저녁 다이제스트 → 다음날 조사 → 그다음날 결정·실행. 이 정의에서 바닥값·상한에 걸리지 않는 종목은 **전부 같은 대응 시간**을 받는다(실측 확인: TLN·GEV·PLTR·IONQ 전부 4.00일).

**결정 ②(상한 신설)**: `NEAR_STOP_CAP = Decimal("0.15")`. 변동성 급등 시 밴드 폭주 방지. 평시 무작동 — 09-15 기준 IREN(median 4.262%)만 걸려 리드타임 3.52일로 단축된다.

**정정(집행자 수치 오류)**: 0917-A 보고의 "IONQ ≈2.3 / GEV ≈3.5"는 **틀렸다**. 발화 조건이 `close ≤ stop×(1+buffer)`이므로 필요 버퍼는 **손절가 기준**(`close/stop − 1`)인데 종가 기준 거리로 나눴다. 정확값 = IONQ 2.45 / GEV 3.73 / TLN 2.53 / GOOGL 15.4 / IREN 17.2 / PLTR 31.9. 계약 테스트에 `test_required_buffer_is_stop_anchored`로 이 경계를 직접 고정했다.

**결정 ③(ET 정합)**: 재발화 창 날짜 비교를 `notified.astimezone(ZoneInfo("America/New_York")).date()`로 바꾼다. 기존 UTC date 비교는 **beat가 22:45 UTC일 때만 우연히 일치**했고 여유는 75분뿐이었다 — 게다가 beat 시각은 코드가 아니라 DB `PeriodicTask`에 있다(공통버그 #28). `as_of = et_today()`이므로 비교도 ET 축으로 맞추는 것이 결합을 끊는다. **동작 변화 0**(현 beat 시각에서 결과 동일), retry가 UTC 자정을 넘겨도 어긋나지 않는다.

**E-6 재실측 (09-15) — 디렉터 사전 계산과 전건 일치**:

| 심볼 | median | 밴드 | 리드타임 | 필요버퍼 | 발화 |
|---|---|---|---|---|---|
| TLN | 1.594% | 6.38% | 4.00일 | 4.04% | 발화 |
| GEV | 2.123% | 8.49% | 4.00일 | 7.91% | 발화 |
| IONQ | 3.537% | 14.15% | 4.00일 | 8.68% | 발화 |
| PLTR | 2.208% | 8.83% | 4.00일 | 70.36% | — |
| GOOGL | 1.197% | 5.00%(바닥) | 4.18일 | 18.40% | — |
| IREN | 4.262% | 15.00%(상한) | 3.52일 | 73.39% | — |

09-16 기준 발화 = TLN·IONQ(GEV는 회복해 필요버퍼 13.08% > 밴드 8.49%). **안 B 대비 차분이 생겼다 — 안 C가 실제로 다른 안이 됐다.**

**게이트**: E-1 monitor **382 passed**(격리 DB·회귀 0) · E-2 기존 20개 diff 0 · E-3 `price_zone` DB 참조 0 · E-4 evidence 0 · E-5 `No changes detected` · E-6 상기 · E-7 메일 렌더 "손절 접근 3건" 확인. **마이그레이션 추가 없음 — 0012 그대로.**

### D-SESSION-NO-KILL-FOREIGN — 타 세션 자원은 종료하지 않는다 [harness]
...
## [2026-09-22] D-GUIDE-TABS-ADOPT — `dashboard.tabs`는 제거가 아니라 등재로 09-30 기한을 푼다 [frontend][dashboard][guide]

- **결정**: 고아 앵커 `dashboard.tabs`를 **ⓐ 가이드 등재**로 해소한다(ⓑ 앵커 제거·ⓒ 기한 연장 기각).
- **가중합**: ⓐ **8.15** / ⓑ 7.70 / ⓒ 6.95 — 가중치 기한방어 .30 · 제품가치 .25 · 검수시간 .20 · 되돌리기 .15 · 규약 .10. 마진 **0.45**.
- **Why**: 탭 줄은 화면을 두 갈래로 나누는 바깥 요소이고 신선도 배지를 품고 있다 — 설명이 붙을 값어치가 있는 자리라 제거(ⓑ)는 제품 가치를 버린다. 연장(ⓒ)은 기한부 allowlist 구조를 장식으로 만든다([[D-GUIDE-ORPHAN-BACKBONE]] 애든덤).
- **배치**: regions **맨 앞**. 탭 줄이 신선도 배지를 품은 바깥 요소이므로 화면 위→아래 순서와 번호를 맞춘다.
- **등재와 allowlist 제거는 같은 커밋**이어야 한다 — 죽은 allowlist 가드가 "등재 완료됐으니 제거하세요"로 RED를 낸다.
- **범위**: `reviewStatus`는 `confirmed` 유지(draft로 되돌리면 이 화면이 야간 루브릭 대상에서 빠져 채점 추세가 끊긴다). 화면 컴포넌트 무접촉. `chainsight.backbone`은 별도 소유(chainsight 앱 트랙)라 손대지 않는다.
