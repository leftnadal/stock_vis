...
**확정 로그 스키마 (한 번 고정)**:
`(user_id, signal_date, ticker, horizon, confidence, conf_ver, rank, presented_as)`
— SignalAccuracy join 키 = `(ticker=stock, signal_date, horizon)`. user_id=제시 대상, rank=top-N 순위, presented_as=제시 수준 마커.
