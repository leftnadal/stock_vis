...
def classify(
    ...
    mag7_hits += [
        s for s in _find_symbols(text_upper, MAG7_SYMBOLS) if s not in mag7_hits
    ]
    mag7_name_hits = _find_keywords(text_lower, MAG7_NAMES)
    if mag7_hits or mag7_name_hits:
        return ClassificationResult(
            MarketPulseNews.Category.MAG7, mag7_hits, mag7_name_hits
        )
    ...
    sector_sym += [
        s for s in _find_symbols(text_upper, SECTOR_ETF_SYMBOLS) if s not in sector_sym
    ]
    sector_name = _find_keywords(text_lower, SECTOR_NAMES)
    if sector_sym or sector_name:
        return ClassificationResult(
            MarketPulseNews.Category.SECTOR, sector_sym, sector_name
        )

    index_sym = list({s for s in explicit if s in INDEX_SYMBOLS})
    index_sym += [
        s for s in _find_symbols(text_upper, INDEX_SYMBOLS) if s not in index_sym
    ]
    index_name = _find_keywords(text_lower, INDEX_NAMES)
    if index_sym or index_name:
        return ClassificationResult(
            MarketPulseNews.Category.INDEX, index_sym, index_name
        )
