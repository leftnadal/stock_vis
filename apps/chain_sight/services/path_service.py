...
def build_path_signature(path_nodes: List[str], edge_snapshot: List[Dict]) -> str:
    ...
    rel_counts = Counter(edge["type"] for edge in edge_snapshot if edge.get("type"))
    if not rel_counts:
        rel_label = "관계 미확정"
...
def build_initial_why_now(path_nodes: List[str], edge_snapshot: List[Dict]) -> Dict:
    ...
    strong_count = sum(
        1 for e in edge_snapshot if e.get("status") in ("confirmed", "probable")
    )
...
def compute_landmark_scores(
    middle_nodes: List[str], full_path: List[str]
) -> List[float]:
...
def _compute_bridge_scores(
    middle: List[str], full_path: List[str], centrality: Dict[str, Dict]
) -> List[float]:
...
def _compute_sector_uniqueness(middle: List[str], full_path: List[str]) -> List[float]:
    """경로 내 섹터 다양성 기여도."""
