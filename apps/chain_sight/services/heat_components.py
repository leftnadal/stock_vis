...
구현 범위 (이 슬라이스):
  C1, C2a, C3, C4, C5, C6, C7 = 실계산 함수.
  C2b, C8              = 계약 만족 스텁 + missing_reason 명시 (각 후속 슬라이스).
...
# ────────────────────────────── C2b 발행 (스텁 — 후속 슬라이스) ──────────────────────────────
def c2b_issuance(*args, **kwargs) -> dict:
    """
    C2b 발행 신호 (§5.2) — **이 슬라이스 미구현 스텁**.

    424B5 90일 건수 z + IPO 캘린더 90일 건수 z 의 평균. 원천 = FMP sec-filings-search/
    form-type + ipos-calendar (일 단위 날짜 창 순회 수집, collect_theme_filings_task 필요).
    수집 beat 가 아직 없어 데이터가 없다 → 계약 만족 결측 반환. 후속 슬라이스에서 구현.
    """
    return make_component(None, raw=None, missing_reason="c2b_not_collected")
