...
def _get_latest_prices(market_date: date):
    ...
    latest_dates = (
        DailyPrice.objects.filter(date__lte=market_date)
        .values("date")
        .distinct()
        .order_by("-date")[:2]
    )
...
def get_price_seeds(market_date: date) -> dict:
    ...
    prev_close_sq = Subquery(
        DailyPrice.objects.filter(stock=OuterRef("stock"), date=prev_date).values(
            "close_price"
        )[:1],
    ...
    qs = (
        DailyPrice.objects.filter(date=latest_date)
...
def get_sector_outlier_seeds(market_date: date) -> dict:
    ...
    stocks_with_return = (
        Stock.objects.exclude(change_percent__isnull=True)
...
def get_relation_change_seeds(market_date: date) -> dict:
    ...
    changed = (
        RelationConfidence.objects.filter(last_observed_at__gte=yesterday)
    ...
    for rc in changed:
        old_idx = (
            status_order.index(rc.previous_status)
            if rc.previous_status in status_order
            else -1
        )
        new_idx = (
            status_order.index(rc.relation_status)
            if rc.relation_status in status_order
            else -1
        )
...
def get_comention_surge_seeds(market_date: date) -> dict:
    ...
    surges = CoMentionEdge.objects.filter(
        last_co_mention_date__gte=week_ago, co_mention_count__gte=5
...
def build_sector_summary(seeds: dict, market_date: date) -> list[dict]:
    ...
    for symbol, info in seeds.items():
        ...
        if current_top is None or info["signal_count"] > seeds.get(current_top, {}).get(
            "signal_count", 0
        ):
    ...
    for sector, summary in sector_map.items():
        sector_stocks = (
            Stock.objects.filter(sector=sector)
