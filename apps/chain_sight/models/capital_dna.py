...
class CompanyCapitalDNA(models.Model):
    ...
    buyback_yield = models.DecimalField(
        max_digits=8, decimal_places=4, null=True, blank=True
    )
    total_shareholder_return_pct = models.DecimalField(
        max_digits=8, decimal_places=4, null=True, blank=True
    )

    net_cash_position = models.BigIntegerField(null=True, blank=True)
    cash_to_market_cap = models.DecimalField(
        max_digits=8, decimal_places=4, null=True, blank=True
    )

    capital_type = models.CharField(
        max_length=30, blank=True, choices=CAPITAL_TYPE_CHOICES
    )
