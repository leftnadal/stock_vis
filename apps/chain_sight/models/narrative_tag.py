...
class CompanyNarrativeTag(models.Model):
    ...
    narrative_strength = models.CharField(
        max_length=10, blank=True, choices=STRENGTH_CHOICES
    )
    narrative_sentiment = models.CharField(
    ...
    analyst_consensus = models.CharField(
        max_length=20, blank=True, choices=CONSENSUS_CHOICES
    )
    analyst_target_vs_price = models.DecimalField(
        max_digits=8, decimal_places=4, null=True, blank=True
    )
    analyst_revision_trend = models.CharField(
        max_length=20, blank=True, choices=REVISION_CHOICES
    )

    generated_by = models.CharField(
        max_length=20, blank=True, choices=GENERATED_BY_CHOICES
    )
