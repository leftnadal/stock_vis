...
@extend_schema(
    tags=["Chain Sight"], responses={200: OpenApiTypes.OBJECT, 404: OpenApiTypes.OBJECT}
)
class ChainSightGraphView(APIView):
    ...
    def get(self, request, symbol):
        ...
        if not result["center"]:
            return Response(
                {"error": f"Stock {symbol} not found in graph"},
...
class ChainSightSuggestionView(APIView):
    ...
    def get(self, request, symbol):
        ...
        # 3. 뉴스 동시출현
        co_mentions = (
            CoMentionEdge.objects.filter(symbol_a=symbol)
            .union(CoMentionEdge.objects.filter(symbol_b=symbol))
            ...
            categories.append(
                {
                    "id": "same_sector",
                    "label": "같은 섹터",
                    "count": same_sector[0]["cnt"],
                    "rel_types": ["BELONGS_TO_SECTOR"],
                    "top_tickers": [],
                    "strength": "weak",
                }
            )
...
class ChainSightTraceView(APIView):
    ...
    def get(self, request):
        ...
        if not from_sym or not to_sym:
            return Response(
                {"error": "from, to 파라미터 필수"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"from": from_sym, "to": to_sym, "found": False, "error": str(e)}
            )
...
class SectorGraphView(APIView):
    ...
    def get(self, request, sector):
        ...
        if limit < 1:
            return Response(
                {"error": "Invalid parameter: limit must be >= 1"},
        ...
        try:
            ...
            if not raw_nodes:
                return Response(
                    {"error": f"Sector {sector} not found or empty"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            caps = sorted(
                [float(n.get("market_cap") or 0) for n in raw_nodes], reverse=True
            )
            total = len(caps)
                ...
                nodes.append(
                    {
                        ...
                        "volume_ratio": seed_info.get("volume_ratio", 0.0)
                        if seed_info
                        else 0.0,
                        "is_seed": ticker in seed_set,
                        "seed_type": seed_info.get("seed_type") if seed_info else None,
                        "seed_reasons": seed_info.get("seed_reasons", [])
                        if seed_info
                        else [],
...
class NeighborGraphView(APIView):
    ...
    def get(self, request, symbol):
        ...
        try:
            ...
            stock_map = {
                s.symbol: s for s in Stock.objects.filter(symbol__in=neighbor_symbols)
            }
                ...
                neighbors.append(
                    {
                        ...
                        "market_cap": float(ns.market_capitalization)
                        if ns and ns.market_capitalization
                        else 0,
                        "daily_return": ns_return,
                        "volume_ratio": ns_seed.get("volume_ratio", 0.0)
                        if ns_seed
                        else 0.0,
                        "is_seed": ticker in seed_set,
                        "seed_type": ns_seed.get("seed_type") if ns_seed else None,
                        "seed_reasons": ns_seed.get("seed_reasons", [])
                        if ns_seed
                        else [],
                        "relation": {
                            "type": n["rel_type"],
    ...
    def _build_chain_signals(self, page, page_size, sector, today):
        ...
        for s1, s2 in pairs:
            ...
            try:
                ...
                trigger = (
                    ", ".join(translated)[:100] if translated else "시드 기반 탐색"
                )
                        ...
                        "edges": [
                            {
                                "type": e.get("type", ""),
                                "score": float(
                                    e.get("truth_score") or e.get("market_score") or 0
                                ),
                            }
                            for e in path_edges
                        ],
