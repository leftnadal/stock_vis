...
def run_seed_selection():
    ...
    if not seeds:
        ...
        if prev:
            ...
            logger.warning(
                f"No seeds for {today}, carried over from previous market date"
            )
        else:
    ...
    for symbol, info in seeds.items():
        ...
        market_cap = (
            float(stock.market_capitalization) if stock.market_capitalization else 0
        )
    ...
    logger.info(
        f"Seed selection complete: {len(seeds_list)} seeds, {len(sector_summary)} sectors"
    )
            ...
            """,
                {
                    "ticker": symbol,
                    "heat": round(heat, 4),
                    "ps": round(price_sig, 4),
                    "vs": round(volume_sig, 4),
                    "rs": round(relation_sig, 4),
                    "ns": round(news_sig, 4),
                },
            )
            processed += 1
