...
def test_post_e4_returns_200_with_valid_request(
    ...
    with patch(
        "apps.portfolio.api.views.run_e4_coach", return_value=mock_llm_response_e4
    ) as mock_run:
...
def test_post_e4_response_passes_e4output_validation(
    ...
    with patch("apps.portfolio.api.views.run_e4_coach", return_value=mock_llm_response_e4):
        response = api_client.post(E4_ENDPOINT, data=e4_request_body, format="json")
...
def test_post_e4_service_exception_returns_500_no_stacktrace(
    ...
    with patch(
        "apps.portfolio.api.views.run_e4_coach",
...
def test_post_e4_service_returns_drifted_output_caught_by_serializer(
    ...
    with patch("apps.portfolio.api.views.run_e4_coach", return_value=drifted):
        response = api_client.post(E4_ENDPOINT, data=e4_request_body, format="json")
