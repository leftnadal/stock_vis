...
def test_post_e3_returns_200_with_valid_request(
    ...
    with patch(
        "apps.portfolio.api.views.run_e3_coach", return_value=mock_llm_response_e3
    ) as mock_run:
        response = api_client.post(E3_ENDPOINT, data=e3_request_body, format="json")
...
def test_post_e3_response_passes_e3output_validation(
    ...
    with patch("apps.portfolio.api.views.run_e3_coach", return_value=mock_llm_response_e3):
        response = api_client.post(E3_ENDPOINT, data=e3_request_body, format="json")
...
def test_post_e3_missing_required_field_returns_400(api_client, e3_request_body):
    ...
    data = response.json()
    assert any(
        "concentration_metrics" in k for k in data
    ) or "concentration_metrics" in str(data)
...
def test_post_e3_service_exception_returns_500_no_stacktrace(
    ...
    with patch(
        "apps.portfolio.api.views.run_e3_coach",
...
def test_post_e3_service_returns_drifted_output_caught_by_serializer(
    ...
    with patch("apps.portfolio.api.views.run_e3_coach", return_value=drifted):
        response = api_client.post(E3_ENDPOINT, data=e3_request_body, format="json")
