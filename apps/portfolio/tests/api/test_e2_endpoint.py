...
def test_post_e2_returns_200_with_valid_request(
    ...
    with patch(
        "apps.portfolio.api.views.run_e2_coach", return_value=mock_llm_response_e2
    ) as mock_run:
        response = api_client.post(E2_ENDPOINT, data=e2_request_body, format="json")
...
def test_post_e2_response_passes_e2output_validation(
    ...
    with patch("apps.portfolio.api.views.run_e2_coach", return_value=mock_llm_response_e2):
        response = api_client.post(E2_ENDPOINT, data=e2_request_body, format="json")
...
def test_post_e2_missing_required_field_returns_400(api_client, e2_request_body):
    ...
    data = response.json()
    assert any(
        "portfolio_return_1y" in k for k in data
    ) or "portfolio_return_1y" in str(data)
...
def test_post_e2_service_exception_returns_500_no_stacktrace(
    ...
    with patch(
        "apps.portfolio.api.views.run_e2_coach",
...
def test_post_e2_service_returns_drifted_output_caught_by_serializer(
    ...
    with patch("apps.portfolio.api.views.run_e2_coach", return_value=drifted):
        response = api_client.post(E2_ENDPOINT, data=e2_request_body, format="json")
