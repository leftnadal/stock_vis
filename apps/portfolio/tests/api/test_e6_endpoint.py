...
def test_post_e6_returns_200_with_valid_request(
    ...
    with patch(
        "apps.portfolio.api.views.run_e6_coach", return_value=mock_llm_response_e6
    ) as mock_run:
        response = api_client.post(E6_ENDPOINT, data=e6_request_body, format="json")
...
def test_post_e6_response_passes_e6output_validation(
    ...
    from apps.portfolio.schemas.commentary_output import E6Output

    with patch("apps.portfolio.api.views.run_e6_coach", return_value=mock_llm_response_e6):
        response = api_client.post(E6_ENDPOINT, data=e6_request_body, format="json")
...
def test_post_e6_service_exception_returns_500_no_stacktrace(
    ...
    with patch(
        "apps.portfolio.api.views.run_e6_coach",
...
def test_post_e6_service_returns_drifted_output_caught_by_serializer(
    ...
    with patch("apps.portfolio.api.views.run_e6_coach", return_value=drifted):
        response = api_client.post(E6_ENDPOINT, data=e6_request_body, format="json")
