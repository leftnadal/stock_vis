...
| **Phase 3-D (FE 리디자인 v1)** — 실제 값 카드 + AI 분석 + 미니차트 (PR-7~10) | phase3_frontend_redesign.md | **PR-7 100% / PR-8 90% / PR-9 부분 / PR-10 부분** | B+ |
...
| `HypothesisEvent` (특허) | integrated_roadmap §1.2 | `thesis/models/learning.py` | (B) 모델 + 인덱스 완료. 13가지 event_type 모두 일관되게 발행되는지는 미확인 — `thesis_views.py`/`thesis_builder.py` 코드 경로에서 import만 확인됨 |
| `ValidityRecord` (특허) | integrated_roadmap §1.3 | `thesis/models/learning.py` | (B) 모델 완료, 마감 시 자동 생성 로직 일부 경로에만. **`is_synthetic` 필드 부재** → Phase 3-B 미대비 |
...
| `KeywordCache` | redesign v2 (talking_builder) 부산물 | `thesis/models/keyword.py` | (A) 빌더 재설계 부산물 — 설계 외 추가됨, `0006~0007` 마이그레이션 |

### 2.2 백엔드 서비스 (설계 §4.1, §5)

| 서비스 | 설계에 명시 | 구현 위치 | 상태 |
| --- | --- | --- | --- |
| `thesis_builder.py` (LLM 대화) | §4.1, §5.5 | `thesis/services/thesis_builder.py` | (A) 구현 (wizard + LLM 모드) |
...
| `news_connector.py` (뉴스→가설) | §4.1 | 없음 | (C) 미구현. 대체로 `keyword_collectors/news.py`가 제한적으로 그 역할 |
...
| `POST /{id}/indicators/auto/` | §6.1 | ViewSet action | (A) |
| `GET /{id}/dashboard/` | §6.1, §6.2 | `DashboardView` | (A) — 카드뷰 + 히트맵 데이터 + raw_value + ai_summary + notable_changes 모두 포함 |
| `GET /{id}/snapshots/` (히스토리) | §6.1 | 없음 | **(C) 미구현** — 그래프뷰/히스토리탭 차단 |
| `GET /{id}/summary/` (쉐이크) | §6.1 | 없음 | **(C) 미구현** — `dashboard.thesis.ai_summary`로 대체 사용 중 |
| `GET /{id}/indicators/{iid}/readings/` | redesign v1 | `IndicatorReadingsView` (FMP fallback 포함, 최대 1825일) | (A) — 설계 대비 max 90일이 1825일로 확장됨 |
| `GET /{id}/indicators/{iid}/explanation/` ([근거]) | §6.1 | 없음 | **(C) 미구현** — 대시보드 응답의 `description`+`recommendation_reason` 필드로 부분 대체 |
...
| `/thesis/new` | 6단계 대화형 빌더 | `app/thesis/new/page.tsx` | (A) FE-PR-3 완료 + Phase A LLM proposal 모드 추가 (`work_done/phase_a_llm_builder.md`) |
...
| **FE-PR-3** 빌더 6단계 | `task_done/FE-PR-3_*.md` + `plan_review_v3.md` | 9개 빌더 컴포넌트 (`BottomSheet`, `ChatBubble`, `MultiSelectFooter`, `NewsSelector`, `OptionButton`, `PremiseCard`, `ProgressBar`, `SuggestionCard`, `TextInput`) | (A) 모두 존재 |
| **FE-PR-4** 지표 설정 | `task_done/FE-PR-4_*.md` | `IndicatorSetupCard`, `AddIndicatorSheet`, `RecommendCard` | (A) |
| **FE-PR-5** 대시보드 v1 (달위상) | `task_done/FE-PR-5_*.md` | `OverallMoon`, `DashboardIndicatorCard`, `RecentChange` | **(D) Phase 3-D에서 폐기 완료** — `OverallMoon`/`DashboardIndicatorCard`/`RecentChange` 디렉토리에서 0건 (삭제 완료). `MoonPhase`(common)는 `(list)/page.tsx`와 `ThesisListCard`에 잔존 |
| **FE-PR-6** 알림 + 마감 | `task_done/FE-PR-6_*.md` | `AlertCard`, `AlertFilterTabs`, `EmptyAlerts`, `OutcomeSelector`, `CloseConfirmDialog` | (A) |
| **FE-PR-7** (구 계획) 대시보드 탭 + 전제 CRUD | Phase2_completion §8 | `Tabs`, `PremiseEditor` … | **(D) 폐기** — `phase3_frontend_redesign.md`에서 별도 탭 구조 도입 금지 결정 |
| **FE-PR-7** (리디자인) 백엔드 raw_value | `phase3_frontend_redesign.md` §4 | migration `0004_add_display_unit` / `0005_populate_display_unit`, `IndicatorReadingsView`, `DashboardView` 응답 확장 | (A) 완료 |
| **FE-PR-8** (구 계획) 히트맵 + 지표 상세 편집 | Phase2_completion §8 | `HeatmapView`, `IndicatorDetailEdit` | **(D) 폐기** — Finviz 히트맵 별도 화면 미도입 |
| **FE-PR-8** (리디자인) 실제 값 카드 + AI 분석 | `phase3_frontend_redesign.md` §5 | `RealValueIndicatorCard`, `AISummarySection`, `NotableChangesSection` | (B) 컴포넌트 모두 존재. **그러나 page.tsx는 `RealValueIndicatorCard` 대신 `IndicatorRow`를 채택** — `RealValueIndicatorCard`는 테스트(`__tests__/thesis/RealValueIndicatorCard.test.tsx`)에서만 import됨 |
| **FE-PR-9** (구 계획) 히스토리 탭 (recharts 라인) | Phase2_completion §8 | `HistoryTab`, `SnapshotTimeline` | **(C) 미구현** — `GET /{id}/snapshots/` 백엔드 부재로 차단 |
| **FE-PR-9** (리디자인) 미니차트 + 기간 | `phase3_frontend_redesign.md` §6 | `ChartToggleButton`, `PeriodSelector`, `IndividualMiniCharts`, `QuarterlySparkline` | (B) 4개 모두 파일은 존재하지만 **앞 3개는 production 코드에서 사용되지 않음** (자체 정의 파일에서만 import). 실제 차트는 `IndicatorRow` 내부에 인라인 통합 + `QuarterlySparkline`만 사용 |
| **FE-PR-10** (구 계획) 마감 아카이브 + ValidityMatrix | Phase2_completion §8 | `ArchiveList`, `ValidityMatrix`, `ClosedThesisDetail` | **(C) 미구현** |
| **FE-PR-10** (리디자인) AI 파이프라인 (Celery) | `phase3_frontend_redesign.md` §7 | `tasks/summary.py`, `notable_changes` 채우기 | (B) `tasks/summary.py` 존재. `notable_changes`가 `alert_engine` 이벤트 기반으로 구조화돼 일관되게 채워지는지 추가 검증 필요 |
...
- 현재 구현 (`app/thesis/[thesisId]/page.tsx`): **단일 페이지 세로 스크롤** — 탭 컨테이너 부재. 지표는 `IndicatorRow`로 토글식 펼침/차트 인라인 구조
...
- 현재 구현:
  - `IndicatorRow` 인라인 차트로 **지표별 raw_value 시계열은 존재** (1M / 1Y / 3Y / 5Y 4단계, FMP fallback 포함, 5년치)
...
- 현재 구현:
  ...
  - 그러나 **마감 가설 목록 페이지/라우트 없음** (`/thesis` 목록은 `status='active'` 위주 필터)
...
| 가설 마감 시점 도래 알림 + 복기 흐름 | §3.8, §3.9 | (B) `target_date`는 alert_type에 정의되어 있으나 복기 UI 부재 |
...
| [근거] 시스템 (롱프레스 용어 설명, 전제 탭 맥락 설명, 지표 [근거] 탭) | (B) 빌더 단계의 long-press 설명은 FE-PR-3에 구현. 관제실 단계의 [근거] 탭은 부분 대체(`description`+`recommendation_reason` 인라인 표시). 별도 `GET /{id}/indicators/{iid}/explanation/` 엔드포인트는 부재 |
...
### 3.9 Phase 3-D 내부 갭 (리디자인 PR-7~10)

| PR | 의도 | 실제 채택 | 갭 |
| --- | --- | --- | --- |
| PR-7 BE | display_unit + raw_value 노출 | 완료 (migration 0004/0005, view 확장) | 없음 |
| PR-8 FE 카드 | `RealValueIndicatorCard` 단독 사용 | `IndicatorRow`(인라인 차트 + 토글 + 분기 sparkline) 채택 | **`RealValueIndicatorCard`는 production unused** — 테스트 외 import 0. 기능적으로는 IndicatorRow가 동등 이상 커버하지만 설계 문서와 실제 코드 베이스 간 불일치 |
| PR-8 FE AI 분석 | `AISummarySection` 빈 문자열 시 미렌더링 | page.tsx에서 사용 중 | 동일 |
| PR-8 FE 변화 | `NotableChangesSection` `severity: warning/info` | page.tsx에서 사용 중 | 동일 |
| PR-9 FE 차트 토글 | `ChartToggleButton` + `PeriodSelector` + `IndividualMiniCharts` | **production unused** — `IndicatorRow` 내부에 차트 인라인 + 자체 period 버튼 | 컴포넌트 3개 dead code. 설계의 "전체 차트 영역 토글" UX는 "지표별 행 클릭 펼치기" UX로 대체됨 |
| PR-9 FE 정리 | `OverallMoon`/`DashboardIndicatorCard`/`RecentChange` 삭제 + `scoreToPhaseMeta` 삭제 | 모두 삭제 완료 | 잔존: `MoonPhase`(common)가 `(list)/page.tsx`와 `ThesisListCard`에서 사용 중 — 리디자인 PR-9 §6.8 체크리스트 "MoonPhase import 검색 후 결정" 단계에서 의도적 잔존이 결정된 것으로 추정 |
| PR-10 BE Celery | `generate_thesis_summaries` (07:30) + alert→notable_changes 변환 | `tasks/summary.py` 존재 | `notable_changes` JSON 구조가 `alert_engine` 이벤트와 1:1 매핑되어 채워지는지 미확인. severity가 "warning/info"가 아닌 alert 모델의 4단계(low/medium/high/critical)로 흘러나오는지 확인 필요 |
...
2. **Phase A LLM 빌더 (`work_done/phase_a_llm_builder.md`)**: 위자드형 빌더 위에 Gemini one-shot proposal 모드(PR-1~3) + Hardening(PR-4~7) + KeywordCache(PR-8~12 일부)를 이미 머지함 — 설계 메인 문서 §2와 다른 트랙으로 빌더가 진화 중.
...
5. **분기 지표(`quarterly_metric_fetcher.py`, `QuarterlySparkline.tsx`)**: 설계 문서에 명시 없음. 실서비스 운영 중 추가된 기능 → 설계 문서 업데이트 필요. 마이그레이션 0008(`add_metrics_data_source`)이 이를 뒷받침.
6. **3계층 아키텍처 일치도**: `monitoring_views.py`의 `DashboardView`가 모델 직접 쿼리 + service 호출 + FMP 클라이언트 인라인 호출이 혼합 (`_fetch_fmp_history`). CLAUDE.md의 "View → Service → Models" 패턴과 일부 어긋남 — 단, 설계 §6.2 응답 형태 우선이 더 중요해 보임.
7. **`IndicatorReadingsView` max_days 확장**: 리디자인 설계에서는 `min(days, 90)` 인데 현재 코드는 `min(days, 1825)` (5년) — 분기 지표 5Y 차트 요구사항이 추가되며 확장됨. 설계 문서 ↔ 코드 불일치.
8. **`MOCK_DASHBOARD` enable**: `USE_MOCK` 플래그는 mock.ts에 토글되어 있어 production 빌드에서도 mock 데이터로 fallback될 위험. 환경 변수 게이트 확인 필요.
...
> **Phase 1 + Phase 3-D는 거의 완성**. Phase 3-C (사용자 질문의 "깊이 + 회고 + 프로필")는 **공식 폐기 후 Phase 3-D로 부분 대체**됐으며, 마감 아카이브 / DNA 프로필 / 커뮤니티는 **모델만 있고 UI·API 0%**. 다만 Phase 3-D 자체도 설계 컴포넌트(`RealValueIndicatorCard`, `IndividualMiniCharts` 등) 일부가 production에서 채택되지 않은 채 dead code로 남아 설계 문서와 코드 베이스가 어긋남.
...
### 5.4 코드 위생 권고 (코드 수정 금지 원칙으로 보고만)

- **dead code 삭제 또는 통합**: `RealValueIndicatorCard.tsx`, `ChartToggleButton.tsx`, `PeriodSelector.tsx`, `IndividualMiniCharts.tsx` 4종은 `IndicatorRow`로 사실상 통합됨. 둘 중 하나로 일원화 필요.
- **설계 문서 갱신**: `phase3_frontend_redesign.md` §6은 "지표별 독립 미니차트"로 정의하지만, 실제 채택은 "지표 행 클릭 토글 + 인라인 차트"임. 설계와 구현 간 표현 통일 필요.
- **MoonPhase 잔존 정책 명시**: 리디자인 PR-9 체크리스트에서 "import 검색 후 결정"으로 미정 상태. 현재 `(list)` 영역에서 사용 중인 점을 설계 문서에 명문화하거나 대체 컴포넌트로 정리.
