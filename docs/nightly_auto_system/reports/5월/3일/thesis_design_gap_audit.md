...
## 0. 사전 컨텍스트 — "Phase 3"의 다중 정의

`docs/thesis_control/` 안에서 "Phase 3"이라는 용어는 세 문서에 걸쳐 **서로 다른 산출물을 가리키며 충돌**한다. 본 감사는 세 정의를 모두 평가한다.
...
**해석**: redesign 문서가 원안 FE-PR-7~11을 사실상 재정의하면서 "내부 점수 숨기고 실제 값 보여주기"로 방향 전환. CLAUDE.md "구현 상태 — Thesis Control Phase 3 진행 중 (FE-PR-7~11)" 표기는 **현실과 불일치**하며, 실제 진행 중 항목은 redesign PR-10 단일 산출물(그조차 미시작)이다.

### 5월 2일 → 5월 3일 변동

`git log -- thesis/ frontend/{components,app,lib}/thesis/ docs/thesis_control/` 결과 **5월 2일 ~ 5월 3일 사이 thesis 영역 신규 커밋 0건**, working tree 변경 0건. 마지막 thesis 영역 커밋은 4월 27일 `b3b9bdf` (FMP Starter rate_limit + indicator_catalog 표시 이름 4건 BE/FE 통일)이다. **5월 2일 갭 분포가 그대로 유지**되어 본 감사는 5월 2일 결과를 근거로 검증·재확인하고, §6 일관성 이슈 / §7 한 줄 결론을 갱신했다.
...
| `FE-PR-5_dashboard.md` | **부분 폐기 (D)** | `OverallMoon` / `DashboardIndicatorCard` / `RecentChange` 는 redesign에서 삭제 명시. `RealValueIndicatorCard` / `AISummarySection` / `NotableChangesSection` 으로 교체. **단** — `app/thesis/[thesisId]/page.tsx` 본 코드 경로는 `RealValueIndicatorCard` 가 아닌 **`IndicatorRow` (인라인 토글 차트 포함)** 을 최종 사용. `RealValueIndicatorCard.tsx` 는 테스트 1건만 import → 사실상 **dead UI 컴포넌트** (§6 H7 참조) |
...
| 항목 | 설계 명세 | 현재 상태 |
|------|----------|----------|
| Celery `generate_thesis_summaries` | 매일 07:30 KST. Stage 3 상태 + raw_value 변화 → Gemini 2.5 Flash 동기 호출 → 2~3문장 한국어 요약 | **부재** — `tasks/eod_pipeline.py` 463줄 안에 함수 미정의 |
| `ThesisSnapshot.ai_summary` 채움 로직 | 위 태스크가 저장 | **부재** — `services/snapshot_builder.py` 의 `defaults={...}` 에 `ai_summary` 키 없음. 모델 default `''` 그대로 |
| `notable_changes` 풍부화 | alert_engine 이벤트(direction_flip / sharp_move / extreme_volatility) → `{indicator_id, type, severity, description, raw_value_before/after, change_pct}` 변환 | **부분 (B)** — `snapshot_builder.py` 의 단순 `\|delta\|≥0.3` 필터로만 채움. severity 분기 / raw_value before/after 매핑 / AI 풍부화 없음 |
| Beat schedule 등록 | DatabaseScheduler에 `PeriodicTask.objects.create(...)` (CLAUDE.md 버그 #28 회피 패턴) | **부재** |

**FE 영향**: `app/thesis/[thesisId]/page.tsx` L75~78 가 `<AISummarySection summary={data.thesis.ai_summary} ... />` 를 렌더링하나, 응답 `ai_summary === ''` 이므로 `if (!summary) return null` 분기로 인해 **사용자 화면에서 항상 미렌더링**. `NotableChangesSection` 도 단순 score delta 기반으로만 동작. → PROGRESS.md "다음 세션 #15" 와 동일.
...
|----|----------|------------|--------------|--------------|
| **FE-PR-7** | 대시보드 3탭 (관제 / 상세 / 히스토리) + 전제 CRUD UI | `[thesisId]/(detail)/`, `[thesisId]/history/` 미생성. 현재 `[thesisId]/page.tsx`는 단일 스크롤 화면 | `DashboardTabs`, `DetailTab`, `PremiseEditor` 부재 | `ThesisPremiseViewSet` 재활용 가능 (BE OK) |
...
## 5. Phase 1 이벤트/유효성 인프라 — 5월 2일 검증 결과 재확인

`thesis/views/thesis_views.py` 와 `thesis/services/thesis_builder.py` 를 본 감사에서 다시 확인. 결과 모두 일치 (5월 2일 검증과 동일).
...
| H3 | redesign PR-10 (`generate_thesis_summaries`) | 2026-03-18 FINAL 결정 후 약 7주 경과, PROGRESS.md / TASKQUEUE.md 에 진행 신호 없음 (`PROGRESS.md` audit P0 후속 큐 #15 에만 plain 항목으로 등재). FE 이미 렌더링 코드 들어가 있어 사용자 가치만 비실현 |
...
| 5월 2일 (2026-05-03) | Phase 1 이벤트 hook **모두 검증 완료 (90% A)**. 4월 27일 이후 thesis 영역 코드 변경 1건 (`b3b9bdf` indicator_catalog 표시명 동기화) — 설계 갭은 동일 |
| **5월 3일 (2026-05-04)** | 5월 2일 → 5월 3일 사이 thesis 영역 변경 **0건** (커밋·working tree 모두). 갭 분포 동일하며, 본 감사는 §3.7 잔재 항목 재검증 (`scoreToPhaseMeta` 가 5개 파일에서 여전히 사용 — redesign 문서 §10 단정과 불일치) + §6 일관성 이슈 H10~H12 신규 추가 (Thesis 모델의 status choices 와 부재 필드를 설계 4.2 와 대조) |
