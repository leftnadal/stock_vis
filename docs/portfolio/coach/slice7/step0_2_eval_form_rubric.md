# Slice 7 Part 1 Step 0.2 — Manual Eval Form (Rubric Applied)

> **목적**: Slice 6 Part 4 raw 데이터를 `manual_eval_rubric.md` 기준으로 재평가하여
> H3 분기(haiku insight gap > 0.50) 진위 검증. 결과는
> `scripts/slice7/remeasure_h3_with_rubric.py`로 자동 판정.
> **결과 저장 경로**: 작성 완료 후 본 파일을
> `docs/portfolio/coach/slice7/step0_2_eval_filled_rubric.md`로 복사 후 점수 기입.
> **평가 방법**: 각 entry에 대해 naturalness / insight를 1~5점으로 평가.
> **Blind**: LLM provider 라벨 가림. preset_id만 노출 (외삽 분석용).
> **Scale**: 1=매우 부족, 2=부족, 3=보통, 4=좋음, 5=매우 좋음
> **Rubric 참조**: `docs/portfolio/coach/manual_eval_rubric.md` (필수 — A·B·C 섹션 우선 숙지)

## 평가 기준 (요약 — 상세는 rubric 참조)

- **naturalness**: rubric §A. 1점·5점도 적극 사용 (안전 평가 회피)
- **insight**: rubric §B. preset 의도 반영 + 행동 시사점 구체성으로 4~5점 변별

**KPI 자가 체크** (rubric §E):

- [ ] 1~5 전 범위 사용 (min=1, max=5 시도)
- [ ] 분포 폭 ≥ 3.0
- [ ] 5점 비율 5~20%
- [ ] note 작성률 ≥ 30%
