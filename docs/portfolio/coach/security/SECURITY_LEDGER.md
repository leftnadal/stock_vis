...
> slice14 머지 시 3-way merge로 §1·§2 통합. (본 통합본은 c-bundle 시점에
> 작성됨 — `slices/c_bundle_merge.md` 참조.)
...
| **C-3** | Django 5.1.7 → 5.2.14 LTS 마이그레이션 | ✅ **종결** | `00dc18d` | `security/c3-django-52-lts` | $0 | [slices/c3_django52.md](slices/c3_django52.md) |
| **ML-1** | ML mini-slice (torch/transformers CVE) | ⏸ 후순위 | - | - | - | - |

### 시리즈 ID 컨벤션
- **C-N**: Cleanup/CVE 시리즈 — Slice 14 후속 보안 트랙. **C-3로 종료** (C 묶음 머지 후 마무리).
...
| **SEC-#3** | `pyOpenSSL` 25.3.0 → 26.0.0 (CVE-2026-27448, CVE-2026-27459 — 각 1건, 총 2건) | C-2 closing (2026-05-24) | 메이저 경계(25→26), 비파괴 범위 밖. C-2 §범위 밖 명시. cryptography 46.x ↔ pyopenssl 25.x는 호환 유지. | 별도 메이저 슬라이스. cryptography 다음 메이저 업그레이드 시점에 함께 검토. C-3 영향 없음 (Django 5.2.14 + cryptography 46.0.7 + pyopenssl 25.3.0 호환 유지). |
...
C-3 자체로 **Django 5.1 EOL** 영향(PYSEC-2025-14/37/47/104~109 + CVE-2025-59682
등 11건)을 일괄 해소 — `slices/c3_django52.md` 참조.
...
| 2026-05-25 | C-3 종결 등록 (Django 5.2.14 LTS) + §3 Django 5.1 EOL 11건 일괄 해소 명시 | C-3 closing (`security/c3-django-52-lts`) |
| 2026-05-25 | C-1+C-2+C-3 통합 (c-bundle 머지) — LEDGER §1·§2·§3·변경 이력 UNION 해결 | c-bundle Step 1.4 |
