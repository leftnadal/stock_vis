...
| 파일 | 의도 |
|------|------|
| `pyproject.toml` | 직접 의존성 4건 갱신 (python-dotenv, requests, pytest) + cryptography 신규 명시(패치 업그레이드 + 명시 승격) |
...
| **cryptography** | (미명시) | **`^46.0.7`** (신규) | caret. **`46.0.3 → 46.0.7` 패치 업그레이드 + `pyproject.toml` 미명시 → 명시 승격** |
