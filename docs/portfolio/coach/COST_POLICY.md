...
> **마지막 갱신**: 2026-05-18 (Slice 11 Step 0 D-3 $4.00 상향)
...
| -------------------------- | --------- |
| 누적 임계                  | **$4.00** |
| CostGuard 사전 경고 (80%)  | **$3.20** |
| Slice 12+ 재상향 트리거    | **$3.40** (도달 시 결정 사이클) |

**비용 패턴 명문화** (Slice 11 D-3): `$1.00 → $1.50 → $2.00 → $3.00 → $4.00` —
슬라이스 누적 압력에 맞춰 단조 증가. 평균 슬라이스 단독 비용 ~$0.33, 5-Part trio 슬라이스
($0.40~0.60 예상)도 cap $1.00 안에 흡수 가능.

**cap 정책 유지**:
- `mini-slice cap: $0.50` (Slice 10 #48 신설)
- `full-slice cap: $1.00` (Slice 9 #43 신설)
...
| -------------------------- | ---------------------------------------------------- |
| **슬라이스 단독 cap**      | **$1.00**                                            |
| **mini-slice cap** (Slice 10 #48 도입) | **$0.50** (정식 cap의 1/2)                 |
| Cap 사전 경고 (80%)        | $0.80 (정식) / $0.40 (mini-slice)                    |
| Cap 위반 시 처리           | 30분 이내 결정 사이클 진입 (재상향 vs 슬라이스 분리) |

**Mini-slice cap 도입 근거** (Slice 10 #48):

- Step 0 단독 슬라이스(`Step 9 슬롯 없음, 단일 부채 격리`)는 LLM 매트릭스 부담이 없음.
- count_tokens API 등 무료 호출 활용 시 $0 실현 가능.
- 정식 cap $1.00 절반인 $0.50로 안전 마진 확보 + 비용 결정 사이클 보수화.
- 적용 기준 + 첫 사례: [MINI_SLICE_PATTERN.md](MINI_SLICE_PATTERN.md) 참조.
...
| Slice 9            | $3.00     | Slice 8 #44 rationale 흡수 + Slice 10 mini-slice $0.22 흡수 (마진 7.9%)     |
| **Slice 11 (현재)** | **$4.00** | **Slice 11 trio 5-Part 예측 $0.40~0.60 + Slice 12+ 흡수 (마진 25%+)**       |
...
- 누적 임계 80% 도달 (Slice 11+: **$3.20**)
- 재상향 트리거 도달 (Slice 11+: **$3.40** — 결정 사이클 사전 진입)
- 슬라이스 cap 80% 도달 (현재 $0.80 full / $0.40 mini)
- 슬라이스 cap 직접 위반
- 단건 임계 위반 누적 3회

### §3.1 임계 도달 시 행동 (Slice 11 D-3 갱신)

- 누적 ≥ 80% ($3.20): 경고 + Slice 매트릭스 축소 검토
- 누적 ≥ 재상향 트리거 ($3.40): 결정 사이클 사전 진입 (Slice 12+ 임계 결정)
- 누적 ≥ 90% ($3.60): 즉시 매트릭스 축소 + 부채 처리 우선
- 누적 ≥ 100% ($4.00): Slice 진행 중단, 정책 재검토
- **슬라이스 cap ≥ 80% ($0.80 full / $0.40 mini)**: 경고 + 남은 단계 비용 외삽 점검
- **슬라이스 cap ≥ 100% ($1.00 full / $0.50 mini)**: 슬라이스 진행 중단, 결정 사이클 진입
...
| 2026-05-18 | Slice 10 Step 0 (#48): mini-slice cap **$0.50** 신설 (MINI_SLICE_PATTERN.md) |
| 2026-05-18 | Slice 11 Step 0 (D-3): 임계 $3.00 → **$4.00**, 80% 경고 $2.40 → **$3.20**, 재상향 트리거 **$3.40** 명문화 |
