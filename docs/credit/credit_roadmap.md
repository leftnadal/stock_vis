...
- "주중 예비치" 해석은 오독 — FMP 채권 ETF nav는 전일(T-1) EOD 확정치이며 게시 시각만 무작위(P2a-1c에서 정정).
- 세션 노트는 실측으로만 인용 — "잔여 worktree 2개" 스테일 사례(실측 = credit 잔여 0, 실측 선행 원칙 재확인).
- 수치 우연 일치는 지시서에 명기 — 07-30 NAV 79.31 = 08-03 EOD close 79.31(혼동 방지, 07-30 신설은 08-03과 무관).
- 시각 산술은 설정 원문 인용으로 확정 — crontab `tz=Asia/Seoul` 1줄이 KST/ET 논쟁을 종결.
