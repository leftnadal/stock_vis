...
## J. 신선도·생존 판정 3원칙 (D-EOD-FRESH-ROOT-NOT-SCHEDULE, 2026-09-10)

헬스체크·신선도·루브릭 등 "데이터/서비스가 살아있나"를 판정하는 코드는:
① 지연은 **거래일 기준**으로 센다(캘린더-일수 임계 금지).
② 자기 실행 시점이 **대상 갱신 이전**인지 반영한다("아직 안 만들어진 것"을 "없어진 것"으로 판정 금지).
③ 실패 메시지에 **"이 판정이 틀릴 수 있는 조건"**을 함께 적는다.
(정본 서사 = common-bugs "신선도·생존 판정 3원칙". 근거 = 측정장치 오탐 5건 누적.)
