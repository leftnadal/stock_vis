...
class MetricCalculator:
    ...
    def calculate_for_symbol(self, symbol: str) -> dict:
        ...
        for fy in fiscal_years:
            ...
            for metric_code, (value, value_status, exclusion_reason) in calculated.items():
                try:
                    dec_val = Decimal(str(round(value, 6))) if value is not None else None
                    CompanyMetricSnapshot.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            'source_detail': {'calculated_at': timezone.now().isoformat()},
                        }
                    )
                    metrics_saved += 1
    ...
    def calculate_for_symbols(self, symbols: list[str] = None) -> dict:
        ...
        for i, symbol in enumerate(symbols):
            ...
            if (i + 1) % 50 == 0:
                logger.info(f"Calc progress: {i+1}/{total} (success={success}, errors={fail})")

        return {'total': total, 'success': success, 'errors': fail, 'error_details': error_details[:20]}

    def _calculate_all_metrics(self, inc, bal, cf, prev_inc, prev_bal, prev_cf, prev_bal_3y, stock):
        """
        ...
        results['operating_margin'] = self._calc_ratio(inc.operating_income, inc.total_revenue)
        ...
        results['revenue_growth_yoy'] = self._calc_growth(
            _safe(inc.total_revenue), _safe(prev_inc.total_revenue) if prev_inc else None
        )
        results['operating_income_growth'] = self._calc_growth(
            _safe(inc.operating_income), _safe(prev_inc.operating_income) if prev_inc else None
        )
        ...
        results['current_ratio'] = self._calc_ratio(bal.total_current_assets, bal.total_current_liabilities)
        ...
        results['ocf_to_net_income'] = self._calc_ratio(cf.operating_cashflow, inc.net_income)
        ...
        results['ar_to_revenue'] = self._calc_ratio(bal.current_net_receivables, inc.total_revenue)
        results['inventory_turnover_days'] = self._calc_inventory_days(bal, inc)
        results['inventory_vs_sales_growth'] = self._calc_inv_vs_sales(bal, inc, prev_bal, prev_inc)
        results['sga_to_revenue'] = self._calc_ratio(inc.selling_general_and_administrative, inc.total_revenue)
        results['asset_turnover'] = self._calc_ratio(inc.total_revenue, bal.total_assets)
    ...
    def _calc_fcf_growth(self, cf, prev_cf) -> tuple:
        if not cf or not prev_cf:
            return (None, 'missing', '전년도 데이터 없음')
        fcf_now = (_safe(cf.operating_cashflow) or 0) - abs(_safe(cf.capital_expenditures) or 0)
        fcf_prev = (_safe(prev_cf.operating_cashflow) or 0) - abs(_safe(prev_cf.capital_expenditures) or 0)
        if abs(fcf_prev) < 1:
    ...
    def _calc_interest_coverage(self, inc, bal, prev_inc) -> tuple:
        ...
        if prev_inc:
            ...
            if prev_interest and prev_interest != 0 and prev_op is not None:
                ...
                if prev_val != 0 and ((val > 0) != (prev_val > 0)) and abs(val) > abs(prev_val) * 10:
