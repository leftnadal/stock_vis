...
CATEGORY_METRICS = {
    ...
    'growth': ['revenue_growth_yoy', 'operating_income_growth', 'fcf_growth_yoy', 'rev_growth_vs_industry'],
    'financial_structure': ['debt_to_equity', 'current_ratio', 'interest_coverage', 'net_debt_to_ebitda', 'cash_runway_years', 'short_term_debt_pct'],
    'cash_flow_quality': ['fcf_margin', 'ocf_to_net_income', 'capex_to_ocf', 'accruals_ratio', 'fcf_conversion', 'cash_from_ops_trend'],
    'operational_efficiency': ['dso', 'ar_to_revenue', 'inventory_turnover_days', 'inventory_vs_sales_growth', 'sga_to_revenue', 'asset_turnover'],
    'dilution_shareholder': ['dilution_3y_cum', 'sbc_to_revenue', 'buyback_offsets_sbc', 'net_shareholder_yield'],
...
class CategorySignalCalculator:
    ...
    def calculate_for_symbol(self, symbol: str, fiscal_year: int = None) -> dict:
        ...
        for category, metric_codes in CATEGORY_METRICS.items():
            signal, score, reason, metric_count, valid_count, contribs = self._calc_category(
        ...
        return {'symbol': symbol, 'fiscal_year': fiscal_year, 'signals_created': signals_created}
    ...
    def calculate_for_symbols(self, symbols: list[str] = None) -> dict:
        ...
        for i, symbol in enumerate(symbols):
            ...
            if (i + 1) % 50 == 0:
                logger.info(f"Signal progress: {i+1}/{total} (success={success}, fail={fail})")

        return {'total': total, 'success': success, 'errors': fail}

    def _calc_category(self, stock, fiscal_year, category, metric_codes, is_special, special_note):
        """
        단일 카테고리의 signal 계산.
