...
from replay_protocol_v012 import parse_action
...
def run_one(spec,config,call,outroot):
 ...
 result={'run_id':rid,'execution_started_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'harness_version':'0.1.2','spec':spec,'config':config,'status':'running','calls':[],
 ...
 try:
  for index in range(config['max_calls_per_run']):
   ...
   remaining-=usage
   text=response['answer'];action,wrapping=parse_action(text)
   result['calls'][-1]['response_wrapping']=wrapping
