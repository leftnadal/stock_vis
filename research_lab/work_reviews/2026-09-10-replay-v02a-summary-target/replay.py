"""Two-arm v0.2A hosted replay with experiment-local completion telemetry."""
...
def execute():
    ...
    try:
        for spec in plan["runs"]:
            run = out / spec["run_id"]
            run.mkdir()
            messages_raw = (ROOT / "visible" / f"{spec['run_id']}.json").read_bytes()
            if sha_bytes(messages_raw) != spec["input_sha256"]:
                raise ValueError("input_hash_mismatch")
            messages = json.loads(messages_raw)
            payload = {key: plan["config"][key] for key in ["model", "temperature", "top_p", "seed", "reasoning"]}
            payload.update({"messages": messages, "max_tokens": plan["config"]["max_tokens"], "stream": False, "n": 1})
            ...
            completion = classify_response(
                ...
                requested_max_tokens=plan["config"]["max_tokens"],
            ...
            result = {
                ...
                "interpretation": "calibration_only_pending_independent_semantic_review",
            }
