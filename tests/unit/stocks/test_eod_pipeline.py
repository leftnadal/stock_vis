...
def mock_pipeline_deps(target_date):
    ...
    with patch('packages.shared.stocks.services.eod_pipeline.EODSignalCalculator') as mock_calc_cls, \
         patch('packages.shared.stocks.services.eod_pipeline.EODSignalTagger') as mock_tagger_cls, \
         patch('packages.shared.stocks.services.eod_pipeline.EODNewsEnricher') as mock_enricher_cls, \
         patch('packages.shared.stocks.services.eod_pipeline.EODJSONBaker') as mock_baker_cls:
...
class TestEODPipeline:
    ...
    def test_pipeline_failure_sets_failed_status(self, target_date):
        ...
        with patch('packages.shared.stocks.services.eod_pipeline.EODSignalCalculator') as mock_calc:
