...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"대상: {len(symbols)}개, FMP: {'Yes' if options['use_fmp'] else 'No'}"
        )
        ...
        self.stdout.write(
            f"수집: {stats['total_pairs']}개 pairs (Finnhub {stats['finnhub_success']}/{stats['symbols_processed']})"
        )
        ...
        self.stdout.write(
            f"로드: {result['loaded']}개, Neo4j PEER_OF: {result['neo4j_total']}개"
        )
