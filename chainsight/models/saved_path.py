...
class SavedPath(models.Model):
    ...
    edge_snapshot = models.JSONField(blank=True, null=True, help_text='저장 시점 관계 스냅샷')
    ...
    recheck_count = models.PositiveIntegerField(
        default=0, help_text='Recheck 횟수. 2회 이상 + 24h → watching→active',
    )
...
class PathAction(models.Model):
    ...
    saved_path = models.ForeignKey(
        SavedPath, on_delete=models.CASCADE, related_name='actions',
    )
    action_type = models.CharField(
        max_length=20, choices=ActionType.choices, db_index=True,
    )
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
