...
STATUS_ORDER = {
    'hidden': 0, 'stale': 1, 'weak': 2, 'probable': 3, 'confirmed': 4,
}
...
def run_recheck(saved_path: SavedPath) -> RecheckResult:
    ...
    result.suggested_action, result.suggested_reason = _decide_suggestion(result, saved_path)
...
def _compute_diffs(old_snapshot, new_snapshot):
    ...
    if len(old_snapshot) != len(new_snapshot):
        logger.warning(f'edge_snapshot 길이 불일치: old={len(old_snapshot)}, new={len(new_snapshot)}')
...
def _classify_edge_change(old: Dict, new: Dict) -> str:
    ...
    if old_status in ('confirmed', 'probable', 'weak') and new_status in ('hidden', 'stale'):
        return 'broken'
...
def _build_updated_why_now(saved_path, result):
    ...
    strong_count = sum(1 for e in new_snapshot if e.get('status') in ('confirmed', 'probable'))
    return {
