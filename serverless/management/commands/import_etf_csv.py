...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "etf_symbol", type=str, help="ETF 심볼 (예: ARKK, SOXX, XLK)"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # 프로필 없으면 기본값으로 생성
            self.stdout.write(
                self.style.WARNING(f"ETF 프로필 없음, 새로 생성합니다: {etf_symbol}")
            )
        ...
        # 기존 데이터 삭제
        deleted, _ = ETFHolding.objects.filter(
            etf=profile, snapshot_date=today
        ).delete()
        if deleted:
