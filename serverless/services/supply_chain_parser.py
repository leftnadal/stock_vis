...
class SupplyChainParser:
    ...
    def _extract_customers(
        self, text: str, source_symbol: str
        ...
        for pattern, base_confidence in self.CUSTOMER_PATTERNS:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                try:
                    ...
                    # 패턴에 따라 회사명/퍼센트 추출
                    if len(groups) == 2:
                        if groups[0].isdigit() or (
                            groups[0] and groups[0][0].isdigit()
                        ):
                    ...
                    if not company_name or not self._is_valid_company_name(
                        company_name
                    ):
                        continue
    ...
    def match_to_stock(self, company_name: str) -> Optional[str]:
        ...
        try:
            ...
            for suffix_pattern in self.COMPANY_SUFFIXES:
                search_name = re.sub(
                    suffix_pattern, "", search_name, flags=re.IGNORECASE
                )
            search_name = search_name.strip()

            # 1. 정확한 이름 매칭
            stock = Stock.objects.filter(stock_name__iexact=company_name).first()
            ...
            # 2. Suffix 제거 후 매칭
            stock = Stock.objects.filter(stock_name__iexact=search_name).first()
            ...
            stock = Stock.objects.filter(stock_name__istartswith=search_name).first()
    ...
    def _deduplicate_relations(
        self, relations: List[SupplyChainRelation]
    ) -> List[SupplyChainRelation]:
        """중복 관계 제거 (높은 신뢰도 유지)"""
        ...
        for rel in relations:
            ...
            if key not in seen:
                ...
                # 더 높은 신뢰도로 교체
                existing = seen[key]
                if confidence_order.get(rel.confidence, 3) < confidence_order.get(
    ...
    def calculate_confidence(
        self, has_percent: bool, percent_value: Optional[float], has_qualifier: bool
    ) -> str:
        """
        신뢰도 계산
