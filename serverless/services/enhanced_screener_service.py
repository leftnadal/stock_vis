...
class EnhancedScreenerService:
    ...
    ENHANCED_FILTERS = {
        "pe_ratio_min",
        "pe_ratio_max",  # P/E Ratio
        "pb_ratio_min",
        "pb_ratio_max",  # P/B Ratio
        "roe_min",
        "roe_max",  # Return on Equity
        "roa_min",
        "roa_max",  # Return on Assets
        "eps_growth_min",
        "eps_growth_max",  # EPS Growth
        "revenue_growth_min",
        "revenue_growth_max",  # Revenue Growth
    ...
    def _fetch_fmp_screener(self, fmp_params: Dict, limit: int) -> List[Dict]:
        ...
        try:
            # 캐시 키 생성
            cache_key = (
                f"fmp:enhanced_screener:{hash(frozenset(fmp_params.items()))}:{limit}"
            )
    ...
    def _fetch_key_metrics_batch(self, symbols: List[str]) -> Dict[str, Dict]:
        ...
        logger.info(
            f"Metrics 캐시: {len(result)}개 HIT, {len(uncached_symbols)}개 MISS"
        )
    ...
    def _fetch_single_key_metrics(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            response = self.fmp_client._make_request(
                "/stable/key-metrics-ttm", params={"symbol": symbol.upper()}
            )
    ...
    def _merge_metrics(
        self, stocks: List[Dict], metrics: Dict[str, Dict]
    ) -> List[Dict]:
        """
    ...
    def _apply_enhanced_filters(self, stocks: List[Dict], filters: Dict) -> List[Dict]:
