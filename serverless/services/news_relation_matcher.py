...
class NewsRelationMatcher:
    ...
    def process_recent_news(self, hours: int = 24) -> Dict:
        ...
        for article in articles:
            ...
            entity_symbols = list(
                article.entities.filter(entity_type="equity")
                .values_list("symbol", flat=True)
                .distinct()
            )
            co_mentions_updated += self._update_co_mentions(
                entity_symbols, article.title
            )
        ...
        for rel_type, patterns in RELATION_PATTERNS.items():
            for pattern in patterns:
                for match in pattern.finditer(text):
                    ...
                    if not self._is_valid_name(source_name) or not self._is_valid_name(
                        target_name
                    ):
                        continue
