...
class KeywordContextBuilder:
    ...
    def estimate_context_tokens(
        context: Dict[str, Any], include_prompt: bool = False
    ) -> int:
        """
        컨텍스트 토큰 추정
        ...
        context_tokens = int(
            len(context_str) * KeywordContextBuilder.CHAR_TO_TOKEN_RATIO
        )
    ...
    def compare_batch_vs_individual(
        num_stocks: int, avg_tokens_per_stock: int = 300
    ) -> Dict[str, Any]:
        """
        ...
        batch_cost = (batch_input / 1_000_000) * INPUT_COST_PER_1M + (
            batch_output / 1_000_000
        ) * OUTPUT_COST_PER_1M

        individual_cost = (individual_input / 1_000_000) * INPUT_COST_PER_1M + (
            individual_output / 1_000_000
        ) * OUTPUT_COST_PER_1M

        savings = individual_cost - batch_cost
        savings_percent = (
            (savings / individual_cost) * 100 if individual_cost > 0 else 0
        )
...
class KeywordCompressor:
    ...
    @staticmethod
    def estimate_compression_ratio(original: List[Dict[str, Any]]) -> float:
        """
        압축률 추정
...
if __name__ == "__main__":
    # 배치 vs 개별 비교
    comparison = KeywordContextBuilder.compare_batch_vs_individual(num_stocks=20)
    ...
    print(
        f"  - 비용: ${comparison['savings']['cost_usd']:.6f} ({comparison['savings']['percent']:.1f}%)"
    )
    print()
