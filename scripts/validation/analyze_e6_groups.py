"""Step 8 — E6 fixture 그룹별 비교 분석 (Q4 hybrid 검증).

baseline (e5_baseline 3) vs focused (e6_focused 4)의 모델별 점수 차이 측정.
"""
...
SCORED_PATH = Path("docs/portfolio/coach/slice4/step8_2way_e6_scored.json")
OUTPUT_PATH = Path(
    "docs/portfolio/coach/slice4/step8_2way_e6_group_analysis.json"
)
...
def main() -> int:
    ...
    # Q4 hybrid 검증
    hybrid_validation = {
        "context": "baseline 그룹(e5_baseline 3)과 focused 그룹(e6_focused 4)의 점수 차이로 hybrid 결정 정당화.",
        "interpretation_guide": {
            "small_diff": "두 그룹 점수 유사 → hybrid 결정 정당 (fixture 다양성에 robust).",
            "baseline_higher": "baseline이 친숙 → focused는 도전적.",
            "focused_higher": "focused가 자연스러움 → E6 특화 fixture 효과.",
        },
    }
    ...
    for model in comparison:
        if "e5_baseline" in comparison[model] and "e6_focused" in comparison[model]:
            b = comparison[model]["e5_baseline"]["score_mean"]
            f = comparison[model]["e6_focused"]["score_mean"]
    ...
    assert loaded["step"] == "step8_e6_group_analysis"
