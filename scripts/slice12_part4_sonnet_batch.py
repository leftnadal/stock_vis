"""Slice 12 Part 4 Step 0 — Sonnet 15 batch matrix.

Part 3와 동일한 15 fixture를 sonnet으로 재실행하여 blind A/B 비교용 commentary 생성.
prod 코드 무변경 (Part 3 smoke와 동일 패턴, provider="sonnet"만 변경).

Output:
  - docs/portfolio/coach/slice12/part4_sonnet_results.json
  - docs/portfolio/coach/slice12/part4_sonnet_dump.md
"""
...
OUT_JSON = REPO_ROOT / "docs" / "portfolio" / "coach" / "slice12" / "part4_sonnet_results.json"
OUT_MD = REPO_ROOT / "docs" / "portfolio" / "coach" / "slice12" / "part4_sonnet_dump.md"
...
SLICE_CAP_HARD = 0.50  # Part 4 단독 sub-cap
...
def _run_case(fixture_name: str) -> dict:
    ...
    return {
        ...
        "expected_gate_triggered": data.get("expected_gate_triggered", False),
        "gate_match": gate_triggered == data.get("expected_gate_triggered", False),
...
def main() -> int:
    print("=" * 70)
    print("Slice 12 Part 4 Step 0 — Sonnet 15 batch matrix")
    print("=" * 70)
    ...
    for name in FIXTURE_NAMES:
        if total_cost >= SLICE_CAP_HARD:
            print(f"\n[STOP] Part 4 sub-cap {SLICE_CAP_HARD} 도달, 중단")
            break
        print(f"\n--- {name} ---")
        r = _run_case(name)
        cases.append(r)
        ...
        print(f"  fitting={r['schema_fitting_pass']} score={r['category_score']:.2f} "
              ...
              f"cost=${r['cost_usd']:.5f}  latency={r['latency_ms']}ms")
        if r["fitting_error"]:
    ...
    summary = {
        ...
        "n_cases_planned": 15,
        "n_fitting_pass": sum(1 for c in cases if c["schema_fitting_pass"]),
        "n_gate_triggered_actual": sum(1 for c in cases if c["gate_triggered"]),
        "n_gate_expected_match": sum(1 for c in cases if c["gate_match"]),
        "total_cost_usd": round(total_cost, 6),
        "total_latency_ms": sum(c["latency_ms"] for c in cases),
        "avg_latency_ms": (
            sum(c["latency_ms"] for c in cases) / len(cases) if cases else 0
        ),
    ...
    md_lines = [
        ...
        f"- gate match: **{summary['n_gate_expected_match']}/{summary['n_cases_executed']}**",
        f"- 총 비용: **${summary['total_cost_usd']:.4f}**",
        f"- 평균 latency: {summary['avg_latency_ms']:.0f}ms",
        "",
        "## §2. 케이스별 결과",
        "",
    ...
    print("\n" + "=" * 70)
    print(f"종료: {len(cases)}/15 case, fitting {summary['n_fitting_pass']}/{len(cases)}, "
