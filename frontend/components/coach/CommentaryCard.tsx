 ...
 * 골격 분해, Part 2 QuotedMetricsSection, Part 3 ActionItemsSection +
 * RiskFlagsSection, Part 4 KeyObservationsSection 추출 완료. 본 시점에
 * CommentaryCard는 **순수 조립부**(인라인 렌더 로직 0건).
 *
...
import KeyObservationsSection from './KeyObservationsSection'
...
export default function CommentaryCard({ output }: CommentaryCardProps) {
  ...
  return (
    <BaseCard summary={output.summary} confidence={output.confidence}>
      {/* ── 핵심 관찰 (6 EP 공통, Slice 17 Part 4 추출) ── */}
      <CardSection visible={observations.length > 0} className="mb-5">
        <KeyObservationsSection keyObservations={observations} />
      </CardSection>
