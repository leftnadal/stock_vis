...
/**
 * E2 포트폴리오 종합 진단 페이지 — Slice 16 Part 1.
 *
 * Slice 15 E1 패턴(`app/coach/e1/page.tsx`) 복제. 데이터 레이어(`useE2Coach`)는
 * 그대로 소비, UI만 책임. 3-상태(빈/로딩/에러/성공) 명시 처리.
 */
...
import type { E2Request } from '@/lib/coach/types'

const PRESETS: { value: E2Request['preset']; label: string }[] = [
...
function deriveSectorAllocation(rows: HoldingRow[]): Record<string, number> {
  const map: Record<string, number> = {}
  for (const r of rows) {
    const sector = r.sector.trim() || 'Unknown'
    const w = Number(r.weight)
    if (!Number.isFinite(w)) continue
    map[sector] = (map[sector] ?? 0) + w
  }
  return map
}

export function E2CoachContent() {
  const [portfolioId, setPortfolioId] = useState('demo-portfolio')
  const [preset, setPreset] = useState<E2Request['preset']>('garp')
  const [portfolioReturn1y, setPortfolioReturn1y] = useState('12.5')
  ...
  const sectorAllocation = useMemo(() => deriveSectorAllocation(validRows), [validRows])

  const canSubmit =
    portfolioId.trim().length > 0 &&
    validRows.length > 0 &&
    portfolioReturn1y.trim() !== '' &&
    !mutation.isPending
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const request: E2Request = {
      ...
      portfolio_return_1y: Number(portfolioReturn1y),
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-6">
        <div className="mb-2 flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-slate-900">E2 포트폴리오 종합 진단</h1>
        </div>
        <p className="text-sm text-slate-600">
          포트폴리오의 보유 종목, 섹터 비중, 1년 수익률을 입력하면 AI 코치가 종합 관점에서 진단해 드립니다.
        </p>
      </header>
      ...
      >
        <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <label className="text-sm">
            ...
            <select
              value={preset}
              onChange={(e) => setPreset(e.target.value as E2Request['preset'])}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none"
            >
              {PRESETS.map((p) => (
          ...
          <label className="text-sm">
            <span className="mb-1 block font-medium text-slate-700">1년 수익률 (%)</span>
            <input
              type="number"
              step="0.1"
              value={portfolioReturn1y}
              onChange={(e) => setPortfolioReturn1y(e.target.value)}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none"
              required
            />
          </label>
        ...
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-700">보유 종목 (섹터 비중 자동 도출)</h2>
          <button
            type="button"
            onClick={addRow}
        ...
        {Object.keys(sectorAllocation).length > 0 && (
          <div className="mt-3 rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600">
            <span className="font-medium">자동 도출 섹터 비중:</span>{' '}
...
export default function E2CoachPage() {
  return (
    <AuthGuard>
      <E2CoachContent />
    </AuthGuard>
