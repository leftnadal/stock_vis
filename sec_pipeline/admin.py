...
class RawDocumentStoreAdmin(admin.ModelAdmin):
    list_display = ['symbol', 'fiscal_year', 'filing_date', 'status', 'extraction_method']
...
class SupplyChainEvidenceAdmin(admin.ModelAdmin):
    list_display = [
        'source_company', 'target_company_name', 'relationship_type',
        'confidence_grade', 'target_company', 'neo4j_dirty',
    ]
...
class BusinessModelSnapshotAdmin(admin.ModelAdmin):
    list_display = [
        'symbol', 'as_of_date', 'direct_customer_contact',
        'contract_model', 'recurring_revenue_signal', 'confidence_grade',
    ]
...
class UnmatchedCompanyQueueAdmin(admin.ModelAdmin):
    list_display = [
        'raw_company_name', 'source_symbol', 'occurrence_count',
...
class PipelineIntelligenceReportAdmin(admin.ModelAdmin):
    list_display = [
        'report_date', 'severity_badge', 'health_score_bar',
        'collection_score', 'extraction_score', 'matching_score', 'created_at',
    ]
    ...
    fieldsets = [
        ...
        ('5-Dimension Scores', {
            'fields': ('collection_score', 'extraction_score',
                       'matching_score', 'sync_score', 'quality_score'),
        }),
        ('LLM Analysis', {
