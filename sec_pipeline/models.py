...
class SupplyChainEvidence(models.Model):
    ...
    # grade: 사용자 facing
    confidence_grade = models.CharField(
        max_length=10, choices=CONFIDENCE_GRADE_CHOICES, default="low"
    )
...
class BusinessModelSnapshot(models.Model):
    ...
    # 5개 분류 필드
    direct_customer_contact = models.CharField(
        max_length=20, choices=FIELD_CHOICES, default="unknown"
    )
    contract_model = models.CharField(
        max_length=20, choices=CONTRACT_CHOICES, default="unknown"
    )
    recurring_revenue_signal = models.CharField(
        max_length=20, choices=RECURRING_CHOICES, default="unknown"
    )
    channel_dependency = models.CharField(
        max_length=20, choices=CHANNEL_CHOICES, default="unknown"
    )
    customer_concentration = models.CharField(
        max_length=20, choices=CONCENTRATION_CHOICES, default="unknown"
    )
...
class CompanyAlias(models.Model):
    ...
    source = models.CharField(
        max_length=20, choices=SOURCE_CHOICES, default="manual_seed"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class PipelineIntelligenceReport(models.Model):
    ...
    severity = models.CharField(
        max_length=10, choices=SEVERITY_CHOICES, default="healthy"
    )
