# Research Working Record Index

**Status:** Non-Authoritative Discovery Index  
**Official Authority:** [`../research_lab/`](../research_lab/)

This file helps humans and agents find relevant Working Records. It is a locator only and must not be treated as an authoritative summary of current Research Lab doctrine.

## Working Record System

**Topic:** Working Record System  
**Keywords:** working record, non-authoritative, historical fidelity, reconstructability, retrieval, index, material checkpoint  
**Latest recorded state:** Use a sibling `research_working/` area, material-checkpoint records, stable topic/keywords, prior-record links, and this lightweight index.  
**Records:**
- [`2026-09-17_working-record-system_01.md`](records/2026-09-17_working-record-system_01.md)

## Judgment / Outcome / Learning

**Topic:** Judgment–Outcome–Learning Feedback Loop  
**Keywords:** judgment quality, outcome bias, signal, review target, candidate learning, learning quality, transfer test, operational learning, prior-learning scan, retrieval trigger, classification review, terminology, counterexample, self-review, relevance screening, use-specific compatibility, positive outcome, 분류 점검, 사전 검토, 반례, 검색 조건, 적용 가능성, 뜻밖의 성공  
**Latest recorded state:** Proposals remain non-authoritative. Checkpoint 02 preserves the pre-review request and wording history; Checkpoint 03 proposes current-situation retrieval cues and use-specific compatibility after reading source records. No final taxonomy, runtime policy, or new admission authority is approved by these records.  
**Records:**
- [`2026-09-17_judgment-outcome-learning-feedback-loop_01.md`](records/2026-09-17_judgment-outcome-learning-feedback-loop_01.md)
- [`2026-09-18_judgment-outcome-learning-feedback-loop_02.md`](records/2026-09-18_judgment-outcome-learning-feedback-loop_02.md)
- [`2026-09-18_judgment-outcome-learning-feedback-loop_03.md`](records/2026-09-18_judgment-outcome-learning-feedback-loop_03.md)

## Research Collaboration / Response Structure

**Topic:** Research Collaboration & Response Structure  
**Keywords:** response structure, context recovery, executive summary, CEO decision, approval scope, communication, 전체 그림, 맥락 복원, 두괄식, 최종 정리, CEO 결정, 답변 구성  
**Latest recorded state:** Checkpoint 01 records the user's request for big-picture context, conclusion-first detail, and a self-contained final review with genuine CEO decision items. This is a communication preference, not a new Methodology or fixed decision-packet schema.  
**Records:**
- [`2026-09-18_research-collaboration-response-structure_01.md`](records/2026-09-18_research-collaboration-response-structure_01.md)

## Record Reconstruction / Evaluation Target

**Topic:** Record Reconstruction Evaluation Target  
**Keywords:** record reconstruction, gold audit, valid provenance route, selected_route_validity, route_set_completeness, compound, ambiguous, AND OR, annotation, retrospective rescoring, gold v0.2 candidate, bounded execution approval, 근거 역할, 정답 기준, 경로 충분성, 재채점, 승인 범위  
**Latest recorded state:** Checkpoint 01 preserves the gold-audit review. Checkpoint 02 records the user's bounded approval for development-only valid-route evaluation, a local gold v0.2 candidate and validator revision, and rescoring existing outputs without model calls. Work execution/results are not yet confirmed here; this does not approve permanent rules or artifact promotion.  
**Records:**
- [`2026-09-18_record-reconstruction-evaluation-target_01.md`](records/2026-09-18_record-reconstruction-evaluation-target_01.md)
- [`2026-09-18_record-reconstruction-evaluation-target_02.md`](records/2026-09-18_record-reconstruction-evaluation-target_02.md)

## Retrieval Rule

When using this index:

1. locate the relevant Topic or Keywords;
2. read the linked Working Record(s);
3. follow `Prior Record` links when historical development matters; and
4. verify the current official state against `research_lab/` before relying on any working conclusion.
