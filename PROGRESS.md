...
> 🟢 **2026-06-18 Phase 1.5 Translation Layer S4 진행 — envelope serializer + FE selector + fallback**: **현재 `origin/main = 3998acc`** (S3 reconcile 머지 후 STEP 0 실측; CS 트랙 계속 전진 — push 시 drift 가능성 대비 rebase 준비). 목적: TranslationLog senses를 overview 응답에 노출 + FE 카드가 sense를 additive로 소비, 미생성/부분/빈 senses 모두 "밴드만"으로 graceful 수렴. **★ STEP 0**: BE 자리 = `_build_payload()`에 최상위 `translations` 블록(cards 무변경), `TranslationLog.objects.order_by("-date").first()` 조회 None→null. FE = page.tsx가 4카드에 `data=cards.X` 주입 → optional `sense` prop 추가(카드 dumb 유지) + 얇은 selector(`translations?.senses?.[key] ?? null`). **카드 키=senses 키 정합 재확인(regime/breadth/sector/concentration) → HALT 없음**(구조 대수술 불요). 마이그레이션 0(모델 무접촉). BOUNDARY-LLM 무접촉. 빈 senses(생성됐으나 0키) vs null(미생성) 구분. 다음 S5(golden+vcr).
>
