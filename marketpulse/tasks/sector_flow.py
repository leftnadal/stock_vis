...
def mp_calc_sector_5min(self, **kwargs: Any) -> dict[str, Any]:
    ...
    return {
        ...
        'leaders': [{'symbol': s.market_index_id, 'rel_strength': float(s.rel_strength)} for s in leaders],
        'laggards': [{'symbol': s.market_index_id, 'rel_strength': float(s.rel_strength)} for s in laggards],
