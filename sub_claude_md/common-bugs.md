...
**처방**: ⑴ 결정론 문구 신설 지시는 **완성 문장 리터럴**을 싣는다(규칙만 X). ⑵ 재발 방지는 테스트 게이트로 박제 — **서술체 종결 정규식**(`/(니다|입력 데이터 대기)\.$/`, 괄호 제거 후) + **위젯 룰 문장과 N자 연속 공통부분 부재**(슬라이딩 윈도우). ⑶ 임계·로직·시그니처는 무변경, 문자열만 교체하는 수리는 diff 범위를 그 파일들로 못박는다.
