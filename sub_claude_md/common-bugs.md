...
**원인**: `MacroEconomicService.get_market_pulse_dashboard()`의 full 캐시(`macro:market_pulse_full`, TTL 60s=realtime) 미스 시 5섹션 **라이브 집계**(FRED~8 + FMP~6). 워밍 beat `refresh-market-pulse-cache`는 `crontab(minute='*', hour='9-16', day_of_week='1-5')` = **ET 장중만** → 사용자 시간대(KST)는 대부분 장외 = 캐시 상시 콜드. **콜드 실측 28.8s / 웜 15ms**(:18765 read-only GET). 60s TTL은 사용 케이던스보다 짧아 재방문마다 콜드 재발.
...
## 카피 규칙만 주고 완성 문장을 안 주면 실행자가 라벨 나열로 축소한다 (채번 대기, HUB-V02-S2/COPYFIX 2026-09-07) `[frontend][process]`

결정론 UX 문구(정적 카피)를 신설할 때, 지시서에 **규칙만** 담고 완성 문장을 안 주면 실행자가 규칙을 **최소 충족하는 라벨 나열**로 축소한다(서술체 대신 명사형 종결·위젯 룰 문장과 중복·조합 통찰이 카테시안 조인으로 붕괴).

**실증(S2)**: 지시서가 **문장 리터럴을 직접 준** breadth·sector 정적 fallback은 정확했고, **규칙만 준** 허브 4문장·RRG는 명사형·룰 중복·통찰 축소가 발생 → COPYFIX로 재작업.

**처방**: ⑴ 결정론 문구 신설 지시는 **완성 문장 리터럴**을 싣는다(규칙만 X). ⑵ 재발 방지는 테스트 게이트로 박제 — **서술체 종결 정규식**(`/(니다|입력 데이터 대기)\.$/`, 괄호 제거 후) + **위젯 룰 문장과 N자 연속 공통부분 부재**(슬라이딩 윈도우). ⑶ 임계·로직·시그니처는 무변경, 문자열만 교체하는 수리는 diff 범위를 그 파일들로 못박는다. ⑷ **가드 정규식이 오탐하면 규칙을 완화하기 전에 입력을 정규화**할 수 있는지 먼저 본다(고유명 마스킹 > 패턴 완화 — S2-GUARDFIX: 연도 가드가 "러셀2000"에 오탐 → 패턴을 `년` 요구로 완화했더니 "2008 수준" 단독 연도가 통과 = 가드 약화. 정답은 고유명 마스킹 후 엄격 패턴 유지).

## 범위 한정 세션이 기존 테스트를 조용히 삭제한다 (채번 대기, S2-GUARDFIX 2026-09-08) `[frontend][process][testing]`

"문자열/카피 전용" 같은 범위 한정 세션이, 문자열 변경으로 깨지는 **기존 단언을 갱신하지 않고 삭제**해 커버리지가 조용히 준다. 문자열이 바뀌면 기존 단언이 깨지므로 *지우는 것이 가장 쉬운 수리*가 되기 때문.
...
## 환경변수도 코드와 같다 — 이미 뜬 프로세스는 구 환경을 들고 돌며, `.env` 손상은 재기동 시점에야 드러난다 (채번 후보, CS-S3-1E 2026-09-17) `[infra][deploy][harness]`

**증상**: 배포 중 `worker_sync.sh`가 daphne를 재기동하자 API(`:18765`)가 **다운**. launchd exit **127**, 로그에 `./.env: line 19: gfoe: command not found`. `:3000`은 200이라 겉으로는 정상으로 보이지만 데이터를 daphne에서 받으므로 화면이 빈다.

**원인**: `.env` 19행에 `gfoe`라는 **단독 4글자 토큰**(`KEY=VALUE`도 주석도 아님)이 있었다. 18행 `EMAIL_HOST_PASSWORD` 값이 12자였고 12+4=16, 양쪽 모두 소문자 — Gmail 앱 비밀번호가 **개행 오삽입으로 쪼개진** 것. 파일 mtime은 **17:30**, 배포는 **17:55**였다. 즉 손상은 25분 전에 이미 있었으나, 기존 daphne 프로세스가 **구 환경을 메모리에 들고** 17:54까지 정상 응답해 아무도 몰랐다. **재기동이 손상을 만든 게 아니라 드러냈다.**

**해결**: ⑴ 수정 전 `.env`를 **영속 위치**에 백업한다(`~/stock-vis-archives/env/`) — 세션 스크래치패드(`/private/tmp` 계열)는 재부팅 소실이라 유일본을 두면 안 된다. ⑵ 19행을 18행에 병합해 복구하고 `KEY=VALUE` 아닌 비주석 행 0 + `set -a; . ./.env` 파싱 성공으로 검증. ⑶ **`.env` 수정 직후에는 반드시 해당 서비스를 재기동해 검증한다** — 재기동 전까지 손상은 잠복한다. ⑷ 파싱 성공 ≠ 값 정확: 값이 틀려도 서비스는 뜨고 **그 값을 쓰는 기능만 조용히 실패**한다.
