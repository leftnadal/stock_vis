...
# 예시
python shared_kb/search.py "Celery 메모리"
python shared_kb/search.py "Redis 연결" --type troubleshoot
python shared_kb/search.py "Docker 배포" --type pattern
```
...
# 예시
python shared_kb/search.py "celery OOM" --type troubleshoot
python shared_kb/search.py "redis connection refused"
```
...
python shared_kb/add.py \
  ...
  --content "100건 단위 청킹 + gc.collect() 호출. CELERY_WORKER_MAX_MEMORY_PER_CHILD=512MB 설정으로 해결." \
  --type troubleshoot \
  --domain tech \
  --tags celery memory \
  --to-queue
