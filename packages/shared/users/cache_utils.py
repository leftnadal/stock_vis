...
class WatchlistCache:
    ...
    def set_watchlist_list(user_id, data, timeout=None):
        ...
        cache_logger.info(
            f"Cache SET: watchlist_list for user {user_id}, TTL={timeout}s"
        )
    ...
    def get_watchlist_stocks(user_id, watchlist_id):
        ...
        if cached_data:
            cache_logger.info(
                f"Cache HIT: watchlist_stocks for user {user_id}, watchlist {watchlist_id}"
            )
        else:
            cache_logger.info(
                f"Cache MISS: watchlist_stocks for user {user_id}, watchlist {watchlist_id}"
            )
    ...
    def set_watchlist_stocks(user_id, watchlist_id, data, timeout=None):
        ...
        cache.set(cache_key, data, timeout)
        cache_logger.info(
            f"Cache SET: watchlist_stocks for user {user_id}, watchlist {watchlist_id}, TTL={timeout}s"
        )
    ...
    def invalidate_watchlist_stocks(user_id, watchlist_id):
        ...
        cache.delete(cache_key)
        cache_logger.info(
            f"Cache INVALIDATED: watchlist_stocks for user {user_id}, watchlist {watchlist_id}"
        )
    ...
    def invalidate_all_user_watchlists(user_id):
        ...
        try:
            ...
            if hasattr(cache, "delete_pattern"):
                ...
                cache_logger.info(
                    f"Cache INVALIDATED (pattern): {deleted_count} keys for user {user_id}"
                )
            else:
                # 기본 백엔드: 주요 캐시만 삭제
                WatchlistCache.invalidate_watchlist_list(user_id)
                cache_logger.warning(
                    f"Pattern delete not supported, invalidated list cache only for user {user_id}"
                )
    ...
    def decorator(func):
        def wrapper(self, request, *args, **kwargs):
            ...
            if response.status_code == status.HTTP_200_OK:
                if cache_type == "stocks" and watchlist_id:
                    WatchlistCache.set_watchlist_stocks(
                        user_id, watchlist_id, response.data, timeout
                    )
                elif cache_type == "list":
