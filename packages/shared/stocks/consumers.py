...
class StockPriceConsumer(AsyncWebsocketConsumer):
    ...
    async def send_current_price(self):
        ...
        if stock_data:
            await self.send(
                text_data=json.dumps({"type": "current_price", **stock_data})
            )
...
class PortfolioConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        if hasattr(self, "room_group_name"):
            ...
            logger.info(
                f"WebSocket disconnected for portfolio: user {self.user.username}"
            )
    ...
    async def send_portfolio_summary(self):
        ...
        if portfolio_data:
            await self.send(
                text_data=json.dumps({"type": "portfolio_summary", **portfolio_data})
            )
