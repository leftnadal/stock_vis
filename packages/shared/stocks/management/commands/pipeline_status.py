...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--quality", action="store_true", help="ingest_quality 상세 출력"
        )
    ...
    def _run_pipeline(self, target_date_str):
        ...
        self.stdout.write(
            self.style.WARNING(f"파이프라인 실행 시작... (date={target or 'auto'})")
        )
    ...
    def _show_status(self, days):
        ...
        self.stdout.write(
            f"{'Date':<12} {'Status':<10} {'Duration':<10} {'Signals':<10} {'Run ID':<38}"
        )
        ...
        for log in logs:
            ...
            if log.error_message:
                self.stdout.write(
                    self.style.ERROR(f"  Error: {log.error_message[:100]}")
                )
    ...
    def _show_quality(self, days):
        ...
        for log in logs:
            ...
            self.stdout.write(
                f"  dollar_vol 필터: {q.get('dollar_vol_filtered', '-')}개"
            )
