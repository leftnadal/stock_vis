...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        top_stocks = Stock.objects.filter(market_capitalization__isnull=False).order_by(
...
class StockDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        recent_daily_prices = DailyPrice.objects.filter(stock=stock).order_by("-date")[
            :30
        ]  # 최근 30일
        context["initial_chart_data"] = ChartDataSerializer(
            recent_daily_prices, many=True
        ).data
...
class StockSearchAPIView(APIView):
    ...
    def get(self, request):
        ...
        if len(query) < 2:
            return Response(
                {"results": [], "message": "검색어는 2자 이상 입력해주세요."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class StockChartDataAPIView(APIView):
    ...
    def get(self, request, symbol):
        ...
        try:
            ...
            if cached_data:
                logger.info(
                    f"Cache hit for chart: {symbol} ({chart_type}, {period_param})"
                )
                return Response(cached_data, status=status.HTTP_200_OK)

            ## 날짜범위 계산
            end_date = datetime.now().date()
            start_date, period_display = self._calculate_date_range(
                period_param, custom_days, end_date
            )
            ...
            if stock:
                ...
                if price_data.count() > 0:
                    ...
                    response_data = self._get_fmp_chart_data(
                        symbol, period_display, chart_type, start_date, end_date
                    )
            ...
            logger.info(
                f"Cache set for chart: {symbol} ({chart_type}, {period_param}) source={'db' if stock else 'fmp'}"
            )
    ...
    def _get_fmp_chart_data(
        self, symbol, period_display, chart_type, start_date, end_date
    ):
        """FMP API에서 차트 데이터 조회"""
        import httpx
...
class StockOverviewAPIView(APIView):
    ...
    def get(self, request, symbol):
        ...
        try:
            ...
            needs_price_sync = (
                stock
                and (not stock.real_time_price or float(stock.real_time_price) == 0)
                and (not stock.volume or stock.volume == 0)
            )

            if needs_price_sync:
                logger.info(
                    f"Stock {symbol} exists but price data is empty, triggering auto-sync"
                )
                ...
                if sync_result.success:
                    ...
                    if not quote_data:
                        raise StockNotFoundError(
                            symbol=symbol, tried_sources=["db", "fmp"]
                        )

                    ## FMP 데이터를 Overview 형식으로 변환
                    change_pct = quote_data.get("changePercentage") or quote_data.get(
                        "changesPercentage", 0
                    )
                    response_data = {
                        "symbol": symbol,
                        ...
                        "data": {
                            ...
                            or quote_data.get("priceAvg50", 0),
                        },
                    }
                    source = "fmp_realtime"

            ## _meta 정보 추가
            response_data["_meta"] = sync_service.get_sync_meta(
                symbol, "overview", source
            )
        ...
        try:
            ...
            if stock:
                ...
                balance_sheets = BalanceSheet.objects.filter(
                    stock=stock, period_type=db_period
            ...
            if not data:
                from .services.financial_statements_fallback import (
                    FinancialStatementsFallbackService,
                )
            ...
            cache.set(cache_key, response_data, cache_ttl)
            logger.info(
                f"Cache set for balance sheet: {symbol} ({period}, source: {source})"
            )
    ...
    def get(self, request, symbol):
        ...
        try:
            ...
            if not data:
                ...
                fallback_service = FinancialStatementsFallbackService()
                data, source = fallback_service.get_income_statement(
                    symbol, period, limit
                )
            ...
            cache.set(cache_key, response_data, cache_ttl)
            logger.info(
                f"Cache set for income statement: {symbol} ({period}, source: {source})"
            )
...
class StockCashFlowAPIView(APIView):
    ...
    def get(self, request, symbol):
        ...
        try:
            ...
            cache.set(cache_key, response_data, cache_ttl)
            logger.info(
                f"Cache set for cash flow: {symbol} ({period}, source: {source})"
            )
        ...
        return Response(
            {
                ...
                "balance_sheets": BalanceSheetTabSerializer(
                    balance_sheets, many=True
                ).data,
                "income_statements": IncomeStatementTabSerializer(
                    income_statements, many=True
                ).data,
