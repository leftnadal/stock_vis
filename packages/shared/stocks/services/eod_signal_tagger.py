...
SIGNAL_METADATA = {
    ...
    "PV1": {
        "title": "가격-거래량 효율성",
        "description_ko": "적은 거래량으로 큰 가격 변동",
    },
...
class EODSignalTagger:
    ...
    def tag_signals(self, signals_df: pd.DataFrame) -> list[dict]:
        ...
        for _, row in signals_df.iterrows():
            ...
            if (
                pd.isna(market_cap_val)
                if isinstance(market_cap_val, float)
                else market_cap_val is None
            ):
                market_cap_val = None
            else:
                try:
    ...
    def _generate_signal_label(
        self, signal_id: str, value: float, direction: str
    ) -> str:
        """
