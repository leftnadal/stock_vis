...
class EODPipeline:
    ...
    def run(self, target_date: date = None) -> "PipelineLog":
        ...
        try:
            ...
            logger.info(
                f"[EODPipeline] Stage1 Ingest: {len(raw_df)}행, {stage1_elapsed:.1f}s"
            )
            ...
            filtered_symbols = (
                int(filtered_df["symbol"].nunique()) if not filtered_df.empty else 0
            )
            log.stages["filter"] = {
                "status": "ok",
                "symbols_before": int(raw_df["symbol"].nunique())
                if not raw_df.empty
                else 0,
            ...
            logger.info(
                f"[EODPipeline] Stage2 Filter: {filtered_symbols}종목, {stage2_elapsed:.1f}s"
            )
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage3 Calculate: {signals_count}종목, {stage3_elapsed:.1f}s"
            )
            ...
            if not filtered_df.empty:
                valid_symbols = (
                    set(
                        filtered_df[filtered_df["date"] == target_date][
                            "symbol"
                        ].tolist()
                    )
                ...
                if valid_symbols:
                    signals_df = signals_df[
                        signals_df["symbol"].isin(valid_symbols)
                    ].copy()
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage4 Tag: {len(tagged)}종목, {stage4_elapsed:.1f}s"
            )
            ...
            news_matched = sum(
                1
                for e in enriched
                if e.get("news_context", {}).get("match_type")
                not in ("none", "profile", "")
            )
            log.stages["enrich"] = {
                "status": "ok",
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage5 Enrich: {news_matched}건 뉴스 매칭, {stage5_elapsed:.1f}s"
            )
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage6 DB Upsert: {upserted}건, {stage6_elapsed:.1f}s"
            )

            # market_summary 구성
            market_summary = self._build_market_summary(
                signals_df, enriched, target_date
            )
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage7 JSON Bake: {bake_result.get('files_written')}파일, {stage7_elapsed:.1f}s"
            )
            ...
            log.save(update_fields=["stages"])
            logger.info(
                f"[EODPipeline] Stage8 Accuracy Backfill: {backfilled}건 업데이트, {stage8_elapsed:.1f}s"
            )
    ...
    def _stage_filter(self, df: pd.DataFrame, target_date: date) -> pd.DataFrame:
        ...
        filtered_today = today_df[
            (today_df["volume"] >= 100_000) & (today_df["dollar_volume"] >= 500_000)
        ]
            ...
            for accuracy in pending:
                try:
                    ...
                    if spy_signal and spy_current:
                        spy_return = (
                            (float(spy_current) - float(spy_signal))
                            / float(spy_signal)
                            * 100
                        )
                    ...
                    setattr(
                        accuracy,
                        excess_field,
                        round(excess, 4) if excess is not None else None,
                    )
    ...
    def _get_index_change(
        self, symbol: str, signals_df: pd.DataFrame, target_date: date
    ) -> float:
        """
