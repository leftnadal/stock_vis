...
class SecureStockCache:
    ...
    def generate_secure_cache_key(cache_type, symbol, **params):
        ...
        if hasattr(settings, "CACHE_SECRET_KEY"):
            ...
            hash_obj = hmac.new(
                settings.CACHE_SECRET_KEY.encode(), base_string.encode(), hashlib.sha256
            )
        ...
        # 6. 접근 로그 기록
        cache_security_logger.info(
            f"Cache key generated: {cache_type} for {clean_symbol}"
        )
    ...
    def secure_cache_get(cache_type, symbol, **params):
        ...
        try:
            ...
            cache_security_logger.error(
                f"Cache get error: {cache_type}, {symbol}, Error: {e}"
            )
            return None
    ...
    def secure_cache_set(cache_type, symbol, data, timeout=None, **params):
        ...
        try:
            ...
            if success:
                ...
                cache_security_logger.error(
                    f"Cache set failed: {cache_type} for {symbol}"
                )
            ...
            # 6. 에러 로깅
            cache_security_logger.error(
                f"Cache set error: {cache_type}, {symbol}, Error: {e}"
            )
            return False
...
class SecureStockBalanceSheetAPIView(APIView):
    ...
    def get(self, request, symbol):
        ...
        balance_sheets = BalanceSheet.objects.filter(
            stock=stock, period_type=period
