...
from services.serverless.models import SectorETFMapping, SectorPerformance
...
class SectorHeatmapService:
    ...
    def get_sector_heatmap(self, target_date: Optional[date] = None) -> Dict:
        ...
        sectors = SectorPerformance.objects.filter(date=target_date).order_by(
            "-return_pct"
        )
    ...
    def get_top_movers_by_sector(
        self, sector: str, limit: int = 5, target_date: Optional[date] = None
    ) -> Dict:
        """
    ...
    def _calculate_single_sector(
        self, target_date: date, sector_name: str, etf_symbol: str
    ) -> Optional[Dict]:
        """
        단일 섹터 성과 계산
        """
        try:
            ...
            etf_change_pct = etf_quote.get("changePercentage") or etf_quote.get(
                "changesPercentage", 0
            )
            ...
            return {
                ...
                "etf_change_pct": Decimal(str(etf_change_pct))
                if etf_change_pct
                else None,
    ...
    def _serialize_sector(self, sector: SectorPerformance) -> Dict:
        ...
        return {
            ...
            "etf_change_pct": float(sector.etf_change_pct)
            if sector.etf_change_pct
            else None,
