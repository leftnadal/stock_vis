...
class AdminStatusService:
    ...
    def get_overview_summary() -> dict:
        ...
        with_todays_price = (
            DailyPrice.objects.filter(date=last_td).values("stock").distinct().count()
        )
        coverage_pct = round(
    ...
    def detect_issues() -> list:
        ...
        try:
            ...
            price_symbols = set(
                DailyPrice.objects.filter(date=last_td).values_list(
                    "stock_id", flat=True
                )
            )
        ...
        try:
            etf_errors = (
                ETFProfile.objects.filter(is_active=True).exclude(last_error="").count()
            )
            if etf_errors > 0:
                issues.append(
                    {
    ...
    def get_stocks_status() -> dict:
        ...
        # SP500
        sp500_active = SP500Constituent.objects.filter(is_active=True).count()
        sp500_latest = SP500Constituent.objects.filter(is_active=True).aggregate(
        ...
        price_symbols = set(
            DailyPrice.objects.filter(date=last_td).values_list("stock_id", flat=True)
        )
        ...
        return {
            "breadth": {
                "today": {
                    ...
                    "advance_decline_ratio": str(breadth_today.advance_decline_ratio)
                    if breadth_today
                    else None,
    ...
    def get_news_status() -> dict:
        ...
        # 종목 커버리지
        entity_coverage = (
            NewsEntity.objects.filter(news__created_at__gte=last_7d)
            .values("symbol")
            .distinct()
            .count()
        )
        ...
        categories_active = NewsCollectionCategory.objects.filter(
            is_active=True
        ).count()
        by_priority = {}
        ...
        latest_cat = (
            NewsCollectionCategory.objects.filter(last_collected_at__isnull=False)
        ...
        return {
            ...
            "sentiment": {
                "avg_7d": str(sentiment_avg.get("avg"))
                if sentiment_avg.get("avg")
                else None,
    ...
    @staticmethod
    def get_task_logs(
        task_name: str = None, task_status: str = None, hours: int = 24, limit: int = 50
    ) -> list:
        try:
            from django_celery_results.models import TaskResult
...
def _get_latest_task_runs() -> list:
    ...
    try:
        ...
        # 태스크별 가장 최근 실행 ID
        latest_ids = (
            TaskResult.objects.filter(task_name=OuterRef("task_name"))
            .order_by("-date_done")
            .values("id")[:1]
        )
        latest_runs = TaskResult.objects.filter(
            id__in=Subquery(
...
def _get_db_table_sizes() -> list:
    try:
        with connection.cursor() as cursor:
            ...
            return [{"table": row[0], "row_count": row[1]} for row in cursor.fetchall()]
    except Exception as e:
