...
class ETFCSVDownloader:
    ...
    def download_holdings(self, etf_symbol: str) -> List[Dict]:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if (
                    e.response.status_code in (404, 403)
                    and self._auto_resolve_url
                    and attempt == 0
                ):
                    logger.warning(
        ...
        if content is None:
            error_msg = (
                f"다운로드 실패 (재시도 {self._max_retries}회): {str(last_error)}"
            )
    ...
    def _parse_xlsx(
        self, content: bytes, parser_type: str, etf_symbol: str
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            raise ETFCSVParseError(
                "openpyxl 라이브러리가 설치되지 않았습니다: pip install openpyxl"
            )
        ...
        try:
            ...
            for row_idx, row in enumerate(all_rows):
                ...
                if "ticker" in row_str and (
                    "weight" in row_str or "percentage" in row_str
                ):
            ...
            if ticker_col is None or weight_col is None:
                logger.warning(
                    f"{etf_symbol}: 필수 컬럼(Ticker, Weight)을 찾을 수 없음. header_map={header_map}"
                )
                return []
            ...
            for row in all_rows[header_row_idx + 1 :]:
                ...
                # 현금/선물 등 제외
                if any(
                    x in ticker for x in ["CASH", "USD", "MARGIN", "FUTURE", "GOVT"]
                ):
                    continue

                try:
                    weight = self._parse_decimal(
                        str(row[weight_col]) if row[weight_col] else "0"
                    )
                    shares = None
                    if shares_col and shares_col < len(row):
                        shares = self._parse_int(
                            str(row[shares_col]) if row[shares_col] else "0"
                        )
    ...
    def _parse_ishares_csv(self, text: str, etf_symbol: str) -> List[Dict]:
        ...
        for row in reader:
            ...
            try:
                weight = self._parse_decimal(
                    row.get("Weight (%)", row.get("Weight", "0"))
                )
    ...
    def _parse_generic_csv(self, text: str, etf_symbol: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines):
            lower = line.lower()
            if ("ticker" in lower or "symbol" in lower) and (
                "weight" in lower or "percent" in lower
            ):
                header_idx = i
                break
    ...
    def _save_holdings(
        self, profile: ETFProfile, holdings: List[Dict], content_hash: str
    ) -> int:
        """
        Holdings 데이터 DB 저장
        ...
        holdings_list = sorted(
            deduped.values(), key=lambda x: x["weight"], reverse=True
        )
        ...
        profile.last_error = ""
        profile.save(
            update_fields=["last_updated", "last_row_count", "last_hash", "last_error"]
        )
