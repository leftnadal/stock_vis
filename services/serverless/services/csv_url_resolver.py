...
class CSVURLResolver:
    ...
    def resolve_csv_url(
        self, etf_symbol: str, parser_type: str, current_url: Optional[str] = None
    ) -> Optional[str]:
        """
        ETF의 최신 CSV URL 찾기
        ...
        # 1단계: 패턴 매칭 시도
        csv_url = self._find_csv_url_by_pattern(html_content, etf_symbol, config)
    ...
    def _build_holdings_page_url(
        self, etf_symbol: str, parser_type: str, config: Dict
    ) -> Optional[str]:
        ...
        # 일반 템플릿
        return template.format(symbol=symbol_upper, symbol_lower=symbol_lower)
    ...
    def _find_csv_url_by_pattern(
        self, html: str, etf_symbol: str, config: Dict
    ) -> Optional[str]:
        """정규식 패턴으로 CSV URL 찾기"""
...
CSV/XLSX 다운로드 URL:"""
    ...
    def _clean_html_for_llm(self, html: str) -> str:
        ...
        for match in re.finditer(
            ...
            if any(
                kw in tag.lower()
                for kw in ["csv", "xlsx", "download", "holdings", "export"]
            ):
                relevant_parts.append(tag)

        # data- 속성 추출
        for match in re.finditer(
            r'data-[^=]*=["\'][^"\']*(?:csv|xlsx|download|holdings)[^"\']*["\']',
            html,
            re.IGNORECASE,
        ):
        ...
        try:
            ...
            if valid_types:
                if not any(vt in content_type.lower() for vt in valid_types):
                    ...
                    if not any(
                        url.lower().endswith(ext) for ext in [".csv", ".xlsx", ".xls"]
                    ):
