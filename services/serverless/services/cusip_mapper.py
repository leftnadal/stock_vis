...
class CUSIPMapper:
    ...
    def map(self, cusip: str, company_name: str = "") -> Optional[str]:
        ...
        if company_name:
            ...
            if ticker:
                ...
                logger.info(
                    f"CUSIP {cusip_clean} → {ticker} (FMP search by name: {company_name})"
                )
                return ticker

        # 4. All methods failed
        logger.warning(
            f"Could not map CUSIP {cusip_clean} (company: {company_name or 'N/A'})"
        )
        return None
    ...
    def map_batch(self, cusip_list: List[Union[str, Dict]]) -> Dict[str, Optional[str]]:
        ...
        logger.info(
            f"Batch mapped {len(results)} CUSIPs, {sum(1 for v in results.values() if v)} successful"
        )
        return results
