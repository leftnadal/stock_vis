...
class StockKeywordAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("종목 정보", {"fields": ("symbol", "company_name", "date")}),
        ("키워드", {"fields": ("keywords", "status", "error_message")}),
        (
            "메타데이터",
            {
                "fields": (
...
class CorporateActionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("기본 정보", {"fields": ("symbol", "date", "action_type", "display_text")}),
        ("상세 정보", {"fields": ("ratio", "dividend_amount", "source")}),
        (
            "메타데이터",
            {
                "fields": ("created_at",),
