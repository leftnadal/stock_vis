...
class AdminActionView(APIView):
    ...
    def post(self, request):
        ...
        logger.info(
            f"Admin action dispatched: {action_type} by {request.user} → task_id={result.id}"
        )
    ...
    def get(self, request, task_id):
        ...
        try:
            ...
            AdminActionLog.objects.filter(task_id=task_id).update(
                status=final_status,
                result_summary=str(task_result.result)[:500]
            ...
            return Response(
                {
                    ...
                    "result": str(task_result.result)[:500]
                    if task_result.result
                    else None,
...
class AdminNewsCategoryView(APIView):
            ...
            result.append(
                {
                    ...
                    "last_collected_at": cat.last_collected_at.isoformat()
                    if cat.last_collected_at
                    else None,
    ...
    def post(self, request):
        ...
        if not value:
            return Response(
                {"error": "값은 필수입니다"}, status=status.HTTP_400_BAD_REQUEST
            )

        # category_type 검증
        if category_type not in ("sector", "sub_sector", "custom"):
            return Response(
                {"error": "유효하지 않은 카테고리 타입"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if priority not in ("high", "medium", "low"):
            return Response(
                {"error": "유효하지 않은 우선순위"}, status=status.HTTP_400_BAD_REQUEST
            )

        # sector/sub_sector → SP500Constituent 존재 여부 확인
        if category_type == "sector":
            if not SP500Constituent.objects.filter(
                sector=value, is_active=True
            ).exists():
                return Response(
                    {"error": f"존재하지 않는 섹터: {value}"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        elif category_type == "sub_sector":
            if not SP500Constituent.objects.filter(
                sub_sector=value, is_active=True
            ).exists():
                return Response(
                    {"error": f"존재하지 않는 서브섹터: {value}"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        elif category_type == "custom":
            if not re.match(r"^[A-Za-z0-9,.\s-]+$", value):
                return Response(
                    {"error": "심볼 형식이 올바르지 않습니다"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            symbols = [s.strip() for s in value.split(",") if s.strip()]
            if not symbols:
                return Response(
                    {"error": "최소 1개 심볼 필요"}, status=status.HTTP_400_BAD_REQUEST
                )
...
class AdminNewsCategoryDetailView(APIView):
    ...
    def put(self, request, category_id):
        ...
        if "category_type" in data or "value" in data:
            ...
            if ct == "sector":
                if not SP500Constituent.objects.filter(
                    sector=val, is_active=True
                ).exists():
                    return Response(
                        {"error": f"존재하지 않는 섹터: {val}"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            elif ct == "sub_sector":
                if not SP500Constituent.objects.filter(
                    sub_sector=val, is_active=True
                ).exists():
                    return Response(
                        {"error": f"존재하지 않는 서브섹터: {val}"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            elif ct == "custom":
...
class AdminNewsSectorOptionsView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {
                "sectors": [
                    {"value": s["sector"], "count": s["count"]} for s in sectors
                ],
                "sub_sectors": [
                    {
                        "value": s["sub_sector"],
