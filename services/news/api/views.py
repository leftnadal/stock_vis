...
class NewsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(
        detail=False, methods=["get"], url_path="stock/(?P<symbol>[^/.]+)/sentiment"
    )
    def stock_sentiment(self, request, symbol=None):
        ...
        entities = NewsEntity.objects.filter(
            symbol=symbol, news__published_at__gte=from_date
        ).select_related("news")
    ...
    def market(self, request):
        ...
        articles = NewsArticle.objects.filter(
            category__in=db_categories, published_at__gte=from_date
    ...
    def trending(self, request):
        ...
        if timeframe not in timeframe_map:
            raise ValidationError(
                {"timeframe": "Invalid timeframe. Choose from: 1h, 24h, 7d"}
            )
        ...
        # 집계 쿼리
        trending_data = (
            NewsEntity.objects.filter(news__published_at__gte=from_date)
            .values("symbol")
            .annotate(news_count=Count("id"), avg_sentiment=Avg("sentiment_score"))
            ...
            results.append(
                {
                    ...
                    if item["avg_sentiment"]
                    else 0.0,
                    "recent_articles": NewsArticleListSerializer(
                        recent_articles, many=True
                    ).data,
                }
            )
    ...
    def sources(self, request):
        ...
        finnhub_count = NewsArticle.objects.filter(
            published_at__gte=from_date, finnhub_id__isnull=False
        ).count()

        # Marketaux 뉴스 건수 (marketaux_uuid가 있는 기사)
        marketaux_count = NewsArticle.objects.filter(
            published_at__gte=from_date, marketaux_uuid__isnull=False
        ).count()
        ...
        sources = [
            {"name": "all", "label": "전체", "count": total_count},
            {"name": "finnhub", "label": "Finnhub", "count": finnhub_count},
            {"name": "marketaux", "label": "Marketaux", "count": marketaux_count},
        ]
    ...
    def daily_keywords(self, request):
        ...
        if not keyword_obj or keyword_obj.status == "failed":
            fallback_obj = (
                DailyNewsKeyword.objects.filter(
    ...
    def generate_daily_keywords(self, request):
        ...
        # Celery 태스크 시작
        task = extract_daily_news_keywords.delay(target_date=date_str, force=force)
    ...
    def keyword_detail(self, request):
        ...
        if article_ids:
            ...
            articles_qs = NewsArticle.objects.filter(id__in=article_ids).order_by(
                "-published_at"
            )[:10]
        else:
    ...
    def _generate_keyword_analysis(
        self, keyword: str, sentiment: str, articles: list
    ) -> str:
...
한국어로 작성하세요. JSON이 아닌 일반 텍스트로 응답하세요."""
    ...
    def insights(self, request):
        ...
        include_market_data = (
            request.query_params.get("include_market_data", "true").lower() == "true"
        )
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="ml-lightgbm-readiness",
        permission_classes=[IsAdminUser],
    )
    ...
    def recommendations(self, request):
        ...
        result = recommender.get_recommendations(target_date=target_date, limit=limit)
    ...
    def collection_logs(self, request):
        ...
        for row in provider_agg:
            ...
            success_rate = (
                round(1.0 - (error_runs / total_runs), 3) if total_runs > 0 else 1.0
            )
    ...
    def pipeline_health(self, request):
        ...
        phase1_error_rate = (
            phase1_errors / phase1_total_runs if phase1_total_runs > 0 else 0
        )
        ...
        phases.append(
            {
                ...
                "hours_since_last_run": round(
                    (now - phase1_last_run).total_seconds() / 3600, 2
                )
                if phase1_last_run
                else None,
                "status": _determine_status(
                    phase1_config, phase1_last_run, phase1_error_rate
                ),
        ...
        phase2_log = (
            NewsCollectionLog.objects.filter(task_name="classify_news_batch")
            .order_by("-executed_at")
            .first()
        )
        ...
        phases.append(
            {
                ...
                "hours_since_last_run": round(
                    (now - phase2_last_run).total_seconds() / 3600, 2
                )
                if phase2_last_run
                else None,
                "status": _determine_status(
                    phase2_config, phase2_last_run, phase2_error_rate
                ),
        ...
        phase3_log = (
            NewsCollectionLog.objects.filter(task_name="analyze_news_deep")
            .order_by("-executed_at")
            .first()
        )
        ...
        phases.append(
            {
                ...
                "hours_since_last_run": round(
                    (now - phase3_last_run).total_seconds() / 3600, 2
                )
                if phase3_last_run
                else None,
                "status": _determine_status(
                    phase3_config, phase3_last_run, phase3_error_rate
                ),
        ...
        label_log = (
            NewsCollectionLog.objects.filter(task_name="collect_ml_labels")
            .order_by("-executed_at")
            .first()
        )
        neo4j_log = (
        ...
        labeled_total = NewsArticle.objects.filter(
        ...
        phases.append(
            {
                ...
                "last_label_run": phase4_last_run.isoformat()
                if phase4_last_run
                else None,
                "last_neo4j_run": phase4_neo4j_last_run,
                "hours_since_last_run": round(
                    (now - phase4_last_run).total_seconds() / 3600, 2
                )
                if phase4_last_run
                else None,
                "status": _determine_status(
                    phase4_config, phase4_last_run, phase4_error_rate
                ),
        ...
        phases.append(
            {
                ...
                "hours_since_last_run": round(
                    (now - phase5_last_run).total_seconds() / 3600, 2
                )
                if phase5_last_run
                else None,
                "status": _determine_status(
                    phase5_config, phase5_last_run, phase5_error_rate
                ),
                "deployed_version": deployed_model.model_version
                if deployed_model
                else None,
                "deployed_f1": deployed_model.f1_score if deployed_model else None,
                "deployment_status": deployed_model.deployment_status
                if deployed_model
                else None,
            }
        )
        ...
        lgbm_model = (
            MLModelHistory.objects.filter(algorithm="lightgbm")
            .order_by("-trained_at")
            .first()
        )
        ...
        phases.append(
            {
                ...
                "hours_since_last_run": round(
                    (now - phase6_last_run).total_seconds() / 3600, 2
                )
                if phase6_last_run
                else None,
        ...
        kw_today = DailyNewsKeyword.objects.filter(
            date=now.astimezone(KST).date()
        ).first()
        ...
        llm_summary = {
            ...
            "error_rate_today": round(
                phase3_errors / max(llm_total_today + phase3_errors, 1), 3
            ),
        }
    ...
    def ml_trend(self, request):
        ...
        history_qs = MLModelHistory.objects.filter(trained_at__gte=cutoff).order_by(
            "trained_at"
        )
        ...
        if len(f1_scores) >= 2:
            ...
            trend_summary["f1_direction"] = (
                "improving"
                if f1_change > 0.01
                else ("declining" if f1_change < -0.01 else "stable")
            )
            # 연속 하락 감지 (최근 3개)
            if len(f1_scores) >= 3:
                recent3 = f1_scores[-3:]
    ...
    def llm_usage(self, request):
        ...
        kw_qs = DailyNewsKeyword.objects.filter(date__gte=cutoff.date()).order_by(
            "date"
        )
        ...
        avg_gen_time = (
            round((totals_agg["total_gen_time"] or 0) / total_days, 0)
            if total_days > 0
            else 0
        )
        ...
        totals = {
            ...
            "total_tokens": (totals_agg["total_prompt"] or 0)
            + (totals_agg["total_completion"] or 0),
    ...
    def task_timeline(self, request):
        ...
        qs = NewsCollectionLog.objects.filter(executed_at__gte=cutoff).order_by(
            "executed_at"
        )
    ...
    def neo4j_status(self, request):
        ...
        cleanup_log = (
            NewsCollectionLog.objects.filter(task_name__icontains="cleanup")
            .order_by("-executed_at")
            .first()
        )
    ...
    @action(
        detail=False,
        methods=["post"],
        url_path=r"alerts/(?P<alert_pk>\d+)/resolve",
        permission_classes=[IsAdminUser],
    )
    def alerts_resolve(self, request, alert_pk=None):
        ...
        if alert.is_resolved:
            return Response(
                {"error": "이미 해결된 알림입니다."}, status=status.HTTP_400_BAD_REQUEST
            )
