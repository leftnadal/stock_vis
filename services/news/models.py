...
class NewsArticle(models.Model):
    ...
    url = models.URLField(max_length=2000, unique=True, help_text=_("원본 기사 URL"))
    ...
    title = models.CharField(max_length=500, help_text=_("기사 제목"))
    summary = models.TextField(blank=True, help_text=_("기사 요약"))
    image_url = models.URLField(
        max_length=2000, blank=True, help_text=_("대표 이미지 URL")
    )
    source = models.CharField(max_length=100, db_index=True, help_text=_("뉴스 출처"))
    published_at = models.DateTimeField(db_index=True, help_text=_("발행 일시"))
    language = models.CharField(max_length=5, default="en", help_text=_("언어 코드"))
    ...
    finnhub_id = models.BigIntegerField(
        null=True, blank=True, help_text=_("Finnhub 기사 ID")
    )
    marketaux_uuid = models.CharField(
        max_length=50, blank=True, help_text=_("Marketaux 기사 UUID")
    )
    fmp_id = models.CharField(max_length=200, blank=True, help_text=_("FMP 기사 ID"))
    alphavantage_id = models.CharField(
        max_length=200, blank=True, help_text=_("Alpha Vantage 기사 ID")
    )
    ...
    is_press_release = models.BooleanField(default=False, help_text=_("보도 자료 여부"))
    is_official = models.BooleanField(
        default=False, help_text=_("공식 발표 (보도자료 등)")
    )
    is_archived = models.BooleanField(
        default=False, db_index=True, help_text=_("아카이브 여부")
    )
    ...
    rule_sectors = models.JSONField(
        null=True, blank=True, help_text=_("규칙 엔진 추출 섹터 리스트")
    )
    rule_tickers = models.JSONField(
        null=True, blank=True, help_text=_("규칙 엔진 추출 티커 리스트")
    )
    llm_analyzed = models.BooleanField(
        default=False, db_index=True, help_text=_("LLM 심층 분석 완료 여부")
    )
    llm_analysis = models.JSONField(
        null=True, blank=True, help_text=_("LLM 심층 분석 결과 JSON")
    )

    # ML Label fields
    ml_label_24h = models.FloatField(
        null=True, blank=True, help_text=_("발행 후 다음 거래일 변동폭 (%)")
    )
    ml_label_important = models.BooleanField(
        null=True, blank=True, help_text=_("섹터별 threshold 적용 중요 뉴스 여부")
    )
    ml_label_confidence = models.FloatField(
        null=True, blank=True, help_text=_("Label 신뢰도 (0~1)")
    )
    ml_label_updated_at = models.DateTimeField(
        null=True, blank=True, help_text=_("ML Label 업데이트 시간")
    )
...
class NewsEntity(models.Model):
    ...
    symbol = models.CharField(max_length=20, db_index=True, help_text=_("종목 심볼"))
    entity_name = models.CharField(max_length=200, help_text=_("엔티티 이름"))
    entity_type = models.CharField(
        max_length=20, choices=ENTITY_TYPE_CHOICES, help_text=_("엔티티 타입")
    )
    exchange = models.CharField(max_length=20, blank=True, help_text=_("거래소"))
    country = models.CharField(max_length=5, blank=True, help_text=_("국가 코드"))
    industry = models.CharField(max_length=100, blank=True, help_text=_("산업 분류"))
...
class EntityHighlight(models.Model):
    ...
    news_entity = models.ForeignKey(
        NewsEntity, on_delete=models.CASCADE, related_name="highlights"
    )
    highlight_text = models.TextField(help_text=_("하이라이트 텍스트"))
    ...
    location = models.CharField(
        max_length=20, choices=LOCATION_CHOICES, help_text=_("하이라이트 위치")
    )
...
class SentimentHistory(models.Model):
    ...
    symbol = models.CharField(max_length=20, db_index=True, help_text=_("종목 심볼"))
    date = models.DateField(db_index=True, help_text=_("집계 날짜"))
    ...
    news_count = models.PositiveIntegerField(help_text=_("뉴스 건수"))
    positive_count = models.PositiveIntegerField(
        default=0, help_text=_("긍정 뉴스 건수")
    )
    negative_count = models.PositiveIntegerField(
        default=0, help_text=_("부정 뉴스 건수")
    )
    neutral_count = models.PositiveIntegerField(
        default=0, help_text=_("중립 뉴스 건수")
    )
...
class DailyNewsKeyword(models.Model):
    ...
    date = models.DateField(unique=True, db_index=True, help_text=_("키워드 추출 날짜"))
    ...
    total_news_count = models.PositiveIntegerField(
        default=0, help_text=_("분석에 사용된 뉴스 총 건수")
    )
    sources = models.JSONField(
        default=dict, help_text=_('소스별 뉴스 건수 {"finnhub": 45, "marketaux": 5}')
    )
    ...
    llm_model = models.CharField(
        max_length=50, default="gemini-2.5-flash", help_text=_("사용된 LLM 모델")
    )
    generation_time_ms = models.PositiveIntegerField(
        null=True, blank=True, help_text=_("LLM 생성 시간 (밀리초)")
    )
    prompt_tokens = models.PositiveIntegerField(
        null=True, blank=True, help_text=_("프롬프트 토큰 수")
    )
    completion_tokens = models.PositiveIntegerField(
        null=True, blank=True, help_text=_("응답 토큰 수")
    )
    error_message = models.TextField(blank=True, help_text=_("실패 시 에러 메시지"))
    ...
    def get_top_keywords(self, limit=5):
        ...
        sorted_keywords = sorted(
            self.keywords, key=lambda k: k.get("importance", 0), reverse=True
        )
...
class MLModelHistory(models.Model):
    ...
    trained_at = models.DateTimeField(auto_now_add=True, help_text=_("학습 완료 시각"))
    model_version = models.CharField(
        max_length=50, help_text=_("모델 버전 (예: lr_v1_week8)")
    )
    ...
    training_samples = models.PositiveIntegerField(help_text=_("학습 데이터 건수"))
    feature_count = models.PositiveIntegerField(default=5, help_text=_("feature 수"))

    # Performance metrics
    f1_score = models.FloatField(help_text=_("F1 Score"))
    precision = models.FloatField(null=True, blank=True, help_text=_("Precision"))
    recall = models.FloatField(null=True, blank=True, help_text=_("Recall"))
    accuracy = models.FloatField(null=True, blank=True, help_text=_("Accuracy"))

    # Weights & config
    weights = models.JSONField(
        null=True, blank=True, help_text=_("Engine C β₁~β₅ 가중치")
    )
    ...
    feature_importance = models.JSONField(
        null=True, blank=True, help_text=_("Feature importance 리포트")
    )
    training_config = models.JSONField(
        null=True, blank=True, help_text=_("학습 설정 (window, split 등)")
    )

    # Safety Gate
    safety_gate_passed = models.BooleanField(
        default=False, help_text=_("Safety Gate 통과 여부")
    )
    safety_gate_details = models.JSONField(
        null=True, blank=True, help_text=_("Safety Gate 상세 결과")
    )
    ...
    deployed_at = models.DateTimeField(
        null=True, blank=True, help_text=_("실적용 시각")
    )

    # Shadow mode comparison
    shadow_comparison = models.JSONField(
        null=True, blank=True, help_text=_("Shadow Mode: ML vs 수동 선별 비교 리포트")
    )
    ...
    def __str__(self):
        return (
            f"{self.model_version} (F1={self.f1_score:.3f}, {self.deployment_status})"
        )
...
class NewsCollectionCategory(models.Model):
    ...
    def resolve_symbols(self) -> list[str]:
        ...
        if self.category_type == "sector":
            ...
            return [s.strip().upper() for s in self.value.split(",") if s.strip()][
                : self.max_symbols
            ]
        return []
