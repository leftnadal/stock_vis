...
class NewsBasedStockInsights:
    ...
    def get_insights(
        ...
        if all_symbols:
            stock_rows = Stock.objects.filter(symbol__in=all_symbols).values(
                "symbol", "sector"
            )
            for row in stock_rows:
                raw_sector = row["sector"]
                sector_map[row["symbol"]] = (
                    raw_sector.strip().title() if raw_sector else None
                )
        ...
        sorted_symbols = sorted(
            symbol_data.items(), key=lambda x: x[1]["total_news_count"], reverse=True
        )
    ...
    def _collect_symbol_data(
        self, target_date: date, keywords: List[Dict], min_mentions: int
    ) -> Dict[str, Dict]:
        ...
        start_datetime = datetime.combine(
            target_date - timedelta(days=6), datetime.min.time()
        )
        ...
        for symbol, data in symbol_data.items():
            ...
            for kw_info in related_keywords:
                ...
                matching_news = self._find_matching_news(
                    data["news_articles"], kw_info["text"]
                )
        ...
        for symbol, data in symbol_data.items():
            if data["keyword_mentions"] and data["news_articles"]:
                ...
                existing_headlines = {
                    m["news_headline"] for m in data["keyword_mentions"]
                }
                ...
                missing = []
                if (
                    "negative" not in existing_sentiments
                    and dist.get("negative", 0) > 0
                ):
                    missing.append("negative")
                if (
                    "positive" not in existing_sentiments
                    and dist.get("positive", 0) > 0
                ):
                ...
                for target_sentiment in missing:
                    for article in data["news_articles"]:
                        if (
                            article["sentiment"] == target_sentiment
        ...
        for symbol, data in symbol_data.items():
            if not data["keyword_mentions"] and data["news_articles"]:
                ...
                for article in data["news_articles"][:3]:
                    ...
                    headline = article["headline"]
                    short_kw = (
                        headline[:20].rstrip() + "…" if len(headline) > 20 else headline
                    )
                    data["keyword_mentions"].append(
                        {
                            "keyword": short_kw,
        ...
        for symbol, data in symbol_data.items():
            ...
            if len(mentions) > 1:
                ...
                reordered = []
                buckets = [
                    by_sentiment["positive"],
                    by_sentiment["negative"],
    ...
    def _supplement_from_keywords(
        ...
        for symbol in list(new_symbols)[:20]:  # 최대 20개
            articles = (
                ...
                .filter(Q(title__icontains=symbol) | Q(rule_tickers__contains=[symbol]))
    ...
    def _find_matching_news(
        self, news_articles: List[Dict], keyword_text: str
    ) -> List[Dict]:
        """키워드와 매칭되는 뉴스 찾기"""
        # 키워드 텍스트를 단어로 분리
...
정확히 {len(unique_headlines)}개의 한국어 키워드를 JSON 배열로만 응답하세요."""
            ...
            translation_map = {
                h: str(s)[:20]
                for h, s in zip(unique_headlines, summaries)
                if isinstance(s, str)
            }
    ...
    def _get_market_data(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if (
                stock.analyst_rating_buy
                or stock.analyst_rating_hold
                or stock.analyst_rating_sell
            ):
                analyst_ratings = {
        ...
        if keyword_obj and keyword_obj.keywords:
            for kw in keyword_obj.keywords:
                if symbol.upper() in [s.upper() for s in kw.get("related_symbols", [])]:
                    # 매칭 뉴스 찾기
                    matching = self._find_matching_news(
                        news_articles, kw.get("text", "")
                    )
                    if matching:
