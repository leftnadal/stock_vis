...
class PersonalizedFeedService:
    ...
    def get_feed(self, user) -> dict:
        ...
        if portfolio_symbols:
            portfolio_news = self._get_news_for_symbols(
                portfolio_symbols, from_date, limit=8
            )
            if portfolio_news:
        ...
        if watchlist_symbols and remaining > 0:
            ...
            if new_symbols:
                watchlist_news = self._get_news_for_symbols(
                    new_symbols, from_date, limit=min(6, remaining)
                )
        ...
        if remaining > 0:
            ...
            if new_symbols:
                interest_news = self._get_news_for_symbols(
                    new_symbols, from_date, limit=min(6, remaining)
                )
    ...
    def _get_portfolio_symbols(self, user) -> list:
        ...
        return list(
            Portfolio.objects.filter(user=user).values_list("stock__symbol", flat=True)
        )
    ...
    def _get_interest_symbols(self, user) -> list:
        ...
        for interest in interests:
            if interest.auto_category_id:
                try:
                    cat = NewsCollectionCategory.objects.get(
                        id=interest.auto_category_id
                    )
