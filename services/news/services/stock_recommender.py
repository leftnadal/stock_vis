...
class NewsBasedStockRecommender:
    ...
    def get_recommendations(
        ...
        symbol_scores = self._calculate_scores_from_keywords(
            keyword_obj.keywords, target_date
        )

        # 뉴스 멘션 데이터 추가
        symbol_scores = self._enhance_with_mentions(
            symbol_scores, target_date, min_mentions
        )

        # 점수 순 정렬
        sorted_symbols = sorted(
            symbol_scores.items(), key=lambda x: x[1]["total_score"], reverse=True
        )
    ...
    def _calculate_scores_from_keywords(
        self, keywords: List[Dict], target_date: date
    ) -> Dict[str, Dict]:
        """키워드에서 종목 점수 계산"""
        symbol_scores = defaultdict(
            lambda: {
                "total_score": 0,
                "keyword_score": 0,
                "reasons": [],
    ...
    def _enhance_with_mentions(
        self, symbol_scores: Dict[str, Dict], target_date: date, min_mentions: int
    ) -> Dict[str, Dict]:
        """뉴스 멘션 데이터로 점수 보강"""
        # 해당 날짜 뉴스 멘션 집계
        ...
        mention_stats = (
            ...
            .values("symbol", "entity_name")
            .annotate(mention_count=Count("id"), avg_sentiment=Avg("sentiment_score"))
        )

        # 최대 멘션 수 (정규화용)
        max_mentions = max((m["mention_count"] for m in mention_stats), default=1)
        ...
        for stat in mention_stats:
            ...
            symbol_scores[symbol]["total_score"] = (
                symbol_scores[symbol]["keyword_score"] + mention_score + sentiment_boost
            )
    ...
    def _apply_sector_diversity(self, sorted_symbols: List, max_items: int) -> List:
        """섹터 다양성 적용 (같은 섹터 종목 제한)"""
        # TODO: 섹터 정보가 필요한 경우 FMP API 연동
        # 현재는 단순히 반환
        return sorted_symbols[:max_items]

    def _fallback_recommendations(self, target_date: date, limit: int) -> Dict:
        """키워드가 없을 때 멘션 기반 추천"""
        import time
