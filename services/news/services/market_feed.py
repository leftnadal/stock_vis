...
class MarketFeedService:
    ...
    def _get_latest_keywords(self):
        ...
        kw = (
            DailyNewsKeyword.objects.filter(status="completed")
            .order_by("-date")
            .first()
        )
        if kw:
    ...
    def _enrich_keywords_with_news(self, keywords: list, best_date) -> list:
        ...
        for kw in keywords:
            ...
            if symbols:
                ...
                headlines = [{"title": n.title, "url": n.url} for n in matching_news]
    ...
    def _get_market_context(self) -> dict:
        ...
        try:
            ...
            movers = MarketMover.objects.filter(
                date__gte=recent_date, category="gainers"
                ...
                hot_movers.append(
                    {
                        ...
                        "change_percent": float(m.change_percent)
                        if m.change_percent
                        else 0,
                        "sector": "",
                    }
