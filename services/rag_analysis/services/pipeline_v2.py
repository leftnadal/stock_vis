...
class AnalysisPipelineV2:
    ...
    async def analyze(
        ...
        try:
            ...
            normalized_symbols = self.entity_normalizer.normalize_stocks(
                entities["stocks"]
            )
            ...
            yield {"phase": "searching", "message": "관련 문서를 검색하고 있습니다..."}
            ...
            if normalized_symbols:
                metadata_filter = MetadataFilterBuilder().add_symbols(
                    normalized_symbols
                )
            ...
            if not search_docs:
                ...
                yield {"phase": "analyzing", "message": "AI 분석을 시작합니다..."}
            ...
            reranked_docs = self.reranker.rerank(
                question=question, documents=search_docs, top_k=self.final_top_k
            )
            ...
            yield {"phase": "compressing", "message": "컨텍스트를 압축하고 있습니다..."}

            compressed_docs = await self.compressor.compress(
                documents=reranked_docs, question=question
            )
            ...
            async for event in self.llm.generate_stream(
                context=context, question=question, max_retries=max_retries
            ):
                if event["type"] == "delta":
                    chunk = event["content"]
                    full_response += chunk
    ...
    def _resolve_search_results(
        ...
        for result in search_results:
            ...
            resolved.append((doc, result["score"], result["scores"]))
    ...
    def _build_compressed_context(
        self, compressed_docs: List[dict], entities: dict
    ) -> str:
        """
        압축된 문서로 컨텍스트 구성
...
"""
        ...
        # LLM 스트리밍
        async for event in self.llm.generate_stream(
            context=empty_context, question=question, max_retries=max_retries
        ):
            if event["type"] == "delta":
                chunk = event["content"]
                full_response += chunk
