...
본 분석은 참고용이며, 투자 조언이 아닙니다. 투자 결정은 본인의 판단과 책임 하에 이루어져야 합니다."""
    ...
    def __init__(self, provider: str = "gemini", enable_cost_tracking: bool = True):
        """
        Args:
    ...
    async def _generate_gemini_stream(
        self, system_prompt: str, context: str, question: str, config: Dict[str, Any]
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """Gemini 스트리밍 생성"""
        if not self._genai:
...
## 분석"""
    ...
    def _build_system_prompt(self, depth: str) -> str:
        """시스템 프롬프트 생성"""
        depth_instruction = self.DEPTH_INSTRUCTIONS.get(
            depth, self.DEPTH_INSTRUCTIONS["medium"]
        )
    ...
    async def estimate_cost(
        self, question: str, context_length: int, entities_count: int = 0
    ) -> Dict[str, Any]:
        """
        예상 비용 추정
        ...
        # 토큰 추정
        estimated_input = (
            context_length + len(question.split()) * 1.3 + 200
        )  # 시스템 프롬프트
...
def get_adaptive_llm_service(
    provider: str = "gemini", enable_cost_tracking: bool = True
) -> AdaptiveLLMService:
