...
class Command(BaseCommand):
    ...
    def _handle_setup(self):
        ...
        if success:
            ...
            raise CommandError(
                "❌ Semantic Cache 인덱스 생성에 실패했습니다. Neo4j 연결을 확인하세요."
            )
    ...
    def _handle_drop(self):
        ...
        if success:
            self.stdout.write(
                self.style.SUCCESS("✅ Semantic Cache 인덱스가 삭제되었습니다.")
            )
        else:
            raise CommandError("❌ Semantic Cache 인덱스 삭제에 실패했습니다.")
    ...
    def _handle_stats(self):
        ...
        if stats.get("status") == "unavailable":
            raise CommandError("❌ Neo4j에 연결할 수 없습니다.")

        if stats.get("status") == "error":
            raise CommandError(f"❌ 에러 발생: {stats.get('error')}")
