...
class AnalysisPipelineE2ETest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.basket = DataBasket.objects.create(user=self.user, name="Tech Analysis")
    ...
    def test_complete_analysis_flow_with_mock_llm(self, mock_anthropic_class):
        ...
        mock_stream.__aiter__.return_value = [
            ...
            MagicMock(
                type="message_stop",
                message=MagicMock(usage=MagicMock(input_tokens=200, output_tokens=50)),
            ),
        ]
    ...
    def test_neo4j_graceful_degradation(self, mock_anthropic_class, mock_neo4j):
        ...
        mock_stream.__aiter__.return_value = [
            ...
            MagicMock(
                type="message_stop",
                message=MagicMock(usage=MagicMock(input_tokens=50, output_tokens=10)),
            ),
        ]
...
class SSEStreamingE2ETest(TestCase):
    ...
    def test_sse_streaming_response(self, mock_anthropic_class):
        ...
        mock_stream.__aiter__.return_value = [
            ...
            MagicMock(
                type="message_stop",
                message=MagicMock(usage=MagicMock(input_tokens=20, output_tokens=5)),
            ),
        ]
...
class MessagePersistenceTest(TestCase):
    ...
    def test_messages_saved_to_database(self, mock_anthropic_class):
        ...
        mock_stream.__aiter__.return_value = [
            ...
            MagicMock(
                type="message_stop",
                message=MagicMock(usage=MagicMock(input_tokens=100, output_tokens=10)),
            ),
        ]
    ...
    def test_suggestions_saved(self, mock_anthropic_class):
        ...
        mock_stream.__aiter__.return_value = [
            ...
            MagicMock(
                type="message_stop",
                message=MagicMock(usage=MagicMock(input_tokens=50, output_tokens=20)),
            ),
        ]
...
class AnalysisPipelineFinalE2ETest(TransactionTestCase):
    ...
    def setUp(self):
        """테스트 데이터 생성"""
        self.user = User.objects.create_user(
            username="testuser_final", password="testpass123"
        )

        self.basket = DataBasket.objects.create(
            user=self.user, name="Final Pipeline Test"
        )
    ...
    def test_final_pipeline_with_cache_miss(
        ...
        pipeline = AnalysisPipelineFinal(
            self.session, enable_cache=True, enable_cost_optimization=True
        )
    ...
    def test_final_pipeline_with_cache_hit(self, mock_cache_find):
        ...
        self.assertEqual(
            cache_hit_event["data"]["cache_info"]["similarity_score"], 0.95
        )
    ...
    def test_final_pipeline_complexity_classification(self):
        ...
        simple_result = classifier.classify_and_configure(
            question="AAPL 현재 가격은?", entities_count=1, context_tokens=100
        )
