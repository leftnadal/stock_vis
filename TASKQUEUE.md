...
| NT-7 | 2026-06-04 | 2026-06-04/marketpulse | mix | apps/market_pulse | **KeyError=해소 / FileNotFoundError=검증후 분리판정** | - | KeyError(unregistered task) 원인 = 좀비 Beat 56670 (NT-10과 단일 origin). 종료 완료(2026-06-06 21:30), 6/7 회차에서 KeyError 0건 검증. FileNotFoundError(613건)는 좀비와 별개 가능 → 6/7 회차에서 잔존하면 별도 STEP 0. **구조 이동은 보류(D1, DECISIONS.md 2026-06-06) → `STRUCT-CLEANUP` 트랙으로 강등.** | 🆕신규 |
...
| NT-11 | 2026-06-06 | beat_schedule_audit | ops | ops(daily_report) | **완료대기검증 (6/7 07:00 KST)** | - | daily_report stray celery 가드 박힘 (분기 B, detection-only). worktree `monorepo/sess-mgmt-nt11-stray-guard`, C1 `0685b27`(가드+표면화) / C2 `428f1ad`(회귀 7건) / C3 `8b0c340`(common-bugs #33 보강). 라이브 오탐 0건 검증 (5 worktree 도출, 정상 launchd 프로세스 안전). NT-11-1 STEP 0=무효(이미 등록). NT-11-2/3/4=별도 보류. **health_check 통합·watchdog 셸 수정은 NT-11b로 분리**. | 🆕신규 |
| NT-11b | 2026-06-07 | NT-11 후속 | ops | scripts/health_check + watchdog 셸 | 보류 | 검문소 정책 결정 후 | health_check.py에 stray 룰 추가 시 **non-blocking WARN(exit 0) only** 강제 — 검문소는 차단형이라 detection-only와 충돌. watchdog 셸/plist에 origin 가드 포팅 = 사용자 수동 영역(셸 AI 무수정). 현재 daily_report 가드만으로 07:00 KST 검출 충분. | 🆕신규 |
