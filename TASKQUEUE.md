...
| GUIDE-MACRO-REVIEW | 거시 허브 가이드(`marketPulse.macro`) 검수 → draft→confirmed 전환 | 사용자/병진 | MP2-SUBPAGES-S1 land | ✅ **완료 (2026-09-15, worktree `sv-guide-macro`, 브랜치 `monorepo/sess-guide-macro`)** | D-GUIDE-MACRO-CQ(B 이어붙이기)·D-GUIDE-MACRO-GLOBAL(ⓐ 문구만 정직화)·D-GUIDE-MACRO-NAV(가 문구 흡수)+AUTO-1~3. `reviewStatus: 'confirmed'` 전환 + `guideData.test.ts` `PENDING_REVIEW` **빈 집합**(대기 0). **diff = FE 2파일**(`lib/guide/marketPulse.ts` +14/-11, `__tests__/guide/guideData.test.ts`). 위젯·백엔드·`tests/dogfood/` diff 0·마이그 0·web 리빌드 불요. 검증: guide vitest **41 passed 전후 동일**·mp2 **396 passed**·`guideAnchors` **11/11 GREEN**(앵커 4개 불변)·tsc **0**·lint **328 problems 전후 동일(순증 0)**·`pytest tests/dogfood` **103 passed / 0 failed 전후 동일**·`tests/architecture` **9 passed**. **★실효는 둘뿐**: ⑴ 가이드 문구가 화면과 정합(없는 데이터 약속 제거 — 해외 지수·DXY) ⑵ `rubric_targets()` **5 → 6**(실측). **dogfood RED 해소는 이 트랙 공로 아님** → `MP2-DOGFOOD-RECONCILE` 참조. 원안 Part 3(dogfood 기대 route 집합)은 **애든덤 A로 폐기**(회귀 유발). |
