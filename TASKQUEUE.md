...
- **CS-RESIDUAL-RC-POLLUTION-SWEEP 잔여 RC 오염 sweep** — R-2/SCE-POLLUTION은 `current()` target-set 기준이라 **SCE가 supersession으로 NULL됐지만 선착지 RC 행이 잔존**하는 케이스 미포착(관찰례: `ORCL→INCY` COMPETES_WITH=Epic Systems Corporation→INCY 오해소 잔재). evidence SEC4종 RC를 백킹 SCE.current() 부재·저유사 기준으로 재감사→비파괴 excluded. (신규·CS-P5 화면 검증 중 발견·backend write=별도 승인)
