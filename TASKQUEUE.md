...
| MON-P3-S1 | IA-2 리스트 페이지 (상태 우선 정렬+스코프 필터 칩+가설만 칩, 카드=달위상·화살표·상태·전제 n/m·D-day) | @frontend | MON-P3-S0 | todo | ADR §결정5 |
...
| CD-STAB | (측정 완료 2026-07-09) cd_state 안정성 축별 분해 + 처방 후보 소급 시뮬(읽기전용, 코드0). **0-2 재현 OK**(289/0.611 정확). **축별 분해(289반전)**: **X기인(rel 1일차분) 184=63.7%** · Y기인(mom5) 60=20.8% · 동시 45=15.6% → **x축이 플래핑 주범**. 지속성: 1일유지 비율 **0.633**(순수 노이즈, run 300 중 190). **후보 소급 반전율**(원본 0.611): A(x 5d MA) 0.332 · A′(진짜 5d 상대수익 mom−bench5d) 0.326 · B(2일 히스테리시스) 0.209 · C(A+B) 0.175. 판단지연: A/A′ 0, B/C 구조상 +1일. B/C 1일유지 0.047/0.054 | @backend | 📊 **측정 완료 — 처방 결정 세션 대기(채택 미정)** | 수치만. 채택·설계는 디렉터 결정 사안(규칙: 측정 세션은 처방 안 고름). bench(SPY)=MarketIndexPrice 저장이라 A′ 실행가능(skip 0) |
