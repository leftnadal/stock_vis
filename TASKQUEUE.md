...
| SCAN-UX-2 | ⑦b 소감 구조화 반영 **(신설·선행)** — ㉮고정영역 컴팩트化(가시 종목 ↑)·㉯연속 상승/하락 방향 분리(`signal_direction` 기보유)·㉰테마 포함 사유 즉답(`signal_value` 기보유)·㉱스토리 프리셋(목업 동반 결정) + SCAN-FIX-1(strip 라벨) 편입 | @frontend (dashboard) | D-SCAN-UX2-FEEDBACK | 🆕 **todo(선행·자동 우선순위 마진 1.45)** | ㉮㉯㉰=FE 공짜 필드(베이커 무접촉·저비용)·㉱=목업 결정 후. 설계 입력=RECON-SCANDIAG-R1 결과. |
| RECON-SCANDIAG-R1 | ⑦b 관찰·이상 3건 진단(**읽기 전용**) — ⑴거래대금(+시총) 필터 무반응(단위 불일치 용의) ⑵필터 반응 지연(A2 성능) ⑶coverage audit 0·0·0+배지 부재(창 이동 가설: 7일 창 전진→8월 중순 61건 창밖 이탈=join 대칭 정상 가능성) | 읽기 전용 | D-SCAN-R1-OBS | 🆕 **todo(발급)** | 원인 확정→SCAN-UX-2 설계 입력. E파트=창 이동 확증·사용자 새로고침 테스트. |
