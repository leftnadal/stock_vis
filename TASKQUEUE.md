...
| SLICE18-CONTAINER | Slice 18 사용자 상태 그릇 4모델(UserGoal·WatchlistItem·WalletHolding·CashBalance). **STEP 0 HALT(2026-07-13)** — 2종 기존 동명·상위집합 중복(WalletHolding=apps/portfolio, WatchlistItem=shared/users) + WatchlistItem 교차앱 소비(dashboard·chain_sight)로 D1 전제 파기. **사용자 결정=정지·디렉터 재설계**(코드 0). 산출=`docs/portfolio/coach/slice18/STEP0_FINDINGS.md`+지시서 배치. baseline 574/0 | 결정 안건(D1 재결정) | 디렉터 재설계 세션 — 신규는 UserGoal·CashBalance 2종 축소 검토, 재사용 2종 어댑터 | 🛑 HALT·재설계 대기 |
