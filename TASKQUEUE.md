...
| C2-S1-DESIGN | **스테이지 1 설계 사이클(D-C2-GATE-SPLIT)** — ⓐ C2-DESIGN-JOIN-MISSES(창 경계 라벨링 정책) ⓑ **news_chip 커버리지 편입**(설계 지시서 서두 **read-only 검증=news_chip 일자별 행수 선행**, 미확인 시 스테이지 2 강등) ⓒ COVERAGE-SURFACE-CONST-UNIFY. 트리거·근거는 D-C2-GATE-SPLIT 포인터(문언 복제 금지). | 설계(dashboard/platform) | D-C2-GATE-SPLIT | 🆕 **등재(다음 순번 본류)** |
| C2-S2-REGATE | **스테이지 2 재게이트(08-13)** — 퍼널 상수 튜닝 · coverage_detail 층 편입. **트리거 문언 = D-C2-GATE-SPLIT 포인터**(방문일 기준·복제 금지): 08-06 이후 신규 방문일 ≥3·그중 coverage_detail 방문 ≥2일 → 개시, 미달 시 자동 연장(재게이트 반복·재결정 불요). 재게이트 측정 = MEASURE-C2-GATE 5축. | 상세(dashboard/platform) | C2-S1-DESIGN | 💤 대기(방문일 트리거) |
