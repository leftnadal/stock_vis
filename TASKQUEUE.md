...
| ID | Task | Agent | Depends On | Status | Output Artifact |
|----|------|-------|------------|--------|-----------------|
| CS-RD1 | 하네스 정합화 + 테마 데이터 적재 (Phase 0–1) | @backend | CS-R9 | **Part A·B done / Part C HALT** | `docs/chain_sight/redesign(26.06)/Cs redesign 01 harness and theme data.md` — Part C 분기 (다): `theme_tags` 생성 파이프라인 부재, 별도 적재 지시서 대기 |
| CS-RD2 | 관심도 M1 엔진 (모델·배치·API, Phase 2) | @backend | CS-RD1 | **blocked** (RD1 Part C 미완 — 보드 연료 0%) | `docs/chain_sight/redesign(26.06)/Cs redesign 02 attention m1 backend.md` |
