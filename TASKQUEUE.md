...
| MP-DATA-MACRO-COVERAGE | (ops/data) 거시 시계열 5종 미수집 — `vix·nfci·hy_oas_pct·t10y2y_pct·t10y3m_pct`가 `RegimeSnapshot.inputs`에서 actual null(sources=MISSING, 5/14만 OK). regime 분류 정확도·margin 게이지 의미값의 데이터 공백. **FRED fetcher 트랙 연결**(PR-A1 11시리즈 모델 기등록 → fetcher 신규만 필요). margin 구조·임계(rules.yaml)는 정확 = **코드 결함 아닌 데이터 사안** | @infra(@backend) | - | 🕒 **trigger-gated** — **다음단계 게이지 FE / 거시 의존 카드 의미값의 선행 조건**. 게이지가 "빈 값"이면 원인 = 이 트랙(메모리-코드 불일치 함정 방지) | 거시 5종 actual 채움(coverage 회복) → margin·게이지 의미값 자동 활성. FRED fetcher 구현은 별도 ops/data 세션(이 세션에서 구현 금지) |
| MP-UX-TITLE-SOURCE | 카드 제목 단일소스 — `card.regime`="시장 레짐"(KO_LABELS) vs CardShell 하드코딩 `"시장 국면"` 불일치(census #4). CardShell titleEn/titleKo + page.tsx CARD_TITLE → i18n 단일소스 전환 | @frontend | - | 🆕 **등록만** (결정 별도 턴) | 권고값 메모 "국면". 등록만 — 구현·결정 분리 |
