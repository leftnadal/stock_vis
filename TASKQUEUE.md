...
| L2-SOURCE-DECISION | L2-X → L2 소스 결정(교체/하이브리드/현행유지). **판정 방식 개정(D-L2X-AUTO-ADJUDICATION): 전건 사람검수 폐기 → 결정론 자동판정 + 감사**. 재료 = `--judge` 병합 CSV + `--aggregate` v2 구간별 지표(grounded·과신·상관불일치) + 감사 표본. 채택 시 "추정" 라벨 동반. | 설계 | 감사 표본 검수(세션밖) | ⏳ **자동판정 완료(240행, LLM0). 감사 표본 18건=`outputs/peer_experiment/peer_experiment_audit.csv`(검수도구 감사모드 [동의/이견]). verdict 회수→결정** |
