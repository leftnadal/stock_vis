...
| MP2-SECTOR-CD | 섹터 판단 화면(CD 4-상태). 순차 연속 슬라이스(D-TREND-CD-SEQ). rel_strength×momentum_5d 사분면 분류(신규 파생 0, baseline=0.0). RRG 회전 맵 = 서브스크린(D-SECTOR-NAV, Slice 3) | **#2.5** | @backend+@frontend | 🔵 **in-progress (Slice 1)** — D-SECTOR-CD 채택(4.50 vs 3.40) | 판정 로직 = payload builder 단독. FE 재분류 금지 |
| MP2-SECTOR-CD-S1 | Slice 1 = cd_state additive(payload) + 판단 카드(세그먼트 토글 [판단\|궤적], 디폴트=판단). `classify_cd_state` 순수함수 + `CD_REL_STRENGTH_BASELINE`/`CD_MOMENTUM_BASELINE`=0.0 상수 + FE CD_STANCE 정적 5문구(LLM 0) + 2×2 미니맵 + 유보 처리 | @backend+@frontend | 🔵 **in-progress** | additive-only, 회전 맵 어포던스 미포함(Slice 3) |
| MP2-SECTOR-CD-S2 | Slice 2 = 모멘텀 시계열. sector_history[] per-date momentum additive 노출 + 궤적 뷰 심화. 변형1/2 목업 미결 | @backend+@frontend | 🕒 대기 (S1 land 후) | sector_history 계약에 momentum 추가(현재 rel_strength+rank만) |
