...
| **[GATE:release] MP-LIVE-VERIFY** | **Phase 1 출시 전 필수 — 라이브 검증 게이트** | @qa + @frontend | MP1-K · MP1-L · MP-KL-F3 | 🟡 **1차 결과(2026-06-11, d5212d4 검증)**: **계약(C·D) 전건 PASS** — overview 키 concentration·/cards/flow 404·i18n 라벨·5 카드 렌더·drawer detail 라이브 확정. **Part B(5종 데이터)**: Briefing은 **MP-LV-D2 수리로 확보**(brief 카드 재게이트 통과) / Concentration은 **MP-LV-D1 결정 대기**. **잔여 재게이트 = Concentration 데이터 생성 + 해당 카드 스모크만** (계약 재검증 불요 — 수리가 계약 미변경) | 검증 보고서(curl 응답 + DOM 채증) + DECISIONS "[2026-06-11] 게이트 1차 결과". Concentration 데이터 확보 후 잔여 스모크 통과 시 "전건 통과" 표기 |
...
| **MP-LV-D1** | **결정 안건** — Concentration task가 FMP `/stable/etf/holdings`(프리미엄, Starter 미지원, CB[fmp_etf] OPEN, 05-06 이후 중단) 의존. 데이터 산출에 필요한 입력 = 종목별 `weightPercentage` 단일. Starter 대체 후보(실측): `/stable/sp500-constituent` + `/stable/key-metrics`(marketCap 기반 가중 근사) 등. **옵션 결정은 채팅 몫** | orchestrator + @backend | - | 🗳 **결정 대기 (수리 금지)** | DECISIONS 엔트리 + 후속 수리 트랙. 측정값: 2026-06-11 MP-LV-D1 실측(필드/대체 엔드포인트/모델 구조/#23 대조) |
...
| MP-UX-POLISH | market-pulse-v2 사용자 대면 표면 개선 — raw 전문어/약어 노출(HHI·top5·top10·dispersion·rotation·AD-line·coverage·momentum) + 단위/맥락 없는 raw 숫자(HHI 0.0211) + 용어 도움 인프라 부재(tooltip/glossary 0) + i18n en 백엔드 라벨 0(FE FALLBACK 의존) | @UI-UX-designer → @frontend | MP-LIVE-VERIFY 전건 통과 후 | 🆕 신규 (재게이트 종료 후 착수) | ⚠ Phase 1.5 Translation Layer와 범위 중첩 가능 — 착수 전 범위 조정. 근거: 2026-06-11 UX 전수조사 |
| MP-I18N-EN | i18n en 로케일 백엔드 라벨 부재 — `get_labels('en')` 빈 응답 + `_meta.warning='unsupported locale: en'`, FE FALLBACK_LABELS(8키)만 의존. ko 28키 대비 en 백엔드 0 | @backend | - | 🆕 minor (MP-UX-POLISH 범위 포함 가능) | `apps/market_pulse/i18n/labels.py` EN_LABELS 추가 또는 의도적 ko-only 명문화 |
