...
| DEPLOY-RUNBOOK | 워커 런타임 동기 절차 정의(주기·체크리스트·runbook) — launchd kickstart + migrate --plan 게이트 + 재기동 창 확인 표준화 | @infra | — | 🕒 **등재(D-MPS-OPS-SYNC 파생)** | 이번 점등에서 임시 집행한 동기 절차(ff checkout→migrate --plan→check/health→창확인→kickstart→PT무손상)를 재사용 가능 runbook으로. 스테일 런타임 트리 재발 방지 |
