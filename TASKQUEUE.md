...
| NEWSFIX-BE (NEWS-MATCH-FIX) | 스캐너 실뉴스 매칭 수리 — **1′ 재배선**(enricher 4쿼리 `StockNews`→`NewsEntity`·경계 분기 내장: `apps.*` 직접 import 시 의존 역전 주입, 주입도 구획 위반 시 HALT+폴백 2안 sync) | @backend (+platform 경계) | D-NEWSMATCH-FIX-PATH | 🆕 **build 대기(지시서 발급됨·NEWSFIX-BE)** | 방향 확정(1′ 4.35 vs 2안 3.60 vs 3안 3.50·마진 0.75). **착지 후 효과 3종 검증**: ⓐ스캐너 `news_context`(profile 폴백 이탈·symbol/industry 매칭 복원) ⓑ추천 카드(다운스트림 자동 치유) ⓒ뉴스 존재 칩(0렌더→점등). SCAN-STORY-LLM 서사 층 선행 제약. **common-bugs `#NN` = 착지 확인 후 차기 mgmt 부여**(비mgmt 채번 금지). |
...
- 🟡 **RC-LAUNCHD-PGBACKUP-TREE** (@infra, 우선순위 3) — **잔여 유일 항목**. 래퍼 교정은 @`9a17e324`로 완료됐으나 **plist 교체 미집행**(2026-08-31 집행 세션에서 `cp` 권한 미허용으로 차단). 초안 `com.stockvis.pg-backup.plist.proposed`(plutil OK) 대기. 절차 = bootout → 백업 → 교체 → bootstrap → 다음 02:00 실행 로그 확인. **위험 하향 정정**: 런타임 트리 `.env`가 Desktop 본체 `.env` **심링크**(동일 실체)로 확인 → 당초 우려한 `.env` drift는 현 구성에서 **발생 불가**. 교정 목적 = 규칙 일관성. 현재 로드·정상(runs 39, exit 0). 초안 = `com.stockvis.pg-backup.plist.proposed`.
