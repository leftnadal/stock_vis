...
|--------|------|------|-----------|------|
| OPS-BRIDGE-0 | M | main 발산 해소(`b0fadfa3`→`monorepo/sess-cs-s3-1d` 보존 · 본체 reset→origin/main) + 즉시 랜딩 2건(CS-S3-1D · EOD-TIME-1) + 배포(`sv sync` + web 리빌드 §2.2) + D-OPS-BRIDGE 등재. 지시서 `inbox/OPS-BRIDGE-0.md` | — | 🟡 in_progress (2026-09-17 ops 세션 · 결과 = `outbox/OPS-BRIDGE-0_보고.md`) → **S2 LANDED `85eae8d8`(CS-S3-1D) · S4 배포 `0ff0949e`(3트리+web 리빌드) · S3 = ⓐ 분해 랜딩(`monorepo/sess-eod-time1-r4`, R4 `0542dd77`) — 착지 해시는 outbox 3차 절** |
...
| CS-S3-1D | M | **D-S3-9 인용 규칙 A+**(BE 중심 + FE 문구 1건). A: 묶음은 `pairs[]` 전 쌍 조회→기사 합집합→**멤버 커버리지 기준 카드 단위 재정렬** + `covered_members` 부착. B: **제목 적중 게이트**(제목이 members 를 말하는 기사만 인용·티커 단어경계 대소문자 구분 + 회사명 사전) — 8-K 템플릿 제외·**evidence 불변**. C: 빈 상태 두 갈래 `title_state`(no_article / **no_member_article** 신규) + FE 문구 분기. 사전 = `Stock.stock_name`(757/757) + `CompanyAlias`(173행·보강 창구). 판정 단일 출처 = `services/title_gate.py` | CS-S3-1C | ✅ **done (게이트 D-2 PASS: 제목 보유 30/30→21/30·지운 카드 9=30%<50%). pytest chainsight+architecture **856**·vitest chainsight **322**·tsc0(변경분)·eslint0·ruff0(변경분)·마이그0·쿼리 80→94. 5장 재대조: GOOG·QCOM(AMZN 제목)·DUK·INTC·NEE·SO(CLF 제목) **제거**, AEP·DUK·SO/IBM·MRK/HD·MDT·MSFT·PG **커버리지 개선**(covered=멤버 전원). health 항목 E 추가. ★배포 후보** |
...
| NEWSFIX-SYNC-BE | **NEWS-MATCH 수리 확정 경로(V2 ⑵)** — 앱측 sync beat: `NewsEntity`→`StockNews` 주기 물질화(멱등·max_retries=3). enricher 무접촉 | @backend/news (services.news) | D-NEWSMATCH-FIX-PATH-V2·D-OWN-NEWS | ✅ **착지 (LAND-NEWSFIX-SYNC `b731d7b4`, 09-02)** | pytest 242→251·makemig no-op·회귀 260 green. **common-bugs 부여 완료 = #128**(MGMT-BATCH-45). ✅ **prod 점등 완료 — 위 stale 서술 정정(SCB-RECOVER-PROBE, 2026-09-15 실측)**: `StockNews` **110,887행**(09-15 09:20 KST 실측; recon 09-10 시점 98,583행 → 계속 증가, 09-14 하루 1,112행)·beat `newsfix-sync-stocknews` **enabled=True**(last_run 2026-09-14 21:30 UTC·total_run 8). 구 서술 "prod 무변(StockNews 0행·beat enabled=False)"는 NEWS-DEPLOY-EXEC 집행 전 기록으로 **사실과 다름**. 근거=`docs/mgmt/SCB-CONTEXT-RECON_report.md` §C + 본 세션 런타임 DB 실측. |
...
| MPS-OPS-GLD-DASH | 티커 스트립 금·은 "—" — 심볼 배선 불일치(402 아님) | @backend | — | ✅ **해소 종결 (HUB-V02-S1, 09-03·커맨드 집행)** | 근본원인=RECON Part E(GCUSD/SIUSD 127행 있으나 sector_group=None 제외 / GLD·SLV BENCHMARK 0행) → **402 반증**(프리미엄벽 아님·순수 배선). **S1 코드**: `_ticker_bar` null-price 스킵(GLD/SLV "—" 제거·live 15심볼 확증) + TickerBar 금/은 라벨. **집행 완료(병진)**: `fix_commodity_ticker_wiring --commit` → **commodity 2행**(GCUSD/SIUSD→BENCHMARK). D-BREADTH-ASOF 자매. cf. SCHEMA-DRIFT-SECTORGROUP(잔여 스키마 정합 별건). |
| SCHEMA-DRIFT-SECTORGROUP | `MarketIndex.sector_group` 모델 non-null(default BENCHMARK) ↔ prod nullable 드리프트 | @backend | — | 🕒 **지뢰 등재(등재만·D 채번 없음·트리거=다음 MarketIndex 모델 접촉)** | S1이 None 쓰기 회피로 우회(GCUSD/SIUSD를 BENCHMARK로 배선). **위험**: 향후 makemigrations가 non-null 마이그레이션 생성 시 prod NULL 행과 충돌. **해소 후보(등재만)**: ⑴ prod NULL 잔여 행 실측 → 값 채움 후 스키마 정합 ⑵ 모델을 nullable로 정직화. 착수 금지(트리거 대기). |
...
| COLOR-TOKEN-UNIFY | AnomalyPanel rose(경보) → `stressAlert` 토큰 통일(색 단일소스 확장) | @frontend | — | 💤 **휴면(트리거=다음 AnomalyPanel 접촉)** | D-MPS-COLOR: 스트레스=경보 프레임 신설. AnomalyPanel은 MPS-2서 무접촉(행위보존) → 다음 접촉 시 rose 경보색을 stressAlert 토큰으로 통일(drift 방지). **추가 대상(2026-08-31)**: 허브 `app/market-pulse-v2/macro/page.tsx`의 amber 배지("N분 전 데이터")·안내 박스("준비 중") 2곳 = 인라인 `amber-50/700/200` 문자열 재기입 → 다음 접촉 시 `StatusBanner`의 STALE 토큰 import로 통일(현재는 HOTFIX-1 속도 우선으로 인라인). |
...
| PLAYBOOK-DOLLAR-V02 | dollar_squeeze 원자재 다리(달러↑→원자재↓) 추가 — GLD·SLV 수집 후 | @backend | ~~MPS-OPS-GLD-DASH(GLD·SLV 조사)~~ **충족** | 🟢 **착수 가능(선행 언락, HUB-V02-S1 09-03)** | **선행 충족**: 금·은 실데이터 확보(GCUSD/SIUSD 127행·BENCHMARK 배선 완료) → GLD·SLV 조사 전제 해소. v0.1은 달러↑ ∧ 위험자산 약세 2조건(축소 정의·chains.yaml 주석). **착수 시**: 원자재 다리(3번째 조건) 추가 → dollar_squeeze v0.2. 원자재 다리 임계/조건 설계 = **별건 결정**. |
...
| MP2-SUBPAGES-HOTFIX1 | pulse 스테일-캐시 즉시 응답(SWR) + 허브 로딩 완화 | **#3 (A)** | @backend+@frontend | ✅ **배포 완료 (main `279d4080`, 08-31)** | D-SUBPAGES-SWR(C). BE: `macro:market_pulse_full{,:stale(24h),:refreshing}` SWR — fresh 미스 시 stale 즉시 반환+백그라운드 갱신 1회(락 dedup), 태스크 `force_refresh=True`. 스키마·뷰 diff 0. FE: `getMarketPulse/useMarketPulse({timeoutMs?})` additive(v1 diff 0), 허브만 20s 타임아웃+재시도 안내+"N분 전 데이터" 배지(amber 토큰 재사용, → COLOR-TOKEN-UNIFY 대상). **배포 검증**: merge+sv sync+web 리빌드+워밍 → pulse fresh 0.18s·**stale 경로 0.13s**(종전 콜드 28.8s)·실데이터 스크린샷 데스크탑 941ms/모바일 766ms. pytest +6, vitest +8(364), E2E 3스펙 2회 GREEN. 채번 #125(콜드캐시)·#126(SSR≠데이터). |
...
| MP2-SUBPAGES-S2 | 거시 허브 무버스 탭 — `MarketMoversSection` 흡수 | **#3 (A)** | @frontend | ▶️ **next (결정 사이클 선행: LLM 키워드 비용 게이트)** | S1·HOTFIX-1 배포 완료 → 이 트랙 next. **착수 전 결정**: `useGenerateKeywords`(Gemini·Celery async) 비용/rate 게이트를 디렉터 결정 사이클로 확정 후 지시서. **S1 STEP 0-7 재료**: 자체 fetch 3훅 — `useMarketMovers`(useQuery·staleTime 5m·**refetchInterval 5m**)·`useSyncMarketMovers`(mutation POST·sync 트리거)·**`useGenerateKeywords`(mutation→`keywordService.generateAllKeywords`=Gemini AI 키워드·Celery async·LLM 비용/rate 주의)**. 엔드포인트=serverless(market_pulse_user 미공유). 인증 경로·비용 게이트는 S2 STEP 0 재확인. 허브 무버스 탭은 현재 "준비 중" 배지. |
| GUIDE-MACRO-REVIEW | 거시 허브 가이드(`marketPulse.macro`) 검수 → draft→confirmed 전환 | 사용자/병진 | MP2-SUBPAGES-S1 land | ✅ **완료 (2026-09-15, `4c941329` · worktree `sv-guide-macro`, 브랜치 `monorepo/sess-guide-macro`·미push)** | D-GUIDE-MACRO-CQ(B 이어붙이기)·D-GUIDE-MACRO-GLOBAL(ⓐ 문구만 정직화)·D-GUIDE-MACRO-NAV(가 문구 흡수)+AUTO-1~3. `reviewStatus: 'confirmed'` 전환 + `guideData.test.ts` `PENDING_REVIEW` **빈 집합**(대기 0). **diff = FE 2파일**(`lib/guide/marketPulse.ts` +14/-11, `__tests__/guide/guideData.test.ts`). 위젯·백엔드·`tests/dogfood/` diff 0·마이그 0·web 리빌드 불요. 검증: guide vitest **41 passed 전후 동일**·mp2 **396 passed**·`guideAnchors` **11/11 GREEN**(앵커 4개 불변)·tsc **0**·lint **328 problems 전후 동일(순증 0)**·`pytest tests/dogfood` **103 passed / 0 failed 전후 동일**·`tests/architecture` **9 passed**. **★실효는 둘뿐**: ⑴ 가이드 문구가 화면과 정합(없는 데이터 약속 제거 — 해외 지수·DXY) ⑵ `rubric_targets()` **5 → 6**(실측). **dogfood RED 해소는 이 트랙 공로 아님** → `MP2-DOGFOOD-RECONCILE` 참조. 원안 Part 3(dogfood 기대 route 집합)은 **애든덤 A로 폐기**(회귀 유발). |
| MACRO-CARD-NA-ARROWS | 지수·섹터·환율·원자재 전 행 N/A | @backend→@frontend | — | 🔵 **근인 확정 (2026-09-19, MACRO-COHERENCE-RECON) · 수리 A 승인 대기** | **근인 = 백엔드 필드명 불일치.** `packages/shared/api_request/providers/fmp/market_pulse_client.py`가 `quote.get('changesPercentage')`(레거시 v3 이름)를 읽는데 FMP `/stable/quote` 실응답 키는 `changePercentage`(s 없음) → **항상 None**. 프로브 확증(SPY `changePercentage=1.13388`, `changesPercentage` 부재) + 디렉터 산술 교차검증(09-17 종가 754.05 + change 8.55001 = 762.60 = 페이로드 price, 4/4 일치). **파급 6곳**: `:215` 지수 · `:246` `get_all_market_quotes`(celery `tasks/macro.py:95` → `MarketIndexPrice.change_percent`에 **NULL 적재, 매일 누적**) · `:272` 섹터 · `:322` 원자재 · `:346` 환율 · `:364` DXY. 같은 저장소 선례 2곳(`stocks/views.py:605`·`stock_sync_service.py:228`)이 이미 `changePercentage or changesPercentage` 폴백을 쓴다 — 이 파일만 누락. **수리안 A(권고)** = 6곳 동일 폴백. 경계 = `packages/**` 수정 + **api·worker 재배포**(⚠api 배포가 3-A 백엔드 5파일 동반 → 타 트랙 조율 선행 = `RUNTIME-DEPLOY-DRIFT-API`). **MACRO-FE1(2026-09-19)로 증상 일부 해소**: 결측을 하락으로 그리던 분기가 없어져 화살표·pill이 중립이 됐다. 값이 채워지면 **코드 변경 없이 정상 동작**. 잔여 = 값 자체. 부수 확정: `price` 원천은 **ETF 프록시**(`INDEX_SYMBOLS` 주석 = FMP Starter가 ^인덱스 미지원) → 값 오류 아님, 라벨이 페이로드 `name`("S&P 500 (SPY)")을 버리고 하드코딩되는 표기 문제. |
| MACRO-NFP-UNIT | NFP 단위 불일치 — 카드는 명, 의미문은 천 명으로 읽었다 | @frontend | — | ✅ **해소 (2026-09-19, MACRO-FE1 `fb463d4e`)** | 판정 = 천 명(k). 원천 FRED `PAYEMS`(Thousands of Persons), `fred_client.py`가 `# 천 명 단위` 주석과 함께 통과시키고 `_parse_value`는 `float()` 그대로(스케일 0). 실값 `nfp_change=162.0` · 교차확인 `initial_claims=196000.0`(ICSA는 명 단위 6자리). 따라서 `NFP_BREAKEVEN=87(k)` 기준 "견조"는 정확했고 **카드가 틀렸다** — `EconomicIndicators.tsx`가 `/1000` 재나눗셈해 `+0K`. **수리**: `/1000` 제거(`162 → +162K`) + `0`을 `N/A`가 아닌 `0K`로(결측과 실제 0 구분) + 아이콘·색을 `null`·`0`에 중립으로. **위젯 파일 diff 0 규율은 병진 승인 예외 1회**(실버그·v1 `/market-pulse` 동시 교정). 회귀 = `macro_widget_units.test.tsx` 8 it. 라이브 실화면 확인 완료(2026-09-19). |
| FALSY-ZERO-GUARD | `0`을 결측으로 읽는 falsy 검사 | @frontend·@backend | — | 🔵 **부분 해소 · 잔여 2건 (2026-09-19)** | `0`과 `null`을 구분하지 않는 패턴. **MACRO-FE1에서 해소 4곳**(`GlobalMarketsCard` 지수 아이콘·섹터 pill, `EconomicIndicators` NFP 아이콘·색·숫자). **잔여 ⑴** `GlobalMarketsCard`의 헬퍼 `formatChange`/`getChangeColor`가 `>= 0`을 초록으로 보므로 **정확히 `0.00%`인 날 초록 상승 아이콘** — 거짓은 아니나 관례와 어긋남. MACRO-FE1이 **의도적으로 남긴 공백**(헬퍼 의미를 바꾸면 다른 소비처·테스트로 번짐). **잔여 ⑵** `apps/market_pulse/services/macro_service.py` upsert `if data.get('change_percent')` → **변화 0인 날을 DB에 NULL로 기록**(결측과 구분 불가). **전수 미조사** — 두 위젯 밖은 안 봤다. common-bugs 채번 후보. |
| RUNTIME-DEPLOY-DRIFT-API | api 런타임이 3-A 백엔드 5파일을 못 받았다 | @ops (**3-A 트랙 판정**) | — | 🕒 **등재만 (2026-09-19)** | 실측: `sv-api-runtime` = `e2e4aa9a`로 3-A 커밋 `4a77f5c8`·`5a481e70`·`4fe41148`·`98055d73` **전건 미포함**. `sv-worker-runtime`은 포함. 못 받은 백엔드 변경 = **정확히 5파일, 전부 3-A**(`0012_claim_near_stop_notified_at.py` · `models/monitor.py` · `services/alerts.py` · `services/price_zone.py` · `services/scenario.py`). **판정 유보** — 워커 전용 기능이면 무해, api가 그 필드를 서빙해야 하면 기능이 반만 올라간 상태. **DB에 0012가 적용됐는지 미측정.** market_pulse 소관 아님. ※`MACRO-CARD-NA-ARROWS` 수리 A 배포가 api를 태우므로 **조율 선행 지점**. |
| MACRO-EDU-DATAFIT | 교육 콘텐츠가 화면에 없는 데이터를 설명 | @frontend | — | 🕒 **등재만 (2026-09-18)** | 글로벌 카드 교육 본문이 DXY·금·원유를 설명하는데 화면엔 DXY 칸 부재(`DATA-GAP-DXY`)·금/은 N/A·원유 미표시. `GUIDE-MACRO-REVIEW`에서 가이드 패널을 두고 고친 "없는 데이터 약속" 문제가 **교육 콘텐츠 층에서 재발**. HUB-SENSE-DETAIL S1이 4장에 교육을 깔면서 노출 — **디렉터 지시서에 콘텐츠-화면 정합 게이트가 없었던 결함**. 심리·금리·물가고용 3장도 같은 점검 필요(미실시). |
| MACRO-HUB-COL-GAP | 2열 높이 불균형 — 왼쪽 열 하단 대형 여백 | @frontend | — | 🕒 **등재만 (2026-09-18)** | 심리 카드+트레이가 금리 카드+트레이보다 짧아 `md:grid-cols-2` 왼쪽 열 하단에 큰 빈 공간. `MACRO-HUB-TAB-HALFWIDTH`(탭 좁힘 시 오른쪽 절반 공백)와 **다른 건**. 경미. |
| MACRO-HUB-TAB-HALFWIDTH | 거시 허브 탭 좁힘 시 `md:grid-cols-2` 오른쪽 절반 공백 | @frontend | — | 🕒 **등재만·착수 금지 (2026-09-15, GUIDE-MACRO-REVIEW에서 분리)** | 상단 서브탭으로 좁히면 `md:grid-cols-2` 섹션에 자식이 1개만 남아 **오른쪽 절반이 빈 공간**이 된다. `?tab=rates`·`?tab=sentiment` **양쪽 재현**. 근거=병진 라이브 스크린샷(09-14). 탭 활성 시 1열로 무너뜨릴지 카드를 넓힐지 = 설계 결정 선행. |
| MACRO-SENSE-PLACEMENT | `SenseNote` 배치 — 위젯 카드 바깥·맨 아래 | @UI-UX-designer→@frontend | — | ✅ **해소(D-SENSE-PLACEMENT = ⓑ, 2026-09-17)** | 카드 아래 끼움 트레이 + 순서 역전 + 4장 균일. 위젯 파일 diff 0. 집행 = `HUB-SENSE-DETAIL` S1. 착수 전 "~700px" 수치는 **재현 실패**(심리 카드 354px) — 인용 금지. 거리 미해결분 40px은 기한부 관찰로 이월. |
| HUB-SENSE-DETAIL | 거시 허브 4카드 설명 스택 이설 + "왜 이렇게 읽나" 토글 | @frontend | `D-SENSE-PLACEMENT` ✅ | 🔵 **S1 랜딩 완료 · S2 대기** | S1 = 트레이 이설·순서 역전·4장 균일(신규 `EducationNote.tsx`·`SenseSlab.tsx`, `macro/page.tsx` 배선, `macro_hub.test.tsx` 14→19 it). 디렉터 검수 합격(독립 실측 대조 + 스크린샷 판독, 봉인 채점표 3/3). **S2** = `macroDetail.ts` + 3줄 토글(오늘 값·이 구간의 뜻·다음 확인 지점) + `MACRO-SLAB-RADIUS` 동반. |
| MACRO-SLAB-RADIUS | 설명 트레이 모서리 반경이 카드와 불일치 | @frontend | — | 🕒 **등재만 · 우선순위 하향 (2026-09-18→19)** | `SenseSlab`이 `rounded-b-lg`(8px), 위젯 카드가 `rounded-xl`(12px). **실화면 판독 결과 체감되지 않는 수준** → 랜딩 차단 아님. `rounded-b-xl` 한 글자 수정, S2에 묶는다. |
| MP2-CORS-DEVPORT | dev CORS가 :3000만 허용 — 워크트리 캡처 포트에서 API 차단 | @backend | — | 🕒 **등재만 (2026-09-17)** | `config/settings.py:331` `CORS_ALLOWED_ORIGINS`가 `:3000`만 허용. 런타임 트리와 별개로 워크트리를 다른 포트(:3100)에 띄워 실렌더 캡처할 때 pulse 응답이 막힌다. HUB-SENSE-DETAIL S1은 백엔드 diff 0을 지키려 **캡처용 chromium에만 `--disable-web-security`**로 우회(repo·설정 무수정). 반복 작업이므로 dev 한정 포트 범위 허용 검토. 추가 관측: `127.0.0.1`이 아니라 `localhost`로 접속해야 요청이 나간다. |
| LEDGER-OVERWRITE-GUARD | `DECISIONS.md` 전량 덮어쓰기 사고 — 8,091줄 → 22줄 | @qa·@infra | — | 🔵 **복구 완료 (2026-09-22) · 재발방지 미착수** | 사고: 커밋 `45fdcaa8`(2026-09-20 09:44, `docs(decision): record Research Chat and Work role boundary`)가 `DECISIONS.md`를 **−8,081 / +12**로 통째 교체해 09-20 09:44~09-22 09:3x 동안 `origin/main`에서 전 결정(D-* 8,069줄)이 사라져 있었다. 다른 원장 3종(TASKQUEUE·PROGRESS·common-bugs)은 무사. **복구 = MGMT 0920 배치**(원본 `45fdcaa8^`=`dcd51032` 8,091줄 전량 + 그 사이 신규 2건 `D-RESEARCH-WORK-BOUNDARY`·`D-GUIDE-ANCHOR-DEFER-1031` 보존, 원본 줄 삭제 0 확인). **근인 = 원장을 append가 아니라 새 파일로 쓴 것** — `.gitattributes`의 `merge=union`은 이런 전량 교체를 막지 못하고 되레 되살아난 줄과 섞일 수 있다. **재발방지 후보**: ⑴ `DECISIONS.md` 대량 삭제(예: −200줄 초과) pre-commit 차단 ⑵ 원장 파일은 append-only 규율 명문화 ⑶ Research 트랙 등 외부 세션에 원장 기입 규약 전달. 채번·집행은 미착수. |
| WEB-RUNBOOK-LAUNCHD | `web-runtime-runbook.md` §2가 실제 서빙 방식과 불일치 | @ops | — | 🕒 **등재만 (2026-09-18, S1 랜딩에서 발견)** | §2는 kill + orphan-subshell 전제인데 `:3000`은 이미 launchd `com.stockvis.web-frontend`(KeepAlive)가 서빙한다 — §4.1이 예고한 "해소 = launchd 정식 등록"이 적용된 상태. 리스너를 죽여도 launchd가 되살리므로 §2 절차는 오해를 낳는다. S1 랜딩은 `launchctl kickstart -k`로 집행(pid 70054/70067 → 13931/13943, 생존 8틱 48초 HTTP 200·pid 불변). §2를 kickstart 기준으로 갱신하고 §4.1의 "예고" 문구를 "적용됨"으로 정정할 것. |
| GUIDE-DRAFT-GATE-SINGLE | draft 게이트 단일화 검토 — `PENDING_REVIEW`가 repo에 남은 유일한 draft 게이트 | @qa | 두 번째 draft 발생 | 🕒 **등재만·선제 추상화 금지 (2026-09-15, 애든덤 A 실측)** | dogfood의 confirmed 단언이 `113b48a3`에서 **draft 허용으로 완화**되면서, `guideData.test.ts`의 `PENDING_REVIEW`가 **repo에 남은 유일한 draft 게이트**가 됐다(현재 빈 집합). 두 번째 draft가 실제로 생기면 `guideAnchors.test.ts`의 **기한부 패턴**(`PENDING_ANCHORS` = `Record<until, why>`, GUIDE-CS-GUARD-1D)과 형태 통일을 검토한다. **지금은 등재만** — draft 1건으로 추상화하면 근거 없는 일반화(γ). |
| P2-SUBPAGES-CLOSE | S1·HOTFIX-1 뒷정리 — 채번 + 브랜치 위생 + 원장 | @mgmt (b44) | HOTFIX-1 배포 | ✅ **done (2026-08-31, mgmt b44)** | 채번 #124(drf-spectacular)·#125(pulse 콜드캐시)·#126(SSR≠데이터)·#127(조기삭제 사고, 신규). D-REMOTE-BRANCH-HYGIENE **가안** 등재(랜딩 후 원격 삭제, 병진 B2 확정 대기). 브랜치 소진표: 로컬 144/소진 129/미소진 15, 원격 62/소진 54/미소진 8. 병진 스크립트 2본(`scratchpad/cleanup_{local,remote}_2026-08-31.sh`, 집행=병진) + 소진표 md + 체크리스트. push=병진 승인(docs-only). |
| BRANCH-HYGIENE-EXEC | 원격 소진 54 삭제 집행 + 로컬 worktree 정리 | 병진 지시·CC 집행 | D-REMOTE-BRANCH-HYGIENE ✅확정 | ✅ **done (2026-09-01)** | 원격 소진 **54 삭제/스킵 0**(게이트 통과). 남은 원격 sess 9 = 미소진 8 + 신규 1(newsfix-sync). 로컬 Group A(hf1+mgmt-b29~38) 제거·worktree 65→55. 보호 대상(본체·런타임 3트리·b44·미소진 15) 무결·과삭제 0. **잔여 = 로컬 Group B**(나머지 소진 worktree, 활성 세션 확인 후 병진 주석 해제) = `cleanup_local` 스크립트에 주석 보존. |
...
| MP2-DATA-BREADTH-CONC | **Breadth** — 날짜 off-by-one 수리(HUB-V02-S1) | @backend | — | ✅ **해소 종결 (원인=날짜 함정·수집 아님·09-03 커맨드 집행)** | ★**수집 미배선 아님**(등록 시 오진단)=`calculators/breadth.py:112` `localdate()`로 EOD 조회→항상 0. RECON·HUB-V02-S1이 원인을 날짜 off-by-one으로 확정(#129). **S1 코드**: `resolve_as_of_date`(직전 거래일)·compute·store 일원화·`_breadth_card` 실데이터 우선+`as_of_date`·`indicator_stale` 신선도 기반. **live 확증**: compute=501/299/201. **집행 완료(병진)**: `recompute_breadth_history --commit` → **791/813**(purge 없이). Concentration은 실제 정상(64행·top10 0.3812). D-BREADTH-ASOF. **잔여 purge = BREADTH-PURGE-2차(별건·병진 판단)**. |
| HUB-V02-S1 | 허브 v0.2 신뢰 수리 3건(Breadth 날짜·Anomaly 정직화·금은 배선) | @backend+@frontend | D-HUB-V02 | ✅ **랜딩·집행 완료 (2026-09-03·main `6e650a38`)** | A-1 Breadth(→MP2-DATA-BREADTH-CONC)·A-2 Anomaly(→HUB-ANOMALY-HONESTY)·A-3 금은(→MPS-OPS-GLD-DASH). pytest +15·vitest +5·E2E 3스펙 2회 GREEN·tsc0·marketpulse 540 무회귀. **커맨드 2건 집행(병진)**: `fix_commodity_ticker_wiring --commit`=**commodity 2행**·`recompute_breadth_history --commit`=**791/813**(purge 없이). 디렉터 검증 PASS + 관찰 2건(→SCHEMA-DRIFT-SECTORGROUP·HUB-ANOMALY-PARTIAL). **다음=HUB-V02-S2**. |
| HUB-ANOMALY-HONESTY | 이상신호 "정상 확인" vs "데이터 없음" 구분 | @backend+@frontend | HUB-V02-S1 | ✅ **done (2026-09-03)** | `_anomaly_section` `status` additive(evaluated/no_data): 행 없을 때 read-time `build_context` 소스 검사(compute-on-read·prod쓰기 0). 전부 MISSING→no_data("판정 불가"), 하나라도 OK→evaluated. FE AnomalyPanel no_data 중립(slate·경보 아님). live=evaluated(inputs 존재). |
| HUB-ANOMALY-PARTIAL | 부분 결측(일부 소스만 OK) 세분화 — evaluated로 뭉침 정직화 | @backend+@frontend | HUB-ANOMALY-HONESTY | 🕒 **관찰 등재(착수 금지·운영 관측 선행)** | S1 정직화는 "전부 MISSING=no_data"까지만. **부분 결측**(일부 소스만 OK)은 현재 evaluated로 뭉침. 세분화(예: `partial` 상태·커버리지 표기)는 **no_data/evaluated 실분포 운영 관측 후 별건**. 선제 추상화 금지(γ). |
| HUB-V02-S2 | 허브 v0.2 S2 — 전역 서브탭 + 정적 번역층(거시·로테이션) + 홈 fallback | @frontend+@backend | HUB-V02-S1 | ✅ **랜딩·배포 완료 (2026-09-09·main `b73da8e7`)** | D-MP2-SUBNAV(A)·D-MACRO-SENSE-STATIC(ⓐ). ⑴ `MarketPulseSubNav`(v1 미포획·개요/거시근거/로테이션/무버스) ⑵ `macroMeaning` 4종(공포탐욕·수익률·물가고용·4지수·COPYFIX 서술체) ⑶ 허브 위젯 아래 SenseNote 형제(위젯 diff 0) ⑷ 홈 **breadth·sector만** 정적 fallback(AUTO-1="카드당 의미문 1개 보장"·regime/concentration 중복 0) ⑸ 로테이션 RRG 한 줄. **백엔드 diff 0**. **3커밋 랜딩**: `e1faef19`(S2)+`0f878ee1`(COPYFIX 서술체·룰 중복 제거)+`32f5b015`(GUARDFIX status enum 불변식·연도 가드). vitest mp2 396 GREEN·tsc 0·E2E 4스펙 2회 GREEN·게이트 6종. **web 리빌드 완료**(런북 2.2·BUILD_ID kV1nr…·:3000 스모크 200·라이브 서브탭 렌더 확인). 잔여 카피 정확화=GUIDE-MACRO-REVIEW. |
| HUB-V02-S3 | 허브 v0.2 S3 — 위젯 지표별 시계열(추세·기간 프리셋) | @frontend+@backend | HUB-V02-S2 | ▶️ **next (D-HUB-V02 사이클)** | (가)그룹 즉시 가능·프리셋 최대 1Y(D-HUB-V02 자동결정 ①②). **S2 랜딩 후** 착수. S3 재료 실측치 = HUB-V02-S2 보고 §S3 참조. |
| DATA-GAP-DXY | `get_dollar_index()` 무응답 → `dxy: null` | @backend/@infra | — | 🕒 **데이터 공백 등재(S2 발견)** | 달러 꼬리말 미구현 사유(macroMeaning은 DXY 언급 금지). **PLAYBOOK-DOLLAR-V02 달러 다리의 선행 공백**. `get_dollar_index` 소스/파서 점검 필요. |
| DATA-GAP-GLOBALIDX | `macro_service.py` `global_indices` 하드코딩 `{}`(스텁) | @backend | — | 🕒 **데이터 공백 등재(S2 발견)** | 해외 지수(ftse/nikkei/hangseng) 비교 문장의 선행 공백. 코드 스텁 → 실수집 배선 필요. 현재 macroMeaning은 미국 4지수만 사용(해외 언급 금지). **비고(GUIDE-MACRO-REVIEW 09-15)**: 데이터 배선과 함께 닫되, **그 전까지의 빈 칸 정직화(제목만 남은 '글로벌 지수' 칸)도 이 트랙 몫**이다. 가이드는 문구로만 정직화했다(D-GUIDE-MACRO-GLOBAL ⓐ) — 위젯 파일 diff 0을 지키기 위해 UI 쪽 정직화는 넘겼다. |
| MACRO-SENSE-TUNE | 정적 문장 임계 재튜닝 — 물가 갭 1.0%p · breadth 0.5중심 사다리 · RRG 과반 경계 | @frontend | 실데이터 누적 | 🕒 **TUNE 대기(선제 조정 금지)** | 물가 갭 1.0%p = 2022–23 재현용(올해 core 2.5~2.9·갭 0.5~0.9 미발동). breadth `BREADTH_THRESHOLDS`(0.6/0.7)=관례 앵커(dev DB n=1 미검증). **RRG 과반 6/11**(`rotationSentence`)은 **6:5도 "과반이 개선 쪽"으로 서술**됨(근소차를 우세로 읽음) → **2/3(8/11) 경계 검토**를 실데이터 누적 후 재튜닝(S2-COPYFIX 등재). 실 SPY breadth·물가 누적 후 재튜닝(concentrationBand TUNE 선례 동일 규율). |
| BREADTH-PURGE-2차 | 비거래일 0-스냅샷 purge — `recompute_breadth_history --commit --purge-bogus` | @backend | MP2-DATA-BREADTH-CONC | 🕒 **대기(병진 판단·수동)** | S1 재계산은 `--purge-bogus` 없이 집행(791/813) → 비거래일 0-스냅샷 **21건 잔존**. purge 커맨드는 멱등. **판단 기준**: Breadth 궤적 차트에서 0-점이 선을 꺼뜨려 보이면 실행. 그 전엔 잔존 무해. |
...
- **CS-STORE-DEDUP 관계 store 이중화 해소 검토** — ★RC-D-0 전수 실측(09-07, `docs/reports/rc_d0_store_dedup_recon.md`) 갱신: RelationConfidence **13,947** vs serverless StockRelationship **246,158**(HELD_BY_SAME_FUND 197,034·SAME_REGULATION **47,346**·CO_MENTIONED 1,523). **두 store 완전 독립**(동시참조 0·승격경로 0·apps참조 0·frontend 실호출 0). SR **활발 유입 중**(13 beat enabled·09-05 최신). 양측 공존 무향쌍 6,503. **미정 핵심 = SR 197k HELD+47k SAME_REG(RC부재·Neo4j미동기·frontend미사용)의 처분.** Neo4j 이중노드(symbol 1,146 ⊎ ticker 534·교집합 0) 별건. D 결정(RC=단일정본) 처분 설계 대기.
...
- ✅ **[EVT-CHAIN-1(+1a)] Phase 2 관계망 이벤트 타임라인 + 미니 위젯 — 구현·소보정 완료(2026-09-03·sess-evt-8·커밋 로컬·push 대기)** (상세=앵커 §6·[[DECISIONS]] D-EVT-CHAIN-1·D-EVT-CHAIN-1a): 시드+RelationConfidence 1-hop 이웃 어닝, Postgres 단독 조인. BE `chain_feed.build_chain_feed`+API `calendar/chain/`·FE 4컴포넌트+scope=stock 附加. **★파라미터 실측 정정: truth_score≥85 → ≥0.85**(도메인 [0,1]·D-RC-SCALE·§0-4⑴). confirmed·top-k10·EARNINGS만·부호중립. **CHAIN-1a**: 관계 라벨=시드 기준 역할(SUPPLIES_TO/DEPENDS_ON 방향 도출·ACQUIRED 게이트로 중립 유지)·창 종점=시드 다음 **어닝**(배당 무시·GOOGL 아티팩트 수리). 원장 재작업 0. BE pytest **21신규**(monitor 56 green·architecture green)·FE vitest **37**(신규13+회귀+게이팅2)·tsc0·lint순증0. 확정=D-EVT-CHAIN-THRESH([EVT-OBS-3] 게이트).
- 🔭 **[EVT-OBS-3] EVT-CHAIN 파라미터 확정 관찰 게이트 (등재 2026-09-03)** — 2주 사용(**≈2026-09-17**) 후 판정: ⑴ 시드별 표시 행(items) **0이 과반**이면 status/truth 임계 재검토 / ⑵ 행 **>15 시드** 존재하면 top-k·창 재검토 → 디렉터 결정 사이클(**D-EVT-CHAIN-THRESH** 확정). **기준선(4-2 어닝창 실측 6종목·창내 행)**: PLTR 3 / TLN 0(타임라인 미표시) / GEV 0 / **GOOGL 3**(CHAIN-1a 어닝창 수리 전 0) / IREN 1 / IONQ 1 → **창내 0 = 2/6**(TLN·GEV). **★보정 3(status가 실질 레버)**: truth_score **max=0.85**라 ≥0.85 필터는 cap 집중으로 **confirmed 필터와 사실상 동치**(confirmed 2,234 중 ≥0.85가 2,139=95.7%). 즉 파라미터 확정 시 실질 조정 레버는 truth 임계가 아니라 **relation_status 등급**(probable 포함 여부 등)·top-k·창. truth 임계 미세조정은 거의 무효. **신규율 ≥3% 지속 시(별건⑨) kind별 신규 분해(EARNINGS/DIVIDEND/SPLIT 각 first_seen)로 귀인 실증** — 월초·분기 경계(배당 선언 유입 vs 어닝 창 롤링) 가설을 데이터로 확정/기각.
- ✅ **[EVT-CHAIN-1B] 이벤트 위젯 상단 pill 밴드 + 타임라인 앵커 (P1) — 구현 완료(2026-09-04·sess-evt-9·커밋 로컬·push 대기)** (상세=[[DECISIONS]] D-EVT-CHAIN-1B·목업 §a P1): FE만. `UpcomingEventsBand`(신규·상태 카드 아래 附加) = 어닝/배당 pill + "관계망 N ↓"(하단 타임라인 smooth scroll)·이벤트 전무 시 비표시. `ChainSection`=타임라인만(위젯 제거·앵커 id=chain-timeline). `UpcomingEventsWidget` 삭제(밴드로 이관). **행위보존**: page.tsx diff=import 1+밴드 삽입 1블록+주석·상태 카드(SlimStrip)/사다리/신호/근거/일지 diff 0. **P3 기각**(상태 카드 편입=모니터 소유 침범). BE·API·파라미터 무변. FE vitest 밴드 3신규+회귀(chain 15·MonitorDetailPage 7)·tsc0·lint순증0·전체 1269 pass(1 기존 red=guideAnchors chainsight.backbone·무관).
- 🐛 **[GUIDE-ANCHOR-BACKLOG] guideAnchors `chainsight.backbone` 선존 red (등재 2026-09-04·GUIDE 트랙 백로그)** — `__tests__/guide/guideAnchors.test.ts` 고아 앵커 실패: `components/chainsight/BackboneView.tsx`가 `data-guide="chainsight.backbone"` 앵커를 박았으나 가이드 데이터 미선언. EVT-CHAIN 무관·전 세션부터 선존(FE vitest 1269 pass 중 유일 red). 해소=가이드 데이터에 앵커 선언 or BackboneView 앵커 제거 → **GUIDE 트랙 소유**.
- 📋 **[MON-JOURNAL-1] 모니터 상세 일지 개편 (점수 차트 + 접힌 일지) — 초안 등재·모니터(구 thesis) 트랙 소유** (디렉터 초안 2026-09-04·`docs/instructions/MON-JOURNAL-1.md` 커밋만·**집행 금지**): 시각 계약 §(b)+(c). 발단=9/3 실화면 "일지 화면 과점·날짜×종합점수 그래프". 제안=점수 추이 차트(관제 개시→오늘·0 기준선·코멘트 마커)+일지 접기(최근 3일). **결정·집행은 모니터 트랙 사이클**(재료 실측: MonitorSnapshot 시계열·`snapshot_series.py`/`sparkline.py` 재사용성). EVT 트랙 범위 밖.
- 💤 **[EVT-CHAIN 후속 후보]** — D군 환류(CompanyEventReaction·어닝 반응 히스토리 저장) · P2-ii(어닝 반응 히스토리 `/stable/earnings` 채택·[[D-EVT-P2-SOURCES]]) · symbol_b 단독 인덱스 부재(§0-4⑵ DB 드리프트·규모 커지면 마이그 검토) — 전부 EVT-CHAIN-1 범위 밖·후속 슬라이스.
...
- ✅ **RC-WATCHDOG-DAPHNE-COVERAGE** — **라이브 확인 2026-09-01**. `sv sync`로 런타임 트리 `9caf9e37` 전진 → `88f87a48` INCLUDED, 런타임 `celery-watchdog.sh`의 `check_service` **4건**(`Web (daphne)` 포함) 확인. watchdog은 StartInterval 잡이라 재등록 없이 다음 발화부터 적용됐다. **어제의 '배포창 편승 대기'는 앱 레이블 오조회에 기반한 불필요한 보류였음**(DECISIONS 정정). ⚠️ 효과 한계는 유효: web은 `KeepAlive=true`라 프로세스 사망은 이미 자동복구되고, 잡 언로드 시엔 kickstart도 실패하므로 **실질 효과는 경보**. hang 미감지. 후속 후보 = `RC-WATCHDOG-HTTP-PROBE` · `RC-WATCHDOG-MAINTENANCE-MUTE`(억제 장치 없음).
...
- ✅🔬 **[RC-PAIR-DEDUP] 프로브 종결 — "부풀림" 전제 불성립(무조치 후보)** — RC-D-0(09-07) 실측 정정: ⑴ **(a,b,relation_type) 진짜 중복 triple = 0**·unique_together `[symbol_a,symbol_b,relation_type]` **이미 존재·완전 충족** ⑵ "동방향 중복 111쌍"(CP∧max>0)은 제약 위반이 아니라 **같은 pair의 서로 다른 relation_type**(다관계·상이 파이프라인 write, 예 COST-TPR=DEPENDS_ON+SUPPLIES_TO)·무향 collapse 아티팩트 ⑶ **부풀림 = N**: `pair_aggregation.py:84,87`=MAX·backbone build_relation_graph=pair max collapse(합산 아님). **실효 제약 후보 없음**(신설 불요). 델타 재현 = max>0 산식 2,418→2,251(−167·데이터 성장, RC-C-1 −166은 과거 스냅샷)·지시서 "market_score>0" 문구 오류(−0). **처분 후보: 종결(무조치)** or 관찰 유지 — 디렉터 판정.
- ✅ **[RC-D-0] 관계 store 이중화 전수 실측 = 측정 완료·처분 설계 대기** — 보고서 `docs/reports/rc_d0_store_dedup_recon.md`(09-07, worktree `sv-rc-d0`). read-only(코드/PG/Neo4j 쓰기 0). STEP 1~4 완료. 디렉터 안건 = ⓐ SR 197k HELD+47k SAME_REG 처분(D 결정 핵심) ⓑ SR 활발 유입(13 beat) ⓒ Neo4j 이중노드(symbol 1,146⊎ticker 534·교집합 0) ⓓ RC-PAIR-DEDUP 종결 ⓔ Neo4j test 엣지 3건 유출 ⓕ 3-4 델타 −167(HALT 문구 대비 판정). **처분·통합·삭제·제약 신설 = 다음 사이클.** push "푸시" 대기.
- ✅ **[MP2-DOGFOOD-RECONCILE] tests/dogfood/test_targets.py 2 RED (귀책=MP2-SUBPAGES)** — `test_every_screen_declares_anchors_and_is_confirmed`·`test_guide_targets_are_loaded_from_guide_data`가 `marketPulse.macro: draft`로 실패(review_status='draft'≠'confirmed'). **소유 트랙=MP2-SUBPAGES**(draft를 confirmed 승격 or dogfood allowlist 처리 — 소유 트랙 판단). 비고: **origin/main baseline RED**(RC-C-1 역머지 시점 `88f87a48` 이후 확인, HOTFIX-1 `c6a902bd` 후에도 잔존 여부 재검증 필요)·**RC-C-1 델타 무관**(`tests/dogfood/`·`frontend/lib/guide/`·`market-pulse-v2/` 경로 diff 0·`chainsight.backbone` guide_data 미등록 grep 0). RC-C-1 push는 디렉터 게이트 예외 승인으로 진행. **★정정·해소(2026-09-15, GUIDE-MACRO-REVIEW STEP 0 실측)**: dogfood 2 RED는 **`113b48a3`(GUIDE-CS-GUARD-1 S1+S3, main 착지 `0bfd185e`)에서 이미 해소**됐다 — ⑴ `test_every_screen_declares_anchors_and_is_confirmed`는 `_has_parsed_review_status`로 **개명·완화**(`assert g.review_status in {"draft","confirmed"}`)되어 **draft를 더 이상 실패시키지 않는다**(draft 화면도 등재 가능·채점 선별은 `rubric_targets` 몫) ⑵ `test_guide_targets_are_loaded_from_guide_data`의 기대 집합에 `/market-pulse-v2/macro`가 **이미 포함**. 실측 baseline = **103 passed / 0 failed**. **GUIDE-MACRO-REVIEW의 기여는 RED 해소가 아니라** `marketPulse.macro` confirmed 전환(= draft 문안이 루브릭 밖에 머물던 상태 종료, `rubric_targets` 5→6)이다.
...
- 🟡 **OPS-HEALTHCHECK-NIGHTLY-WIRE** — **구현·랜딩 완료 · launchd 등록만 상신 대기**(2026-09-01). 상신 `scratchpad/OPS-HC-WIRE_상신_20260901.md`. 옵션 A(독립 잡 `com.stockvis.healthcheck` **05:40 KST** + 별도 메일) = `D-HC-NIGHTLY-WIRE`. 신설 `auto_agent_system/healthcheck/`(run_healthcheck.sh·report_health_mail.py·plist) + `auto_agent_system/common/mail.py`(dogfood와 `send()` 공유). 산출물은 **트리 밖** `~/stock-vis-nightly/health/`(구 wrapper의 트리 내부 쓰기 = sv sync 충돌 원인 → 은퇴, deprecated 헤더). **수동 1회 실증**: JSON 18건(❌0) 생성 + 메일 실발송 1통 + 런타임 트리 무오염. 유닛 26 passed. **배포창 의존 없음**(미적용 마이그레이션 0건 — 어제의 '편승 대기' 판단은 앱 레이블 오조회였음, DECISIONS 정정 참조). 순서 = `sv sync` → plist bootstrap → 익일 05:40 발화 확인.
...
- 🕒 **SCB-CONTEXT-LAYER** — 채점 카드에 애널리스트 논거·현재 상황 비교 맥락 추가(병진 09-02 소감: "가격만 나오니 그렇구나 싶다"). **✅ recon 완료(2026-09-10, SCB-CONTEXT-RECON·보고서 `docs/mgmt/SCB-CONTEXT-RECON_report.md`) · **✅ 원천 결정 확정 = `D-SCB-CONTEXT-SOURCE-1`(DECISIONS, 2026-09-17)**: 논거 축=`/stable/grades`(등급 변경 사건 타임라인) · 맥락 축=기존 `NewsEntity` 624,562행 조인(수집 0) · `/stable/grades-news`=**트리거 보류**(S2 착지 후 연결 정확도 부족 소감 시 규모 1콜 후 재평가) · `grades_historical`=폐기 아님, "분포 추이" 표시로 흡수. 가중합 안1 3.80/안2 4.30/안3 3.55/안4 4.30 · 마진 0 → 타이브레이커 "게이트 가용성은 휘발성 자산". **🕒 S1 보류 · S2 선행으로 순서 반전(디렉터 결정 2026-09-18, `D-SCB-CONTEXT-SOURCE-1 정정`)** — 맥락 축 **GREEN→AMBER**. 근거 = recon이 잰 것은 **조회 가능성**이지 **귀속 정확도**가 아니었음. 교차 근거 2건(둘 다 하한값·독립 측정): **CS-S3** `docs/reports/cs_s3_comention_provenance_recon.md`(09-15 prod) 주어 적중 **18/50=36%** · **SCB-CONTEXT-S1**(09-17) 제목 관련성 **464/925=50.2%**(IREN 76.2%~**GEV 10.5%**). 실물 = GEV miss 신호 ±3일 뉴스 5건 중 GE Vernova 기사 **0건**. 가중합 안1 3.20/안2 3.55/안3 3.05/**안4(S2 선행) 4.60**·마진 1.05>1.00 자동 결정. **S1 선행조건 = "뉴스↔종목 귀속 규칙 교정"** — **CS-S3의 S3-2 결정 사이클과 묶어서 다룬다**(각자 규칙을 만들면 갈라진다). **S1 재개 시 payload 상한 = base 87,012 B 대비 +15%(약 +13 KB)** — 본 세션 실측 인라인 3건 = **+310.2%**로 **상한의 20배** → 인라인 3건은 재개 시에도 **불성립**(lazy 로드 전제). 재개 시 어댑터 위치 = **앱 계층**(`news_source.py:6-8`·D-BOUNDARY-NO-DYNAMIC-EVASION; 구 S1 지시서 §3-6 "shared에 두되"는 **철회**). §1 실측 정본 = `docs/mgmt/SCB-CONTEXT-S1_recon.md`(1-E 원문 3세트 포함 — 재개 시 목업 재료).
- 🟡 **SCB-CONTEXT-S2** (**진행 중** — §1 완료·모델/클라이언트/배선/테스트 landed·**백필만 `승인 BACKFILL` 대기**, 2026-09-21) — **논거 축 `/stable/grades` 수집**. **저장 구조 확정 = `D-SCB-GRADES-KEY-1`**(6필드 unique + `source_row_count`, DECISIONS 2026-09-21). **적재 범위 = 유니버스 9심볼**(`_coach_universe`) — 스냅샷의 `SMR`·`XE` 2종은 제외(유니버스 정의를 단일 진실로·D-I1b-1 정합). **배선 = 후보 ②**(`apps.portfolio.tasks.ingest_analyst_grades` 신설·기존 4콜 경로 무접촉·halt 패턴 복제). **beat 등록 = 병진 수동**(#28, DB PeriodicTask). 산출: 모델 `AnalystGradeChange`+마이그 `stocks.0019`(prod 적용 완료) · `FMPClient.get_grades()` · `grade_change_writer`(dedup+멱등 upsert). ✅ **§3-5 백필 집행 완료(2026-09-22, `승인 BACKFILL`)** — 외부 콜 9 · fetched **6,137** · created **6,136** · updated 0 · halted False · 20.2s. 사후 대조 **전건 일치**(9심볼 각각 DB행 == §1-B 키 수, 차 0 · min/max date 전건 일치 · 2012-02-08~2026-09-18 · action maintain 5,466/downgrade 349/upgrade 321 = §1-B에서 완전중복 1행 접힌 값과 정합). `source_row_count>1` **전수 1건** = `AAPL | 2023-06-16 | Jefferies | Buy→Buy | maintain | count=2`(예상과 일치). 재수집 6,137 = 09-19 수집과 동일 → **09-19 이후 신규 등급 변경 0**. ★남은 것 = **beat 등록·enable**(D-DSS-BEAT-1 2단 스위치·병진 수동·#28). **S3(카드 노출)는 별 슬라이스.**
- 🔴 **FMP-RATELIMIT-GUARD-SWEEP** (별 세션·이번 슬라이스 범위 밖) — **FMP 외부 호출 태스크 전수**에서 `FMPRateLimitError` 터미널 halt 가드 유무를 조사하고, 없는 곳에 `capture_symbols` 패턴을 복제한다. **발견 1건 = `ingest_stock_splits`**(`apps/portfolio/tasks.py:136`, 디렉터 실측 2026-09-22): 예외 처리가 `except (KeyError, ValueError, TypeError): continue` 뿐이라 **`FMPRateLimitError` 미포착·continue** → 카운터 소진 후에도 남은 심볼을 계속 두드린다. 대조군(가드 있음) = `capture_symbols`(`analyst_signal_writer.py:137`) · `ingest_analyst_grades`(SCB-CONTEXT-S2 신설) = **패턴 불일치**. **이번 세션에서 수리하지 않음.**
- 🕒 **SCB-S2-DUP-SEMANTICS** — 같은 날·같은 기관의 **상이 등급 변경 6군 성격 미확인**(전체의 0.1%). 실재 사건인지 FMP 데이터 오류인지 판별 안 함(디렉터 결정 ②, 2026-09-19: 둘 다 보존하고 진행). 실측 사례 = `TSLA/2018-05-05/Argus`(downgrade+upgrade 공존) · `GOOGL/2023-06-28/UBS` · `NVDA/2018-05-05/B. Riley FBR` 등. **판별 시점 = S3 표시 규칙 설계 때 또는 `grades-news` 트리거 해제 시.**
- 🕒 **SCB-S2-IONQ-COVERAGE** — `IONQ` 등급 변경이 **2024-08-12 이후 공백**(30행이 2021-11-22~2024-08-12에만 존재). 2년 넘게 신규 0건 — 정상인지 원천 커버리지 결함인지 **미확인**. S2를 막지 않음(디렉터 결정 ③). **커버리지 확인 필요.**
- 🔴 **SCB-CONTEXT-S3** (다음 슬라이스·지시서 대기) — **등급 변경 사건의 카드 노출**. S2가 적재한 `AnalystGradeChange`를 성적판 신호 카드에 표시. 선행 완료 = S2 적재. **주의**: S1 실측상 payload 상한은 base 87,012 B 대비 +15%(≈+13 KB)이고 `scorecard_cache_key`에 새 좌표 추가는 S3 사안(S2에서 무변경 유지). **CC 자기 착수 금지.** 프로브 실측(SCB-RECOVER-PROBE 2026-09-15): **200 OK · NVDA 1,158행 · 2012-02-13~2026-09-04 · `gradingCompany`/`previousGrade`/`newGrade`/`action`/`date` 실재** · 402 없음(현행 Starter 키). 미구현 = `packages/shared/api_request/providers/fmp/client.py`에 `grades` 메서드 부재. `grades-news`는 **트리거 보류**(D-SCB-CONTEXT-SOURCE-1). **CC 자기 착수 금지.**** 요지: **맥락 축 GREEN**(NewsEntity 614k·StockNews 98k 부활[#128 반전·beat ON]·좌표쿼리 sub-second·주입지점 E3 AnalysisContext budget 7000) / **논거 텍스트 RED**(grades_historical 100%채움이나 등급 카운트 추이일 뿐·FMP grades-historical numeric-only 확증·논거 텍스트 엔드포인트 미구현). rag_analysis 휴면(RED)·Neo4j/RC 14,072 생존(AMBER). 디렉터 결정 = 논거 축 (a)카운트추이 대체 (b)신규 FMP수집 (c)news 우회 택1. **✅ 논거 원천 프로브 완료(SCB-RECOVER-PROBE, 2026-09-15) — 논거 축 RED → GREEN 반전**: FMP `/stable/grades`(200·NVDA 1,158행·2012-02-13~2026-09-04·`gradingCompany`/`previousGrade`/`newGrade`/`action`/`date` 실재) + `/stable/grades-news`(200·`newsTitle`/`newsURL`/`newsBaseURL`/`newsPublisher`/`priceWhenPosted`/`publishedDate` 실재) **둘 다 현행 Starter 키로 즉시 수집 가능**(402 아님). 텍스트 입도=헤드라인+출처+URL+당시가(본문 prose는 FMP 미제공). → 디렉터 선택지 (b)신규 FMP수집이 **플랜 게이트 해제됨**. 미구현=`packages/shared/api_request/providers/fmp/client.py`에 두 메서드 부재. **stale 정정**: StockNews 0행·beat enabled=False 기록 → 위 NEWSFIX-SYNC-BE 행에서 정정 완료. cost_ledger "측정 위치 오류" 가설은 **반증** — 런타임 3트리 모두 `docs/portfolio/coach/cost_ledger.jsonl` 31행(본진과 동일·append 0건)·마지막 기록 **2026-05-26** → coach LLM 휴면 또는 로깅 미작동. 설계 슬라이스 발행 대기.
...
- 🔵 **GUIDE-CS-REFRESH 2단계** (@frontend) — 정문(`/chainsight`)용 `chainsight.feed` 가이드 신규 등재(문구·앵커 작성). **트리거: `monorepo/sess-s3s1`(묶음 카드·부제 정직화) main 머지.** 착수 시 화면 재측정 후 문구 작성 → 병진 검수 → confirmed. 등재 후 `chainsight.main`의 임시 이설 주석을 정규 상태로 정리. **[2026-09-20 트리거 충족 + 기한 연장]** 착수 트리거였던 `monorepo/sess-s3s1`(및 `-b`·`-c`)이 **전건 origin/main 착지 완료 — 지금 착수 가능**(디렉터 실측). 연동된 `chainsight.backbone` 앵커 유예는 `D-GUIDE-ANCHOR-DEFER-1031`로 **2026-10-31까지** 연장됐다. 미이행 시 그날 `guideAnchors.test.ts`가 RED.
...
- 🔴 **GUIDE-ORPHAN-DASHTABS** (@frontend — **소유: dashboard 앱 트랙**) — `dashboard.tabs` 앵커(`components/eod/DashboardTabs.tsx:28`, `006d48cf` DASH-TAB 09-09)가 가이드 데이터 미등재 = 고아. **기한 2026-09-30**까지 ⑴ `lib/guide/dashboard.ts`에 등재(문구 작성) 또는 ⑵ `data-guide` 속성 제거 중 하나. **미이행 시 `guideAnchors.test.ts`의 기한 테스트가 자동 RED**가 되어 전 랜딩을 막는다. **[2026-09-20 기한 연장]** `D-GUIDE-ANCHOR-DEFER-1031` — 기한 **2026-09-30 → 2026-10-31**(1회 연장, 재연장은 소유 트랙의 명시적 요청·근거가 있을 때만). 미이행 시 `guideAnchors.test.ts`가 그날 스스로 RED가 되고, **이 앵커와 무관한 세션까지 guide 스위트에서 막힌다.** 소유 = dashboard 앱 트랙. **[2026-09-22 해소]** 연장은 불필요해졌다 — dashboard 트랙이 `cb246e46`(GUIDE-TABS-1, 09-22 09:49)로 해결 ⑴(`lib/guide/dashboard.ts` 등재)을 집행하고 `PENDING_ANCHORS`에서 `dashboard.tabs`를 제거했다. **이 항목 종결.**
