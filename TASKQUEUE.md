...
| MACRO-SENSE-TUNE | 정적 문장 임계 재튜닝 — 물가 갭 1.0%p · breadth 0.5중심 사다리 | @frontend | 실데이터 누적 | 🕒 **TUNE 대기(선제 조정 금지)** | 물가 갭 1.0%p = 2022–23 재현용(올해 core 2.5~2.9·갭 0.5~0.9 미발동). breadth `BREADTH_THRESHOLDS`(0.6/0.7)=관례 앵커(dev DB n=1 미검증). 실 SPY breadth·물가 누적 후 재튜닝(concentrationBand TUNE 선례 동일 규율). |
