...
| B1-S1 | 원시 시리즈 백필 (깊이 A 조인트 천장 **2023-07-10**, D-B1-SCOPE). 대상 = FRED 9(NFCI×4·HY×2·T10Y3M·VIXCLS·T10Y2Y) + yahoo 2(VIX3M·MOVE) + SPY OHLCV. 경로 = `backfill_v2_a1 --from 2023-07-10`(창·멱등·FREDClient 단일). 신규 커맨드 0, 스키마 무변경 | market_pulse 트랙(병진 수동 실행) | **병진 수동 백필 실행 대기** → 실행 후 Part 4 검증 | 🔵 **계획 완료(후보 리포트 발행)** — 실행=병진, API≈12콜/신규≈5k행 |
| B1-S2 | 벡터 소급 합성 (별도 지시서) = 주간(NFCI) forward-fill 일간 정렬 + 14벡터 소급 합성 + rules.yaml 소급 적용(과거 국면 재구성, D-ANALOG-GATE 원문 경로). S1 백필 land 의존 | market_pulse 트랙 | **B1-S1 실행 완료 후** | 🕒 대기 |
