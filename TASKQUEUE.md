...
| MP2-SECTOR-CD | 섹터 판단 화면(CD 4-상태). 순차 연속 슬라이스(D-TREND-CD-SEQ). rel_strength×momentum_5d 사분면 분류(신규 파생 0, baseline=0.0). RRG 회전 맵 = 서브스크린(D-SECTOR-NAV, Slice 3) | **#2.5** | @backend+@frontend | 🔵 **in-progress (Slice 2)** — S1 land(`32c2390`) | 판정 로직 = payload builder 단독. FE 재분류 금지 |
...
| MP2-SECTOR-CD-S2 | Slice 2 = 모멘텀 시계열. sector_history[] per-date momentum_5d additive 노출(저장값) + `cd_momentum_baseline` 메타 + FE 모멘텀 모드(디폴트=순위 유지)+판정선 hline(서빙값)+국면 스트립 레인(D-SECTOR-MOM-LANE 변형2, `regime_history_30d` 소비) | @backend+@frontend | 🔵 **in-progress** | momentum=저장 노출(재계산0, STEP 0-2: 528행/48일/NULL0). 변형2 확정 |
| CD-FLAP-WATCH | (관측) cd_state 뱃지 일간 반전 빈도 관측 — rel_strength가 1일 차분(sector_m1−bench_m1)이라 일간 플래핑 리스크 이론상 존재. Slice 2(모멘텀 시계열)+실데이터 수일 축적 후 판정. 잦으면 히스테리시스/장창 x축 안건 오픈 | @backend | 🔭 **관측** | **지금 코드 수정 금지.** 판정 데이터 = 며칠 축적 후 |
| MP2-SECTOR-CD-S3 | Slice 3 = RRG 회전 맵 서브스크린(D-SECTOR-NAV 옵션 B, 라우트 1 추가) + Slice 1 카드에 회전 맵 어포던스 활성화 | @frontend | 🕒 대기 (S2 land 후) | 판단 카드→회전 맵 진입. Phase 2 서브페이지 첫 패턴 |
