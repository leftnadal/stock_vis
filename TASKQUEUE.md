...
- 🔭 **[EVT-OBS-1] collect-calendar-events 첫 자동 발화 관찰 게이트** — beat 등록 완료(DB PeriodicTask, 17:45 ET, enabled). **검증 예정일 = 2026-08-28**(첫 자동 발화 08-27 17:45 ET 이후). **증거 = last_run_at 아님, DB 행**: ⑴ 당일 last_seen_at 갱신 행 수(재관측 다수) ⑵ 신규 행 수(<1% 기대) ⑶ stale 전이 수 ⑷ bisect 텔레메트리(성분별 depth·extra_calls). **에스컬레이션**: bisect depth 4 도달 또는 콜 상한(12) 근접 시 → 상한/청크 재결정 사이클 소집. 초회 적재(EVT-IMPL-3) 기준선 = 13,229행(scheduled 12,180·occurred 1,021·stale 28).
