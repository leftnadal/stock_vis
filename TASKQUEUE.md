...
| GUIDE-MACRO-REVIEW | 거시 허브 가이드(`marketPulse.macro`) 검수 → draft→confirmed 전환 | 사용자/병진 | MP2-SUBPAGES-S1 land | 🟡 **검수 대기(병진)** | reviewStatus:'draft'로 착지(guideData 테스트 allowlist 등재). coreQuestion="오늘 국면의 거시 근거는 무엇인가"·regions 4(심리/금리/지표/글로벌). 검수 후 confirmed 전환 + allowlist에서 제거. **연동**: 이 confirmed 전환이 `MP2-DOGFOOD-RECONCILE`(dogfood 2 RED = marketPulse.macro draft) 해소 경로. **⚠ HUB-V02-S2 랜딩 후 검수**(S2가 허브에 정적 SenseNote 층을 추가 → 가이드 문구가 새 표면과 정합해야 함). |
...
| HUB-V02-S2 | 허브 v0.2 S2 — 전역 서브탭 + 정적 번역층(거시·로테이션) + 홈 fallback | @frontend+@backend | HUB-V02-S1 | ✅ **코드 done (2026-09-07, FE 전용)** | D-MP2-SUBNAV(A)·D-MACRO-SENSE-STATIC(ⓐ). ⑴ `MarketPulseSubNav`(v1 미포획·개요/거시근거/로테이션/무버스) ⑵ `macroMeaning` 4종(공포탐욕·수익률·물가고용·4지수) ⑶ 허브 위젯 아래 SenseNote 형제(위젯 diff 0) ⑷ 홈 **breadth·sector만** 정적 fallback(AUTO-1="카드당 의미문 1개 보장"·regime/concentration 중복 0) ⑸ 로테이션 RRG 한 줄. **백엔드 diff 0**. vitest mp2 403 GREEN(+macroMeaning 26·subnav 11·fallback 3단·중복게이트)·tsc 0·E2E **4스펙 2회 연속 GREEN**(서브탭 왕복 신규). 실렌더 스크린샷 4(홈/허브 데·모/로테이션). **잔여=push+web 리빌드(런북 2.2) 병진 승인**. 정확 카피=디렉터 목업 검수. |
