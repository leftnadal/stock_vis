...
| MP-TRANSLATION-S5 | golden + vcr 하네스 (+ Brief 동반 보강) — 실 LLM 출력 계약 가드 | @backend + @qa | MP-TRANSLATION-S4 | 🟢 **진행 중** (2026-06-19) | **★ STEP 0: vcrpy 미설치·카세트 인프라 0 → HALT 보고 → 사용자 결정 스냅샷 fixture 방식**(실 Gemini 1회 녹화 JSON → golden 재생, 신규 의존성 0·CI 무네트워크). golden 계약: JSON 구조·카드당 길이·`llm.safety` 금지어/refusal 0·**밴드 방향 모순 부재(약한 계약, meaning.ts 임계 복제 0=§10)**. Translation 3~4 시나리오 + Brief 1세트(golden 부재 종결). 마이그레이션 0, 프로덕션 로직 무변경. **본체 완결(S2~S5) + BOUNDARY-LLM 트리거(a) 충족 플래그**(차터 무접촉) |
