...
| SCAN-B2-TECH-BE | ② 기술축 베이커 보강 **(선행·초저비용)** — 캘큘레이터 기계산 `rsi_14·high_52w·dist_52w·sma_50/200`를 tagger 고정 키셋 드롭 **해제** + baker `_build_preview_stock` 서피스. 재계산 0. **shared 위임** | platform (shared) | RECON-VALUATION-R1(done) | 🆕 **todo(선행)** | D-SCAN-B2-DERIVE. tagger `eod_signal_tagger.py:204-224` + baker:376. |
| SCAN-B2-FUND-BE | ② 펀더축 베이커 보강 **(후행·중비용)** — statement enrichment(baker preload 패턴)·**TTM dedupe(period_type)**·밸류=market_cap÷TTM·퀄리티=ROE/margin/debt/current·**sector 중앙값 집계(n·폴백 표기)**. 정칙 ⑶강화·⑷·⑺. **shared 위임** | platform (shared) | SCAN-B2-TECH-BE | 🆕 **todo(후행)** | D-SCAN-B2-DERIVE. payload +30% 수용. |
| SCAN-B2-FE | ② 축 칩 점등 — **1차=TECH 칩(TECH-BE 착지 후)·2차=FUND 칩(FUND-BE 착지 후)** | @frontend (dashboard) | SCAN-B2-TECH-BE | 🆕 **todo(대기)** | 정칙 ⑴로 미착지 칩 자연 생략(슬롯 additive). |
...
| P16-PLAYBOOK-CHAIN | Phase 1.6 본론 = 플레이북 체인 정의(감지 조건→서술 카드) — **결정 사이클 선행** | @UI-UX+@backend | 결정 대기 | 🕒 **정찰 완료·결정 대기(1.6-S0 §2-B 재료 첨부)** | 엔진=선언형(`anomaly/rules.yaml` id/name/against/threshold·현 4규칙 R02 concentration/R04 vix/R09 sector_z/R12 dispersion). stress_input 훅=classifier.py:113 **스텁(받고 `del`·미배선)**=체인 접점. 즉시 조건화 가능 신호(추가 수집 0): VIX/VIX3M·MOVE·HY/IG 크레딧(BAMLH*)·커브 T10Y2Y/T10Y3M·달러 DTWEXBGS(daily)·NFCI/STLFSI4(weekly)·anomaly ctx(top10_weight·vix_change_pct·max_abs_sector_z·cross_dispersion)·stress 5축(vol/credit/curve/fin_cond/price). 표면=CardShell 재사용 신규 CardSummary(hero 하단 그리드). 체인 *제안*은 결정 사이클 몫 |
...
- **CS-P5-FE-CARD 후속 고도화** (CS-P5-GO Phase B 후속·미랜딩분은 `monorepo/sess-p5-fe-card` 랜딩 후) — ⑴ 카드 필터·정렬(현재 검색 1개만·OUT였음) ⑵ 테마 슬롯 로직·데이터 연결(현재 placeholder만·**VOCAB-TAU 선행 대기**) ⑶ 카드 시각 고도화(연결 밀도 표현·방향 시각·그룹 캡 상호작용) ⑷ 접근성·가상화(펼침 카드 대량 시). (신규)
...
- **CS-SECTOR-BACKFILL 미분류 154 sector 백필**(CS-P5 R-1 정밀화·조치 금지) — no-sector Stock **154 전량 기존 683**(신규 72는 FMP 프로필로 sector 기충족·0건). industry도 154 전건 동반 결측. exchange=NASDAQ103/NYSE41/None10. **무콜 백필 가능=SP500Constituent 캐시 sector 보유 6뿐**(BF.B·BRK.B·LLY·MRVL·AVGO·WMT), **나머지 148=FMP `/stable/profile` 재콜 필요**(148콜 예산). 판정=병진(무콜 6 우선 반영 or 148 예산 승인). (신규·R-1)
