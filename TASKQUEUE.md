...
| MP-TRANSLATION-S4 | envelope serializer + FE selector + fallback — overview 최상위 `translations` 블록(cards 무변경) + 카드 sense 소비 + 3상태 fallback | @backend + @frontend | MP-TRANSLATION-S3 | 🟢 **진행 중** (2026-06-18) | BE: overview `_build_payload`에 `translations` 블록(senses+model_version+generated_at+status, 없으면 null) + serializer + cards 불변 가드. FE: 얇은 selector(`translations?.senses?.[key] ?? null`) + 4카드 optional `sense` prop(dumb 유지) + fallback(null/누락/빈 senses → 밴드만, 에러·빈줄 없음). 밴드·raw additive 불변. **★ 카드 키=senses 키 정합(HALT 없음).** 마이그레이션 0(모델 무접촉). 다음=S5(golden+vcr) |
