...
| CS-EXP-U2 | **유니버스 편입(U2)** — 테마 ETF holdings의 비SP500 US 종목을 Stock 유니버스에 편입해 그룹 밀도↑. **게이트(중앙값≥10) 통과의 실제 유일 경로**(ETF 추가로는 자격 그룹 수만↑, 중앙값 불변 — LOAD에서 데이터 확정) | @backend | CS-EXP | **ready** (게이트 병목 확정, 디렉터 U2 범위 결정 대기) | `CS-EXP-LOAD_measurement.md` 구조적 결론 / `StockSyncService.sync_overview` 메커니즘 |
