...
| MP2-ALERTS | 능동 알림. **범위 피벗**: Slice 0 = **regime 전환 → 이메일 1통**(구 ops 모니터링 아님). **경계 = D-ALERTS-BOUNDARY-R1(개정)** — 알림 코어(디스패처·정책·AlertLog·AlertSubscription·delivery port) **packages/shared 신설**, 앱 문구는 registry 주입. 구조 = 3단(D-ALERTS-ARCH). 채널 = 이메일(D-ALERTS-CHANNEL) | **#2** | @infra+@backend | 🟡 **STEP 0 done(0fd70ea) — Slice 0은 승계 게이트 대기** | STEP 0 실측 완료(M1~M8). **⚠ 승계 게이트(디렉터 판단)**: `services/serverless/ScreenerAlert`+`AlertHistory`가 사용자 알림 프레임워크(구독·cooldown·채널 플래그·이력) 상당 부분 기존재(delivery 미구현) → shared 코어 신규 vs serverless 일반화 결정 필요. `services/news/AlertLog`(ops, 이름 충돌). 상세 = DECISIONS D-ALERTS-BOUNDARY-R1 STEP 0 실측 |
| MP2-ALERTS-S0 | Slice 0 = regime 전환 트리거(intraday `mp_calc_regime_15min` 격리 지점, dedup 필수) → 3단 파이프라인 → 이메일 1통. **신규 마이그레이션 발생**(AlertLog·AlertSubscription @ shared) → **후보 보고 → 병진 수동 `manage.py migrate`** 절차 포함. 착수 = 승계 게이트 해소 후 | @infra+@backend | **승계 게이트 해소 대기** | prod 마이그레이션 첫 발생 트랙(MP2-DELTA는 migration 0이었음) |
...
| P1-B-WORKER-WORKTREE | **worker 전용 worktree**(`~/worktrees/sv-worker-runtime` detached origin/main) + `celery-worker.sh` PROJECT_DIR/plist 수정 + OUTPUT 서빙 위치 심링크 + `scripts/worker_sync.sh`(fetch+re-detach+재기동 원커맨드) 신설 — 브랜치 표류 트레드밀 종료. **설계 확정 D-B-WORKER**, 스크립트 예외 D-OWN-B-WORKER. 참조 D-P1-OBSERVE-DONE 결함① | ops/infra | **실행 승인** | ✅ 실행 승인(착수가능) |
...
| P1-BEAT-PRECHECK | 월요일 18:30 ET beat 전 점검: `7c2f186..0fd70ea`(+전진분) diff에 **bake 경로 코드 포함 시** 공유 트리 re-detach + 워커 재기동(ops 원포인트). **B′(P1-B-WORKER-WORKTREE) 완료 시 대체됨** — 월요일 beat 전 B′ 미완이면 원포인트 fallback **1회**만 | ops 원포인트(B′ 대체 예정) | **월요일 beat 전(B′ 미완 시)** | 🆕 등재(B′로 대체 예정) |
