...
| MP2-SECTOR-CD | 섹터 판단 화면(CD 4-상태). 순차 연속 슬라이스(D-TREND-CD-SEQ). rel_strength×momentum_5d 사분면 분류(신규 파생 0, baseline=0.0). RRG 회전 맵 = 서브스크린(D-SECTOR-NAV, Slice 3) | **#2.5** | @backend+@frontend | 🔵 **in-progress (Slice 3, 마지막)** — S1 `32c2390`·S2 `20d2734` land | 판정 로직 = payload builder 단독. FE 재분류 금지. **S3 land 시 트랙 종결 → 차순위 = B-1(ANALOG·TREND S4 이중 언락 키)** |
...
| MP2-SECTOR-CD-S3 | Slice 3 = RRG 회전 맵 서브스크린(D-SECTOR-NAV 옵션 B, 라우트 1 추가) + Slice 1 카드에 회전 맵 어포던스 활성화 + `cd_rel_strength_baseline` 메타(x축 판정선). 데이터=기존 단일 fetch(sectors[]+sector_history, STEP 0-3 신규 엔드포인트0) | @backend+@frontend | 🔵 **in-progress** | 판단 카드→회전 맵 진입. 꼬리 5일(D-CD-TRAIL) raw 폴리라인 재분류 금지. **land 시 MP2-SECTOR-CD 트랙 종결** |
