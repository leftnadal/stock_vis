...
| DEPLOY-RUNBOOK | 런타임 동기 절차 정의(주기·체크리스트·runbook) — **api 런타임 포함**(스테일 런타임 **3개 실증: worker+web+api**) | @infra | — | 🕒 **등재(D-MPS-OPS-SYNC/WEBSYNC/APISYNC 파생)** | **공유 런타임 = 병렬 세션 공용 싱글톤. 런북 1장=고아 스윕, 2장=동기 절차, 3장=집행 감사 기록.** worker: ff checkout→migrate --plan→check/health→창확인→kickstart→PT무손상. web: ff checkout→**빌드 먼저**(npm run build·.next 백업=무중단 폴백)→성공 시 kickstart→스모크. **api(daphne)**: ff checkout→kickstart com.stockvis.web→스모크(authed 200). **자동 동기 메커니즘 = `worker_sync.sh`(수동 진입점 `sv sync`, 자동 스케줄러 부재) — 런북이 흡수할 기존 자산**. 재기동 cadence **불규칙 = 세션 구동 이벤트 상시 존재 실증**(08-18 14:57 api 동기=병렬 세션 sv sync). **함정: launchd kickstart가 관리 이탈 orphan(PPID→1)을 못 죽여 EADDRINUSE(:3000 web / :18765 api) → orphan 수동 kill 후 재kickstart**(common-bugs). 스테일 런타임 트리 재발 방지 |
| SYNC-AUDIT-LOG | `worker_sync.sh`에 집행 감사 로그 1줄 추가(시각·pid·ppid·호출 컨텍스트=tty/parent) — 미귀속 동기 실행 구조적 재발 방지(D-MPS-OPS-APISYNC 파생, 08-18 14:57 귀속을 로그 3중 추론으로 특정한 비용 제거) | @infra (`scripts/worker_sync.sh`) | — | 🕒 **소형 하드닝 등재(이번 세션 수리 금지)** | 현재 worker_sync는 실행 주체 자취를 안 남겨 사후 귀속이 reflog+web.log+프로세스 포렌식 3중 대조에 의존. 진입 시 `echo "[sync] invoked pid=$$ ppid=$PPID tty=$(tty) at=$(date)"` 1줄로 다음 미귀속 사건의 특정 비용 소거 |
| MPS-OPS-GLD-DASH | 티커 스트립 GLD 시세 "—"(누락) — Stock 테이블에 GLD 레코드 자체 부재 확인 | @backend | — | 🔎 **소형 조사 후보(MPS-OPS 관찰, 이번 세션 수리 금지)** | DailyPrice 이전에 Stock 시드 없음. 스트립이 GLD를 기대하나 종목 미등록 → 시드 필요 여부·대체 소스 판단은 별건 |
