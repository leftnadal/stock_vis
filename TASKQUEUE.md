...
| MKX-URLNORM-BACKFILL | S3(지시서⑦)가 AV/FMP url 정규화를 **forward-only** 적용 → 기존 저장 raw url 은 무변경. 정규화 통일 시 병합됐을 **cross-provider 혼합 충돌 145그룹**(전체 충돌 281그룹·잉여 ~3,954행, 단 소수 대형 그룹=빈/플레이스홀더 url 가능성 caveat) 잔존. 기존 중복 병합 backfill(정규화 url 기준 dedup + NewsEntity 재귀속)은 **행위 변경 리스크 높음**(병합=co-mention 카운트 변동) → 별도 설계·승인 필요. | @backend | co-mention 이중카운트 교정 필요 시 | 🆕 저우선(backfill 후보, 신중) |
