...
| CD-STAB | (측정, 차순위) cd_state 안정성 **축별 분해 측정** — CD-FLAP-WATCH의 일평균 반전율 0.611을 x축(rel_strength=1일 차분) 부호변화 기여 vs y축(momentum_5d) 부호변화 기여로 분해. 어느 축이 플래핑 주범인지 정량화 → 히스테리시스/장창 x축 설계 결정 입력 | @backend | 🔭 **차순위(측정 우선)** | CD-FLAP-WATCH 후속. 읽기전용 소급 측정(코드 수정 0). 디렉터 히스테리시스 결정 근거 데이터 |
