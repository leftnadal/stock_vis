...
| AGENT-S1-DEPLOY | 운용본 배치 + launchd 등록(05:20 KST) | 병진(수동) | AGENT-S1 착지 · **RC-A-1 배포창 ②③④** | ⛔ **HALT 지속(2026-08-31 갱신) — RC-A-1 혼재창 이미 개방** | **08-31 갱신**: 혼재창은 **이미 열려 있다**(08-29 11:24 타 세션 sync로 런타임 `408cb20e`에 `23318e25` 유입·마이그 0033 여전히 미적용). **오염 0건**(v3.0 2,282행 중 `>1.0` 0건·버전×눈금 일치)이나 **읽기 왜곡 1건 실측**: dashboard strip θ=p85=**60.0**(구 눈금) SQL 필터라 **신규 2,282행이 배지에서 누락**. 마이그는 멱등이라 지금 실행이 곧 해소책 → **우선순위 반전(빨리 닫을 것)**. 교정 명령문 = scratchpad `RC_A1_배포창_교정본_20260831.md`. — **원래 차단 사유(08-28)**: 배치에 필요한 `sv sync`가 worker 트리를 origin/main으로 re-detach한 뒤 **celery-worker·beat를 무조건 kickstart**한다(`worker_sync.sh:155-160`, 조건부 아님). 동기 구간(런타임 `9460430f` → origin/main)에 **RC-A-1 D-RC-SCALE 코드(`23318e25`)가 들어 있고 마이그 `chainsight.0033`은 prod DB 미적용**(`migrate --plan` 실측: Alter field score_version + Raw Python). 지금 sync하면 **[0,1] 눈금 코드 + [0,100] 눈금 DB로 워커가 기동** = D-RC-DEPLOY-WINDOW가 막으려는 **혼재창**을 여는 것. RC-A-1 병진 순서는 ②worker·beat **정지** → ③migrate 0033 → ④재기동. **선행 해소 후 재개**: RC-A-1 ②③④(최소 ③) 완료 → `sv sync` → `run_dogfood.sh` 1회 → `launchctl bootstrap`. 현재 런타임은 구 눈금 코드로 **정상 가동 중**(23318e25 미포함 확인) — 방치해도 무해. launchd 단독 등록도 불가(런타임 트리에 `auto_agent_system/dogfood/` 부재 → 매일 실패). 명령문 = scratchpad `AGENT_S1_상신_20260827.md`. `sv-worker-runtime/.env` 심링크 확인됨. |
