...
| CS-EXP | 테마 ETF holdings 확대 + 유니버스 U2 편입 + 백필 (디렉터 지시서) | @backend | CS-RD1·CS-UNIV | **in-progress** (STEP0 `714b8fd` / (c)복구 2/6 `a7a245e` / GATE·SOURCE 측정 `cccb6c8`·`32c47aa` / LOAD `7de1416` — XBI·KRE·PAVE 적재. **보정 게이트 미달: 중앙값 7<10**. 이전 측정 수치는 snapshot 누적 행수 오류였음) | `Cs exp universe expansion.md` + `CS-EXP_STEP0_findings.md` + `CS-EXP-LOAD_measurement.md` |
| CS-EXP-U2 | **유니버스 편입(U2)** — 테마 ETF holdings의 비SP500 US 종목을 Stock 유니버스에 편입해 그룹 밀도↑. 게이트 통과의 유일 경로 | @backend | CS-EXP | **decided** (디렉터: 게이트 X=8 + 전체편입 136종 확정 `ad2fca2`+U2SIM. 실행=CS-EXP-U2EXEC 대기) | `CS-EXP-U2SIM.md` / DECISIONS "CS-EXP-U2 결정" |
| CS-EXP-U2EXEC | U2 실행 — 136종 sync_overview 편입 + DailyPrice 90일 백필(종목당1콜, FMP≤1500, 실패율≤5% HALT) + 게이트 재측정(목표 중앙값≥8) | @backend | CS-EXP-U2 | **ready** (실행 계약 대기) | 예측: 자격9·중앙값26, 유니버스 535→~671 |
