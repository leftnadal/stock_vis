...
| L2-ADOPT | D-L2-SOURCE 결정 2 구현 — Peer LLM 태그 채택 + 판정기 거부권 파이프라인(A단계). | @backend/@frontend | — | ✅ **A단계 완료(무비용, 미머지)**: STEP0 스키마 게이트 PASS·A1 거부권 0.2 재집계(1/240=CRM↔MTCH)·A2 파이프라인(`peer_domain_tagging`·`tag_peer_domains`·훅·pytest16)·A3 파일럿(119채택/1거부권)+마인드맵 llm_tag 우선(vitest12)·A4 견적. 커밋 4슬라이스(`b009427a`·`8ef222b4`·`d480b29d`+원장). 리포트 `docs/chain_sight/L2_ADOPT_A_STAGE_REPORT.md`. **머지=병진 승인 대기.** |
| L2-FULL-SWEEP | **B단계** — 전량 태깅(9,161쌍) `tag_peer_domains --apply`. 견적(A4): Gemini 무료 7일 분할($0, 일 1,500·idempotent 재개) vs 유료 단일 세션(~$4.4·추정). 착수 직후 마이크로배치 5-10콜 실단가 확정. | @backend/@infra | **★병진 견적 승인 게이트(비용 발생)** | 🆕 등재(B단계, 착수 전 승인 필수) |
