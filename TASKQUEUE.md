...
| **MP1.5-FIX** | **단일 FE 슬라이스** — ⒜ A1 brief 모달 `body` fallback 매핑 ⒝ A2 authAxios refresh 인터셉터 401 재시도 ⒞ A3 `<Pie label>` 포맷터(`toFixed`)+레이블 겹침 처리 ⒟ "cache: MISS" 엔드유저 노출 정리 ⒠ **① 유효 종목 수(1/HHI) 카드 표시** | @frontend | ✅ **완료 (2026-06-25, `2c9fbca` + 시각검증) — A3-tail 잔존으로 "조건부"** | D-P15-SCREENGATE. 시각검증 실측: A1 본문·① 유효종목수(≈51종)·cache 가드(dev전용, 프로덕션 비노출 코드입증)·회귀 = PASS / A2 = vitest 갈음 / **A3 = 대형조각·%포맷·미반올림 해소, 좌상단 소형조각 라벨 겹침 잔존(P2) → MP1.5-A3-TAIL 분리**. 커밋 `0f86e55`(A1)·`9529671`(A3)·`a079870`(cache)·`2c9fbca`(①) |
| **MP1.5-A3-TAIL** | **A3 도넛 좌상단 소형조각 라벨 겹침** — `ConcentrationDetail.tsx` `<Pie label>` 좌상단 인접 소형조각(META≈3.3%·GOOG≈5.4% 부근) 라벨 겹침 잔존. 임계 각도/최소표시비중 하향 또는 leader-line 충돌 분산 | @frontend | 🟢 **active 후보 #1 (착수 가능)** | D-P15-SCREENGATE A3 조건부 잔여. frontend 한 곳(`ConcentrationDetail.tsx`). 대형조각·%포맷·미반올림은 MP1.5-FIX에서 해소 완료 → 잔여는 소형조각 겹침만. **통과 시 Phase 1.5 게이트 완전 종결 → Phase 2 진입.** 다음 세션 (가) 착수 대상 |
...
>
> **Phase 1.5 종결 조건(2026-06-25, D-P15-SCREENGATE)**: MP1.5-FIX 5건 중 4건(A1 본문·① 유효종목수·cache 가드·회귀) 실측 PASS + A2 vitest 갈음 = 시각 게이트 **조건부 통과**. **잔여 = MP1.5-A3-TAIL(도넛 좌상단 소형조각 라벨 겹침, P2) 단 1건.** A3-tail 통과 시 Phase 1.5 게이트 **완전 종결 → Phase 2(#1 MP2-ANALOG) 진입**. 기존 트랙 유지: MP2-ANALOG(#1)·MP2-ANALOG-COND-RESULT(②③ 데이터게이트)·MP2-DATA-BREADTH-CONC(B1)·MP2-E2E-SAFETYNET·MP2-MOBILE-EYECHECK.
