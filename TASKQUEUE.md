...
| MPS-OPS-D1 | 익일 beat 자동 실행으로 DTWEXBGS·STLFSI4 신규행 유입 확인(수동 트리거 성공 ≠ 스케줄 성공) | @infra | MPS-1-DEPLOY | 🕒 **D+1 확인(2026-08-15)** | OPS-NEWS-FRESHNESS 교훈. mp_sync_fred_indicators_daily 스케줄런(17:40 NY) 후 두 series latest 전진 확인 |
| DEPLOY-RUNBOOK | 런타임 동기 절차 정의(주기·체크리스트·runbook) — **web 런타임 포함**(스테일 런타임 **2개 실증: worker+web**) | @infra | — | 🕒 **등재(D-MPS-OPS-SYNC/WEBSYNC 파생)** | worker: ff checkout→migrate --plan→check/health→창확인→kickstart→PT무손상. web: ff checkout→**빌드 먼저**(npm run build·.next 백업=무중단 폴백)→성공 시 kickstart→스모크. **함정: launchd kickstart가 관리 이탈 orphan(PPID→1)을 못 죽여 EADDRINUSE :3000 → orphan 수동 kill 후 재kickstart**(common-bugs). 스테일 런타임 트리 재발 방지 |
...
- **SCE-POLLUTION-CLEANUP fuzzy-era SCE 오매칭 정제**(CS-P4 A-1·**R-2 정밀화 2026-08-18**) — **R-2 실측 정정**: A-1 "38 distinct serving_layer=evidence 존재"는 raw SCE(v1 포함) 기준. **실 서빙(`current()`→RC evidence SEC4종) 유입 = 1건**(`FTNT→DIS`=Marvell Technology→DIS **오 CompanyAlias 시드**·fuzzy 아님); 나머지 37(Ablecom→ALGN·Cerner→CVX·Arrow→EA·Change→HCA·Epic→INCY 등)은 **v2 supersession으로 target=NULL 무력화·미착지**. **부수 self-loop 2건**(`V→V`·`GS→GS`=자회사→모회사 자기티커·D-CARD-GATE 연결선 오염). 정제 = ⑴오 alias(Marvell→DIS) 삭제 ⑵착지 self-loop 가드(source==target 드롭) ⑶(선택)37 NULL-target 잔여 SCE 정리. 수정=별도 승인(A-1/R-2는 목록·카운트만·무수정). (신규·R-2 반영)
