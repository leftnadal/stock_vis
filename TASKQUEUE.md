...
| MACRO-SENSE-PLACEMENT | `SenseNote` 배치 — 위젯 카드 바깥·맨 아래(설명 대상과 ~700px 이격) | @UI-UX-designer→@frontend | — | 🕒 **등재만·착수 금지 (2026-09-15, GUIDE-MACRO-REVIEW에서 분리)** | `SenseNote`가 위젯 카드 **바깥·맨 아래**에 붙어 ⑴ 설명 대상 값과 **700px 가까이 떨어지고** ⑵ **위젯 자체 교육 토글보다 아래**에 온다(오늘의 적용이 일반론 밑에 깔림). 근거=병진 라이브 스크린샷(09-14). **★`HUB-SENSE-DETAIL`의 선행 결정 대상** — 토글을 SenseNote에 다는 슬라이스라 배치가 먼저 정해져야 한다. |
