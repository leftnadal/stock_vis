...
| MACRO-CARD-NA-ARROWS | 거시 허브 지수 카드 — `change_percent` 전건 null → 전 행 빨간 하락 화살표 + "N/A" | @frontend | — | 🕒 **등재만·착수 금지 (2026-09-15, GUIDE-MACRO-REVIEW에서 분리)** | 페이로드 `change_percent`가 전건 null인데 `IndexRow`의 화살표 분기(`change_percent && >= 0`)가 **falsy로 떨어져** 전 행이 하락(빨강)으로 렌더된다 — 값은 "N/A"인데 방향은 단정. 섹터 pill 정렬도 전부 0 동률이라 **순서가 무의미**. 근거=디렉터 실측(09-10). 원인이 페이로드(백엔드)인지 렌더(FE)인지 **분리 진단 선행**. **[2026-09-17 S1 검수 증거]** 원인이 **읽는 필드 불일치**로 좁혀졌다(실측): `globalIndicesSentence`는 `change`(절대 변화) 부호만 보고, `IndexRow`는 `change_percent`를 그리는데 전건 `null`이라 `N/A`+하락 화살표로 떨어진다(`macroMeaning.ts` 주석에 `change 부호만(change_percent 미사용)` 명시 = S2 의도 설계). 결과적으로 **한 카드 안에서 표는 전 행 N/A인데 의미문은 "지수 간 방향이 갈렸습니다"라고 말한다**(라이브 스크린샷 확증). S1이 의미문을 카드 바로 아래로 끌어올려 **모순이 같은 시야에 들어왔다** → 우선순위 상향 근거. 분리 진단 선행 원칙 유지. |
...
| MACRO-SLAB-RADIUS | 설명 트레이 모서리 반경이 카드와 불일치 | @frontend | — | 🕒 **등재만 (2026-09-17, S1 검수 발견)** | `SenseSlab`이 `rounded-b-lg`(8px), 위젯 카드가 `rounded-xl`(12px). 조립체 아래 모서리가 위보다 각지고 좌우에 미세 단차. 확대해야 보이는 수준 → 랜딩 차단 아님. `rounded-b-xl` 한 글자 수정 → **S2에 묶는다**. |
| MP2-CORS-DEVPORT | dev CORS가 :3000만 허용 — 워크트리 캡처 포트에서 API 차단 | @backend | — | 🕒 **등재만 (2026-09-17, S1 캡처 발견)** | `config/settings.py:331` `CORS_ALLOWED_ORIGINS`가 `:3000`만 허용. 런타임 트리와 별개로 워크트리를 다른 포트(:3100)에 띄워 실렌더 캡처할 때 pulse 응답이 막힌다. S1은 백엔드 diff 0을 지키려 **캡처용 chromium에만 `--disable-web-security`**로 우회(repo·설정 무수정·렌더값은 라이브 실응답). 반복 작업이므로 dev 한정 포트 범위 허용 검토. 추가 관측: `127.0.0.1`이 아니라 `localhost`로 접속해야 요청이 나간다. |
