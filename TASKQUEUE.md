...
| B1-S2 | 벡터 소급 합성 (D-B1-SYNTH) = **현행 로직 as_of 매개변수화**(별도 forward-fill 발명 없음 — max_age 캐리가 주간 NFCI 정렬 자연 수행) + 시계열 순차 hysteresis chaining + rules.yaml 소급 적용(과거 국면 재구성, D-ANALOG-GATE 원문 경로). S1 백필 land 의존 | market_pulse 트랙(커맨드 land=Claude Code / 합성 실행=병진 수동) | **커맨드 land(`monorepo/sess-b1-s2`: as_of `f347dc0` + 커맨드 `1710b54`) → dry-run 후보 리포트 → 병진 실행 대기 → Part 4** | 🔵 **in-progress** — as_of 매개변수화(additive, 회귀 8) + `backfill_v2_regime_vectors`(멱등·불가침 3중 방어·dry-run) 커맨드 land. dry-run 실측(prod 무쓰기): 703 영업일·완전벡터 **683**·분포 TRANSITION469/LATE_BULL228/CRISIS6 |
| B1-S2-FIRE | **B-1 트랙 종결 발화 항목(사전 등재)** — 병진 합성 실행 + Part 4 불가침 검증 통과 **시** 발화: ⑴ **MP2-ANALOG un-dorm** — D-ANALOG-GATE 트리거 (b)"B-1 백필 land 후 소급 벡터 재구성 검증 통과" 충족(+ (a) 다양성도 dry-run서 비-LATE_BULL 475일/CRISIS 6 관측 = 게이트 개방). ⑵ **MP2-TREND S4 동면 해제** — TREND 트랙 "S4 트리거 = B-1 land(Phase 5)" 충족. | market_pulse 트랙 | **병진 실행 + Part 4 통과 시 발화** | 🕒 발화 대기(land 후) |
