...
| FE-8000-PROD-APPLY | sv-web-runtime을 `af1e37a`로 갱신(`worker_sync`/re-detach) + **재빌드**(NEXT_PUBLIC_API_URL 빌드 인라인 → 재빌드 필수) + **:3000 재기동**. GATE TRUE(.env.local 키 존재) 확인 완료(MERGE-CLEANUP-2). :3000 서빙 부활 + impression 수집 재개 + **WEB-RUNTIME-RUNBOOK 초안 채록 겸함** | @infra | FE-DEAD-8000-SWEEP 착지(완료) | 🆕 **등재 (집행 예정, MGMT-BATCH-10)** |
| WEB-RUNTIME-RUNBOOK | :3000 서빙 실체(**launchd 입양 고아 `npm run dev`**, sv-web-runtime) 성문화 — dev/prod 구분·기동/재빌드·번들 검증 절차. `.next.old-*` 격리분 정리 포함. **입력 초안 = FE-8000-PROD-APPLY 집행 시 채록되는 절차 기록** | @infra | FE-8000-PROD-APPLY 집행 | 🔼 **상향 (저우선→표준, MGMT-BATCH-10)** — 근거 ⑴ :3000 고아 프로세스 사멸 실측(07-16, ⑳ 검증 중 발견 = 도그푸딩 impression 수집 중단 리스크 실증) ⑵ FE-8000-PROD-APPLY 선결 절차와 동일 매듭 |
