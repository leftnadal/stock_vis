...
| C2-S1-BUILD | **스테이지 1 빌드(D-C2-S1-* 3결정 구현)** — **S1-B1** = ⓐ-1 창밖 노출 라벨("창밖 N·90일 내 전량 매칭"·항등식 검증). ⚠**news_chip 강등 분기 발동** → ⓑ-3 표면 분해 FE(합산 헤드라인)는 **S2 결정 후 재론**, S1-B1은 **ⓐ-1 라벨 단독**으로 축소. **S1-B2** = ⓒ-3 surfaces 상수 모듈 단일화 + 가드 테스트(모듈 위치 build STEP 0 실측·공용 인프라 판명 시 shared 위임 분기). 순서 **S1-B1 → S1-B2**. | @frontend/@backend (구획 build STEP 0 확정) | C2-S1-DESIGN(종결) | 🆕 **등재(다음 순번 본류)** |
