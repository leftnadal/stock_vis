...
| NEWS-URLNORM-IDQUERY | 🔴 **S3 배포 전 선결.** 현행 `normalize_news_url`가 쿼리 **전량 제거** → **id가 query에 있는 URL(youtube `?v=`·finviz `?t=`·CMS `?idxno=/?no=`)을 오충돌**시킴. S3가 이 규칙을 AV/FMP로 확대 → 배포(worker_sync) 후 신규 수집에서 distinct 페이지가 첫 1건으로 붕괴(2건째부터 미저장/오병합) 위험. 지시서⑧ 실측: 충돌 잉여 3,954행 중 **3,815(96%)가 이 과다strip 착시**. **교정 = 정규화를 "tracking-param(utm_*/fbclid 등)만 제거, id-쿼리 보존"으로 정밀화**(지시서⑧ Part B 분류기 로직). | @backend | **worker_sync로 S3 배포 전** | 🔴 선결(배포 게이트) |
