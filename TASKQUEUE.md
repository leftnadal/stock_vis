...
| SCAN-B2-FUND-BE | ② 펀더축 베이커 보강 **(후행·중비용)** — statement enrichment(baker preload 패턴)·**TTM dedupe(period_type)**·밸류=market_cap÷TTM·퀄리티=ROE/margin/debt/current·**sector 중앙값 집계(n·폴백 표기)**. 정칙 ⑶강화·⑷·⑺. **shared 위임** | platform (shared) | SCAN-B2-TECH-BE(done) | 🆕 **설계 대기(입력=사용자 화면 소감 + payload 실측 가드)** | D-SCAN-B2-DERIVE. **STEP 0 payload 실측 가드 필수**(D-SCAN-R1-CORRECTION 관찰 ⑵·TECH 이미 +26%)·초과 시 symbol-ref 축약 편입 상신. |
| SCAN-B2-FE | ② 축 칩 점등 (기술 칩 1차) | @frontend (dashboard) | SCAN-B2-TECH-BE(done) | ✅ **착지+배포 완료 (LAND-SCAN-B2FE `418b2a8e` 08-27 · DEPLOY-EXEC-2 `9460430f` 08-28)** | D-SCAN-B2TECH-CONTRACT enum 맵. 번들 마커 6종+baked 402/403 확증·점등 조건 성립. **스캐너 아크 ①FE(B1·B2-TECH·B2-FE) 완결·라이브**. cf. D-SCAN-DEPLOY-CORR. |
...
| RC-NEO4J-WORKER-TREE | **launchd 실행 트리 교정** — `celery-worker-neo4j` + 전수 점검에서 나온 동류 2건 | @infra | — | 🟡 **상신 대기(기동 금지 유지)** | **08-31 ops 세션 전수 점검 완료 → 상신 `scratchpad/RC-NEO4J-WORKER-TREE_상신_20260831.md`.** 결함은 **단건 아닌 부류**: plist 3건(`celery-worker-neo4j`·`celery-watchdog`·`pg-backup`)이 Desktop 지향 + **래퍼 스크립트 3건이 `PROJECT_DIR`을 Desktop 하드코딩** → **plist만 고치면 무효**(래퍼가 다시 Desktop으로 cd). 교정 초안 = `scripts/ops/launchd/*.plist.proposed` 3건(plutil lint OK) + `exec-tree-selflocate.diff.proposed`(self-locate, 선례 `verify-pair.sh`). 안전 게이트 = 런타임 트리 `8cfbcabb`에 RC-A-1 v3.0 `23318e25` **INCLUDED**(origin/main 0/0) 확인 후에만 bootstrap. **밀림 실측 = 실효 0건**(dirty 2,831 전량 `target_symbol_id IS NULL` → sync 대상 아님; Redis `neo4j` 큐 28건 no-op 적체) → 정지 유지 비용 0, 서두를 이유 없음. 집행=병진 수동. |
...
| MP2-SUBPAGES | v1 위젯 5종(FearGreedGauge·YieldCurve·EconomicIndicators·GlobalMarkets·MarketMovers) v2 하위 페이지 흡수 (= `MP-V1-ABSORB` 실행) | **#3 (A)** | @frontend | ▶️ **next (트리거 충족·D-P2-ENTRY-1)** | P2-DLITE(D-lite) land로 트리거 충족. **P2-DLITE STEP 0-7 정찰**: macro 4종(FearGreedGauge·YieldCurveChart·EconomicIndicators·GlobalMarketsCard)=props 구동(상위 `app/market-pulse/page.tsx` `useMarketPulse`→macroService→`/api/v1/macro/*`)·이식 난도 낮음(presentational). MarketMoversSection=자체 fetch(useMarketMovers/Sync/Keywords)·난도 중(훅 동반). v2 라우팅=page.tsx + rotation/(서브 1). |
...
- 🔴 **RC-LAUNCHD-WATCHDOG-TREE** (@infra, 우선순위 2) — `com.stockvis.celery-watchdog` plist + `scripts/celery-watchdog.sh` 실행 트리 교정. Desktop 트리의 `.env`·Django `send_mail`을 구 코드로 실행 중. 초안 = `com.stockvis.celery-watchdog.plist.proposed` + self-locate diff. **집행 순서: neo4j 워커 교정·검증 합격 후에 올릴 것**(미교정 잡을 kickstart 방지). 의존 = RC-NEO4J-WORKER-TREE.
- 🟡 **RC-LAUNCHD-PGBACKUP-TREE** (@infra, 우선순위 3) — `com.stockvis.pg-backup` 동일 교정. DB 쓰기 없음(오염 위험 0)이나 Desktop `.env`로 DB 접속정보를 결정 → 트리 간 `.env` drift 시 **엉뚱한 DB 백업 = 조용한 실패**. 현재 로드·정상(runs 39, exit 0). 초안 = `com.stockvis.pg-backup.plist.proposed`.
