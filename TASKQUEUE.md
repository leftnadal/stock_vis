...
| **[GATE:release] MP-LIVE-VERIFY** | **Phase 1 출시 전 필수 — 라이브 검증 게이트**. 서버 기동 + `GET /api/v2/market-pulse/overview` 200 응답 + 5 card_id 각각 `/cards/<id>/detail` 응답 + `page.tsx` 실 렌더(5 Summary + CardDetailContainer 펼침 + AnomalyPanel + NewsPanel + StatusBanner) 대조 | @qa + @frontend | MP1-K · MP1-L · MP-KL-F3 | 🚦 **차단(release)** — **선결 전부 충족(2026-06-11)**: F3 health 판정 완료 + F2로 최종 계약(card_id=concentration) 확정. **게이트 실행 준비 완료, 유일 잔여 release blocker** (서버 기동 + 라이브 렌더 대조만 남음) | 검증 로그(스크린샷 + curl 응답) + DECISIONS push 라인. 검증 통과 시 DECISIONS에 "K/L 라이브 확정" 1줄 + 본 행 "통과" 표기 |
