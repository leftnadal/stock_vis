...
- ✅ **[EVT-CHAIN-1] Phase 2 관계망 이벤트 타임라인 + 미니 위젯 — 구현 완료(2026-09-03·sess-evt-8·커밋 로컬·push 대기)** (상세=앵커 §6·[[DECISIONS]] D-EVT-CHAIN-1): 시드+RelationConfidence 1-hop 이웃 어닝, Postgres 단독 조인. BE `chain_feed.build_chain_feed`+API `calendar/chain/`·FE 4컴포넌트+scope=stock 附加. **★파라미터 실측 정정: truth_score≥85 → ≥0.85**(도메인 [0,1]·D-RC-SCALE·§0-4⑴). confirmed·top-k10·EARNINGS만·부호중립. 원장 재작업 0. BE pytest 15신규(monitor 50 green·architecture green)·FE vitest 35(신규11+회귀+게이팅2)·tsc0·lint순증0. 확정=D-EVT-CHAIN-THRESH([EVT-OBS-3] 게이트).
- 🔭 **[EVT-OBS-3] EVT-CHAIN 파라미터 확정 관찰 게이트 (등재 2026-09-03)** — 2주 사용(**≈2026-09-17**) 후 판정: ⑴ 시드별 표시 행(items) **0이 과반**이면 truth 임계 인하 검토 / ⑵ 행 **>15 시드** 존재하면 top-k·창 재검토 → 디렉터 결정 사이클(**D-EVT-CHAIN-THRESH** 확정). **기준선(§0-4⑶·4-2 실측 6종목)**: PLTR 이웃10·창내3 / TLN 0·0(타임라인 미표시) / GEV 2·0 / GOOGL 10·0(다음=배당 09-04·창 협소) / IREN 2·1 / IONQ 1·1. **관찰=창내 행 0이 4/6**(GOOGL은 배당창 아티팩트) → 임계/창 조정 후보 신호. truth_score max=0.85라 ≥0.85가 사실상 confirmed와 근접(2,139/2,234).
