...
| SLICE19C-ENGINE | Slice 19c 배치 엔진 v2 — 드로다운 비례 다이얼 + 손잡이 5종(A/G/w/L/E, 사용자 주권·기본 보수) + 원장 2종(PortfolioSnapshot·AdvisoryRun). 하드 10%/30% → 산식·L 대체(의도 변경). 코어 랭킹 하이브리드(신뢰도 0.60+진입가 0.25+통화여력 0.15, w 재정규화·상한 0.20 신뢰도 지배 불변식) + 탐험 레인 E(젊음<30일). 원칙 3계층 재정의(예측 3계층). 계약 v3. **결정론·LLM 0**. base `ef8990c`·baseline 602 | portfolio 트랙 | 🔄 진행 중(`monorepo/sess-slice19c-dial`, 2026-07-16) | 🔄 진행 |
