...
| MPS-OPS-GLD-DASH | 티커 스트립 금·은 "—" — 심볼 배선 불일치(402 아님) | @backend | — | 🟡 **S1 코드 done·데이터 커맨드 병진 대기(HUB-V02-S1, 09-03)** | 근본원인=RECON Part E(GCUSD/SIUSD 127행 있으나 sector_group=None 제외 / GLD·SLV BENCHMARK 0행). **S1 코드**: `_ticker_bar` null-price 스킵(GLD/SLV "—" 제거·live 15심볼 확증) + TickerBar 금/은 라벨. **잔여(병진 승인 후)**: `fix_commodity_ticker_wiring --commit`(GCUSD/SIUSD→BENCHMARK, dry-run: 각 127행). 402 반증. |
...
| MP2-DATA-BREADTH-CONC | **Breadth** — 날짜 off-by-one 수리(HUB-V02-S1) | @backend | — | 🟡 **S1 코드 done·소급 재계산 병진 대기(09-03)** | ★수집 미배선 아님=`calculators/breadth.py:112` `localdate()`로 EOD 조회→항상 0. **S1 코드**: `resolve_as_of_date`(직전 거래일)·compute·store 일원화·`_breadth_card` 실데이터 우선+`as_of_date`·`indicator_stale` 신선도 기반. **live 확증**: compute=501/299/201. **잔여(병진 승인 후)**: `recompute_breadth_history --commit`(dry-run: 792 거래일·108행[106개 0]·21 purge후보). Concentration은 실제 정상(64행·top10 0.3812). D-BREADTH-ASOF. |
| HUB-V02-S1 | 허브 v0.2 신뢰 수리 3건(Breadth 날짜·Anomaly 정직화·금은 배선) | @backend+@frontend | D-HUB-V02 | ✅ **코드 done (2026-09-03)·데이터 커맨드 병진 대기** | A-1 Breadth(→MP2-DATA-BREADTH-CONC)·A-2 Anomaly(→HUB-ANOMALY-HONESTY)·A-3 금은(→MPS-OPS-GLD-DASH). pytest +15·vitest +5·E2E 3스펙 2회 GREEN·tsc0·marketpulse 540 무회귀. dry-run 2건 확보(집행 0). **잔여 = push+web 리빌드 + 커맨드 2건(recompute·commodity wiring) 병진 승인**. |
