...
urlpatterns = [
    ...
    # 바구니 아이템 추가
    path(
        "baskets/<int:pk>/add-item/",
        DataBasketAddItemView.as_view(),
        name="basket-add-item",
    ),
    # 바구니에 주식 데이터 추가 (데이터 타입 선택)
    path(
        "baskets/<int:pk>/add-stock-data/",
        DataBasketAddStockDataView.as_view(),
        name="basket-add-stock-data",
    ),
    # 바구니 아이템 제거
    path(
        "baskets/<int:pk>/items/<int:item_id>/",
        DataBasketRemoveItemView.as_view(),
    ...
    # 세션 목록/생성
    path(
        "sessions/", AnalysisSessionListCreateView.as_view(), name="session-list-create"
    ),
    # 세션 상세/삭제
    path(
        "sessions/<int:pk>/", AnalysisSessionDetailView.as_view(), name="session-detail"
    ),
    # 세션 메시지 목록
    path(
    ...
    # 채팅 스트리밍 (SSE)
    path(
        "sessions/<int:pk>/chat/stream/", ChatStreamView.as_view(), name="chat-stream"
    ),
