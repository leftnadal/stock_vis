...
class TokenBudgetManager:
    ...
    def __init__(self, complexity: str = "moderate"):
        ...
        self.budget = self.DEFAULT_BUDGETS.get(
            complexity, self.DEFAULT_BUDGETS["moderate"]
        )

    def allocate(
        self, content_blocks: List[ContentBlock], reserved_tokens: int = 0
        ...
        allocation_info = {
            ...
            "used": used_tokens,
            "remaining": available_budget - used_tokens,
            "utilization": used_tokens / available_budget
            if available_budget > 0
            else 0,
    ...
    def build_context(
        self, selected_blocks: List[ContentBlock], include_sources: bool = True
    ) -> str:
        """
...
class DynamicBudgetManager(TokenBudgetManager):
    ...
    def adjust_budget_for_cost(
        self, current_daily_cost: float, daily_limit: float
    ) -> Dict[str, int]:
        """
        비용 상황에 따라 예산 조정
...
def get_dynamic_budget_manager(
    complexity: str = "moderate", cost_limit: float = 0.01
