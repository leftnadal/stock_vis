...
try:
    ...
    CACHE_OPERATIONS = Counter(
        ...
        [
            "operation",
            "result",
        ],  # operation: check/store, result: hit/miss/success/failure
    )
    ...
    TOKEN_TOTAL = Counter(
        "stockvis_tokens_total", "Total tokens used", ["type", "model"]
    )
    ...
    COST_USD = Counter(
        "stockvis_cost_usd_total", "Total cost in USD", ["model", "cached"]
    )
...
def record_analysis_request(
    status: str, cache_hit: bool, model: str, latency_seconds: float
):
    """분석 요청 기록"""
    ANALYSIS_REQUESTS.labels(
        status=status, cache_hit=str(cache_hit).lower(), model=model
    ).inc()
...
def record_cache_operation(
    operation: str, result: str, similarity_score: Optional[float] = None
):
    """캐시 작업 기록"""
...
def record_cost(cost_usd: float, model: str, cached: bool = False):
    """비용 기록"""
